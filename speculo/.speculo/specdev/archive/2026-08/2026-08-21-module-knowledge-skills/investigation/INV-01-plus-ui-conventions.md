...
frontend 前端编码、注释与动态权限路由的现行事实与 Skill 切片边界是什么？

## Research: frontend 前端编码约定、注释约定与动态权限路由

- Decision / target: 为后续在 `<Path>.agents/skills/namewta-fullstack-development/</Path>` 编写 Skill 提供一手证据地图；本文件为唯一 owning artifact。
- Scope / version: `<Path>frontend/</Path>` 当前工作树；对照后端菜单/权限接口 `<Path>ruoyi-vue-plus-namewta/ruoyi-modules/ruoyi-system/</Path>`。前端栈：Vue 3.5.40、TypeScript ^6.0.3、Vite ^8.1.5、Pinia 4.0.2、oxlint ^1.75.0、oxfmt ^0.60.0、pnpm@10.34.5、Node `>=20.19.0`（见 `<Path>frontend/package.json</Path>`）。
...
- Type: code fact
- Source: `<Path>frontend/package.json</Path>`（scripts / oxlint / oxfmt / vue-tsc / vitest）
- Confidence: high
...
- Type: code fact
- Source: `<Path>frontend/.oxlintrc.json</Path>`
- Confidence: high
...
- Type: code fact
- Source: `<Path>frontend/.oxfmtrc.json</Path>`；`<Path>frontend/package.json</Path>` scripts.fmt
- Confidence: high
...
- Type: code fact
- Source: `<Path>frontend/.editorconfig</Path>`
- Confidence: high
...
- Type: code fact
- Source: `<Path>frontend/tsconfig.json</Path>`
- Confidence: high
...
- Type: code fact
- Source: 对 `<Path>frontend/</Path>` 的 glob；`<Path>frontend/package.json</Path>`；`<Path>frontend/.oxlintrc.json</Path>`
- Confidence: high
...
- Claim: Prettier/ESLint 遗留仅出现在：(1) 手写 `// prettier-ignore`：`<Path>frontend/src/utils/request.ts</Path>`、`<Path>frontend/src/plugins/tab.ts</Path>`；(2) 生成声明头：`<Path>frontend/src/types/auto-imports.d.ts</Path>` 含 `/* eslint-disable */`、`/* prettier-ignore */`；`<Path>frontend/src/types/components.d.ts</Path>` 含 `/* eslint-disable */`、`/* prettier-ignore */`、`// oxlint-disable`。二者均被 oxfmt ignorePatterns 排除。
- Type: code fact
- Source: 上述三个文件；`<Path>frontend/.oxfmtrc.json</Path>` ignorePatterns
- Confidence: high
...
- Type: code fact
- Source: `<Path>frontend/vite.config.ts</Path>`；`<Path>frontend/vite/plugins/index.ts</Path>`；`<Path>frontend/package.json</Path>`
- Confidence: high
...
- Claim: 路径别名由 tsconfig `paths` 与 Vite `resolve.tsconfigPaths: true` 共同生效；`unplugin-auto-import` 自动导入 vue / vue-router / @vueuse/core / pinia 及 Element Plus resolver，dts 写入 `src/types/auto-imports.d.ts`。因此 `useStorage` 可在 `<Path>frontend/src/utils/auth.ts</Path>` 无显式 import（与 `<Path>frontend/src/store/modules/app.ts</Path>` 显式 import 并存）。
- Type: code fact
- Source: `<Path>frontend/tsconfig.json</Path>`；`<Path>frontend/vite.config.ts</Path>`；`<Path>frontend/vite/plugins/auto-import.ts</Path>`；`<Path>frontend/src/types/auto-imports.d.ts</Path>`
- Confidence: high
...
- Claim: oxlint/oxfmt 都 ignore `doc`，但 `<Path>frontend/docs/</Path>` 与 `<Path>frontend/doc/</Path>` 均不存在。
- Type: code fact
- Source: glob `frontend/docs/**`、`frontend/doc`；ignorePatterns 见 R-002/R-003
- Confidence: high
- Limits: 可能是上游遗留忽略项。
- Artifact impact: 不要把 `frontend/docs` 写成约定来源。

### R-011

- Claim: `src/` 下不存在 `TODO`/`FIXME`/`HACK`/`XXX` 标记。`待办` 仅出现在 workflow 业务文案（待办任务），不是代码债注释。
- Type: code fact
- Source: 对 `<Path>frontend/src/</Path>` 的 TODO/FIXME 与「待办」检索
- Confidence: high
...
- Type: code fact
- Source: `<Path>frontend/src/api/demo/demo/index.ts</Path>`；`<Path>frontend/gen/api.ts.ftl</Path>`
- Confidence: high
- Limits: 部分 API（如 `<Path>frontend/src/api/menu.ts</Path>`、`<Path>frontend/src/api/login.ts</Path>` 的 `getInfo`）只有单行 `//` 或无完整 JSDoc。
...
- Type: code fact
- Source: `<Path>frontend/src/views/demo/demo/index.vue</Path>`；`<Path>frontend/src/views/system/post/index.vue</Path>`；`<Path>frontend/src/views/monitor/operlog/index.vue</Path>`；`<Path>frontend/gen/index.vue.ftl</Path>`
- Confidence: high
...
- Type: code fact
- Source: `<Path>frontend/src/views/system/user/index.vue</Path>`（约部门树/对话框/dropdown 注意）；`<Path>frontend/src/views/demo/demo/index.vue</Path>`；`<Path>frontend/src/views/system/post/index.vue</Path>`
- Confidence: high
...
- Claim: 绝大多数 TS/Vue 文件没有版权头/`@author` 文件头。例外：`<Path>frontend/src/directive/common/copyText.ts</Path>` 有 `v-copyText` + `Copyright (c) 2022 ruoyi`。登录/注册页 Copyright 是 UI 文案，不是源码头。
- Type: code fact
- Source: 对 Copyright/@author 的检索；`<Path>frontend/src/directive/common/copyText.ts</Path>`；`<Path>frontend/src/views/login.vue</Path>`
- Confidence: high
- Limits: 未逐文件审计全部 src。
...
- Claim: 仓库没有独立“注释规范”文档。可观察约定仅来自真实代码与 gen 模板（R-012–R-015）。`<Path>frontend/src/router/index.ts</Path>` 顶部有一段路由配置项说明块注释（hidden/alwaysShow/roles/permissions/meta），属于框架自述，不是全仓注释风格。
- Type: code fact
- Source: `<Path>frontend/src/router/index.ts</Path>` 文件头；无 `CONTRIBUTING*`/`STYLE*`；无 `frontend/docs/`
- Confidence: high
...
- Claim: 应用启动在 `<Path>frontend/src/main.ts</Path>` 中 `import './permission'`（副作用注册全局前置守卫），并 `app.use(router)`、`directive(app)`、`app.use(plugins)`（挂 `$auth`）。
- Type: code fact
- Source: `<Path>frontend/src/main.ts</Path>`；`<Path>frontend/src/directive/index.ts</Path>`；`<Path>frontend/src/plugins/index.ts</Path>`
- Confidence: high
- Limits: 无。
...
- Type: code fact
- Source: `<Path>frontend/src/permission.ts</Path>`；`<Path>frontend/src/utils/validate.ts</Path>` `isPathMatch`/`isHttp`
- Confidence: high
...
- Type: code fact
- Source: `<Path>frontend/src/utils/auth.ts</Path>`；`<Path>frontend/src/store/modules/user.ts</Path>`
- Confidence: high
...
- Type: code fact
- Source: `<Path>frontend/src/api/login.ts</Path>`；`<Path>frontend/src/api/system/user/types.ts</Path>`；`<Path>ruoyi-vue-plus-namewta/ruoyi-modules/ruoyi-system/src/main/java/org/dromara/system/controller/system/SysUserController.java</Path>`；`<Path>ruoyi-vue-plus-namewta/ruoyi-modules/ruoyi-system/src/main/java/org/dromara/system/domain/vo/UserInfoVo.java</Path>`；`<Path>ruoyi-vue-plus-namewta/ruoyi-modules/ruoyi-system/src/main/java/org/dromara/system/service/impl/SysPermissionServiceImpl.java</Path>`；`<Path>ruoyi-vue-plus-namewta/ruoyi-common/ruoyi-common-core/src/main/java/org/dromara/common/core/constant/SystemConstants.java</Path>`
- Confidence: high
...
- Type: code fact
- Source: `<Path>frontend/src/router/index.ts</Path>`
- Confidence: high
...
- Type: code fact
- Source: `<Path>frontend/src/router/index.ts</Path>` `export const dynamicRoutes`；`<Path>frontend/src/store/modules/permission.ts</Path>` `generateRoutes`
- Confidence: high
...
- Type: code fact
- Source: `<Path>frontend/src/store/modules/permission.ts</Path>`；`<Path>frontend/src/utils/createCustomNameComponent.tsx</Path>`；`<Path>frontend/src/permission.ts</Path>` `isHttp`
- Confidence: high
...
- Type: code fact
- Source: `<Path>frontend/src/store/modules/permission.ts</Path>` `filterDynamicRoutes`；`<Path>frontend/src/types/router.d.ts</Path>`；`<Path>frontend/src/plugins/auth.ts</Path>`
- Confidence: high
...
- Claim: 指令注册名为 `hasPermi` 与 `hasRoles`（复数）。模板主流写法 `v-hasPermi="['module:res:op']"`；`<Path>frontend/src/views/system/user/index.vue</Path>` 混用 `v-has-permi`。Vue 将 camel/kebab 视为同一指令。无任何模板使用 `v-hasRole`/`v-hasRoles`（仅 error 文案举例）。指令在 `mounted` 时无权限则 `removeChild`；值必须是非空数组，否则 throw。`*:*:*` 视为全部权限。`hasRoles` 将 `superadmin` **或** `admin` 视为放行。
- Type: code fact
- Source: `<Path>frontend/src/directive/index.ts</Path>`；`<Path>frontend/src/directive/permission/index.ts</Path>`；`<Path>frontend/src/views/demo/demo/index.vue</Path>`；`<Path>frontend/src/views/system/user/index.vue</Path>`；全仓 `v-hasRoles`/`v-hasRole` 检索仅命中 directive 源文件
- Confidence: high
...
- Type: code fact
- Source: `<Path>frontend/src/plugins/auth.ts</Path>`；`<Path>frontend/src/utils/permission.ts</Path>`；`<Path>frontend/src/views/system/user/index.vue</Path>` import checkPermi
- Confidence: high
...
- Type: code fact
- Source: `<Path>frontend/src/layout/components/Sidebar/SidebarItem.vue</Path>`；`<Path>frontend/src/layout/components/TopBar/index.vue</Path>`；`<Path>frontend/src/store/modules/tagsView.ts</Path>`；`<Path>frontend/src/layout/components/AppMain.vue</Path>`
- Confidence: high
...
- Type: code fact
- Source: `<Path>frontend/src/views/login.vue</Path>`；`<Path>frontend/src/utils/request.ts</Path>` 拦截器与 `globalHeaders`
- Confidence: high
...
- Type: code fact
- Source: `<Path>frontend/src/layout/components/SocialCallback/index.vue</Path>`；`<Path>frontend/src/permission.ts</Path>` whiteList
- Confidence: medium
- Limits: 未跑 OAuth 流程。
...
- Claim: `frontend/docs` 不存在。父仓库确认约定的文档是 `<Path>docs/upstream/customization-map.md</Path>`：动态路由「后端已按 Client 过滤；前端继续 addRoute，不再做跨 Client 菜单过滤」；多 APP 用 `VITE_APP_CLIENT_ID`。
- Type: code fact
- Source: glob `frontend/docs` 空；`<Path>docs/upstream/customization-map.md</Path>` 前端表
- Confidence: high
...
- Claim: 子模块内另有 CRUD 编码 Skill：`<Path>frontend/.codex/skills/frontend-crud-coding/SKILL.md</Path>` 与 `<Path>frontend/.codex/skills/frontend-crud-coding/references/frontend.md</Path>`。它确认无 `.prettierrc`、用 `pnpm fmt`/`pnpm lint`、EditorConfig 2 空格、新代码默认 `v-hasPermi`、已混用则保持文件现状、dropdown 用 `checkPermi`。几乎不覆盖动态路由与注释。
- Type: code fact
- Source: 上述两个文件
- Confidence: high
...
- Source: `<Path>frontend/src/directive/permission/index.ts</Path>`；`<Path>frontend/src/plugins/auth.ts</Path>`；`<Path>frontend/src/utils/permission.ts</Path>`；`<Path>ruoyi-vue-plus-namewta/ruoyi-common/ruoyi-common-core/src/main/java/org/dromara/common/core/constant/SystemConstants.java</Path>`
...
1. `getInfo` 失败后 `return { path: '/' }` 在 logout 抛错时的导航结果。路径：`<Path>frontend/src/permission.ts</Path>`。
2. `login?redirect=` 与 401 `query.redirect` 是否双重编码。路径：`<Path>frontend/src/permission.ts</Path>`、`<Path>frontend/src/views/login.vue</Path>`、`<Path>frontend/src/utils/request.ts</Path>`。
3. catch-all 404 注册顺序与动态 addRoute 的运行时匹配，未做浏览器验证。路径：`<Path>frontend/src/router/index.ts</Path>`。
...
7. `loadView` 找不到组件时用户看到什么（空白/404）。路径：`<Path>frontend/src/store/modules/permission.ts</Path>` `loadView`。
