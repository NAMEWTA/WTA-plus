...
path = Path('/tmp/wta-t45/run-minio-diagnostic-ui-v4.py')
spec = importlib.util.spec_from_file_location('t45_ui_v4_runner_offline', path)
...
class Offline(unittest.TestCase):
    ...
    def test_delete_sql_is_exact_owned_id_key_and_invalid_policy(self):
        bad_id = 8_305_000_000_000_000_001
        sql = driver.exact_owned_bad_delete_sql(lambda value: "'" + value + "'",
                                                bad_id, 'xabcdef123456')
        self.assertIn(f'WHERE oss_config_id={bad_id}', sql)
        self.assertIn("AND config_key='xabcdef123456'", sql)
        ...
        with self.assertRaisesRegex(RuntimeError, 'delete identity differs'):
            driver.exact_owned_bad_delete_sql(lambda value: value, 1, 'xabcdef123456')
        with self.assertRaisesRegex(RuntimeError, 'delete identity differs'):
            driver.exact_owned_bad_delete_sql(lambda value: value, bad_id, 'unowned')
