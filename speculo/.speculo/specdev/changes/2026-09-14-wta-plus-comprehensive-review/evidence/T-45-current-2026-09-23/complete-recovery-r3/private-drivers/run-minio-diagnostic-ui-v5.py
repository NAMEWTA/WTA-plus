...
RUN_ROOT = Path('/tmp/wta-t45/ui-v5-runs')
...
UI_ROOT = Path('/tmp/wta-t45/ui-v3')
...
UI_HASHES = {
    'spec': 'c36ec034aa472b6213abe5d2f4a40e71b97d04fc03ffc5e107d5bca3982605ad',
...
REVIEWED_PRODUCT_SOURCE_HEAD = 'd98936b6881ad7c8093476a65fe9736203c60af7'
PRODUCT_UI_INPUTS = {
    'frontend/packages/web-domains/system/src/oss-config/OssConfigPage.vue':
        'bfca123e85f6fb232314ba8fa8112695db656437ff48edfd75e3f5342952db53',
    'frontend/apps/admin-web/src/views/login.vue':
        '9200070a189ccfcee0ff33069c9145158a1d7c8d5426b31cc25e998bfdaf8386',
    'frontend/apps/admin-web/src/layout/components/Navbar.vue':
        'b54debb7e3cce2e6cb1f7435424cb6dd855c3c62f1586bd2e0a8deaeb9b046f8',
    'frontend/packages/web-domains/system/src/index.ts':
        'd5a9eb5666436d270ebefb03e6857860b795c6bcc27e693c6eb55f94450a4ee9',
    'frontend/apps/admin-web/src/application/session.ts':
        '703553d951b463e7be94580d8b03ad63fe5e13a146bbaccda3130662cd59a64e',
}
...
PHASES = ('preflight', 'owned_services', 'six_sql', 'owned_minio', 'backend',
          ...
          'negative', 'switch_b', 'private_b', 'openapi', 'browser_fixture',
          'browser', 'postcheck')
...
def safe_oss_config_page(status, response, id_bad, key_bad, id_public, id_private):
    """Inspect only the exact first UI page; return bounded, non-secret facts."""
    if status != 200 or type(response) is not dict or response.get('code') != 200:
        raise RuntimeError('owned Admin config list HTTP contract differs')
    data = response.get('data')
    if type(data) is not dict or type(data.get('rows')) is not list or type(data.get('total')) is not int:
        raise RuntimeError('owned Admin config list shape differs')
    rows, total = data['rows'], data['total']
    if total < 0 or len(rows) > 10 or total != len(rows):
        raise RuntimeError('owned Admin config list first page incomplete')
    seen = set()
    bad_exact = False
    unknown = 0
    for row in rows:
        if type(row) is not dict:
            raise RuntimeError('owned Admin config row shape differs')
        raw_id = row.get('ossConfigId')
        if type(raw_id) is int and raw_id > 0:
            object_id = raw_id
        elif type(raw_id) is str and re.fullmatch(r'[0-9]{1,20}', raw_id):
            object_id = int(raw_id)
        else:
            raise RuntimeError('owned Admin config row identity differs')
        if object_id in seen:
            raise RuntimeError('owned Admin config row identity differs')
        seen.add(object_id)
        policy = row.get('accessPolicy')
        if type(policy) is not str:
            raise RuntimeError('owned Admin config row policy differs')
        if policy not in ('0', '2'):
            unknown += 1
            if object_id != id_bad or policy != '9' or row.get('configKey') != key_bad:
                raise RuntimeError('owned Admin config list has unrelated invalid policy')
        if object_id == id_bad:
            bad_exact = policy == '9' and row.get('configKey') == key_bad
    return ({'status': status, 'code': response['code'], 'total': total,
             'rowcount': len(rows), 'bad_present': id_bad in seen,
             'public_present': id_public in seen, 'private_present': id_private in seen,
             'unknown_policy_count': unknown}, bad_exact)


def require_owned_bad_before(page, bad_exact):
    if (not page['bad_present'] or not bad_exact
        or not page['public_present'] or not page['private_present']
        or page['unknown_policy_count'] != 1):
        raise RuntimeError('owned invalid policy page precondition differs')


def require_owned_bad_after(before, after):
    if (after['bad_present']
        or not after['public_present'] or not after['private_present']
        or after['unknown_policy_count'] != 0
        or after['total'] != before['total'] - 1
        or after['rowcount'] != before['rowcount'] - 1):
        raise RuntimeError('owned invalid policy page cleanup differs')


def exact_owned_bad_delete_sql(compared_hexsql, id_bad, key_bad):
    if (type(id_bad) is not int
        or not 8_305_000_000_000_000_000 <= id_bad < 8_305_000_010_000_000_000
        or type(key_bad) is not str or not re.fullmatch(r'x[0-9a-f]{12}', key_bad)):
        raise RuntimeError('owned invalid policy delete identity differs')
    compared_key = compared_hexsql(key_bad)
    if not compared_key.endswith(' COLLATE utf8mb4_general_ci'):
        raise RuntimeError('owned invalid policy comparison collation differs')
    return (f'DELETE FROM sys_oss_config WHERE oss_config_id={id_bad}'
            + ' AND config_key=' + compared_key
            + " AND access_policy='9'; SELECT ROW_COUNT();")
...
def validate_product_ui_contract():
    """Pin C3's real page words/selectors before any owned service starts."""
    for name, expected in PRODUCT_UI_INPUTS.items():
        if base_sha(ROOT / name) != expected:
            raise RuntimeError('C3 product UI input differs')
    page = (ROOT / 'frontend/packages/web-domains/system/src/oss-config/OssConfigPage.vue').read_text()
    login = (ROOT / 'frontend/apps/admin-web/src/views/login.vue').read_text()
    navbar = (ROOT / 'frontend/apps/admin-web/src/layout/components/Navbar.vue').read_text()
    route = (ROOT / 'frontend/packages/web-domains/system/src/index.ts').read_text()
    session = (ROOT / 'frontend/apps/admin-web/src/application/session.ts').read_text()
    expected = (
        (page, '<h3>OSS 配置</h3>'),
        (page, 'aria-label="只读诊断"'),
        (page, 'title="OSS 访问诊断"'),
        (page, '单对象读取不证明全桶安全'),
        (page, "ALLOWED: '观察到允许', DENIED: '观察到拒绝', UNKNOWN: '未知'"),
        (page, "OBJECT_HEAD: '诊断对象匿名 HEAD', OBJECT_GET: '诊断对象匿名 GET'"),
        (page, "POLICY_READ: '策略对象读取声明', POLICY_WRITE: '策略危险写声明'"),
        (page, "ACL_LIST: '桶 ACL 列举声明', ACL_WRITE_RISK: '桶 ACL 写入或修改声明'"),
        (login, 'class="login-form"'),
        (login, 'class="submit-button"'),
        (navbar, 'class="avatar-wrapper"'),
        (route, "['system-oss-config', 'system/oss/config', 'OssConfig'"),
        (session, "key: 'Admin-Token'"),
    )
    if not all(needle in text for text, needle in expected):
        raise RuntimeError('C3 product UI words or selectors differ')
    return {'reviewed_product_source_head': REVIEWED_PRODUCT_SOURCE_HEAD,
            'bound_product_files': len(PRODUCT_UI_INPUTS)}
...
def safe_browser_failures(data):
    """Retain only fixed case identity/status and a numeric line in this spec."""
    if type(data) is not dict:
        return []
    maximum_line = len(UI_SPEC.read_text().splitlines())
    exact_stack = re.compile(re.escape(str(UI_SPEC.resolve())) + r':([0-9]{1,5}):([0-9]{1,4})')
    failures = []
    def exact_location(value):
        if type(value) is not dict:
            return None
        source = value.get('file')
        if type(source) is not str:
            return None
        source_path = Path(source)
        candidates = ([source_path.resolve()] if source_path.is_absolute() else
                      [(root / source_path).resolve() for root in (UI_ROOT, ROOT / 'frontend')])
        line, column = value.get('line'), value.get('column')
        if (UI_SPEC.resolve() in candidates and type(line) is int and type(column) is int
            and 1 <= line <= maximum_line and 0 <= column <= 1000):
    ...
    def observe(suites):
        ...
        for suite in suites:
            if type(suite) is not dict:
                continue
            for spec in suite.get('specs', []):
                if type(spec) is not dict or spec.get('title') not in UI_TITLES:
                    continue
                source = spec.get('file') or suite.get('file')
                if type(source) is not str:
                    continue
                source_path = Path(source)
                candidates = ([source_path.resolve()] if source_path.is_absolute() else
                ...
                for test in spec.get('tests', []):
                    ...
                    for result in test.get('results', []):
                        if type(result) is not dict or result.get('status') not in ('failed', 'timedOut', 'interrupted'):
                            continue
                        item = {'title': spec['title'], 'status': result['status']}
                        ...
                        for error in errors:
                            if type(error) is not dict:
                                continue
                            location = exact_location(error.get('location'))
                            if location is None and type(error.get('stack')) is str:
                                for match in exact_stack.finditer(error['stack']):
                                    line, column = int(match[1]), int(match[2])
                                    if 1 <= line <= maximum_line and 0 <= column <= 1000:
            ...
            observe(suite.get('suites', []))
    observe(data.get('suites', []))
    return failures
...
def main():
    ...
    if args.preflight:
        ...
        contract = validate_product_ui_contract()
        ...
        print(json.dumps({'mode': 'static-only', 'services_started': False,
                          'product_ui_contract': contract, 'inputs': inputs}))
    ...
    try:
        ...
        report['product_ui_contract'] = validate_product_ui_contract()
        ...
        # The earlier INVALID_ACCESS_POLICY negative has already been proved.
        # Keep that result, then remove only its owned row before the editable
        # page projects every physical policy in its first page.
        if report['diagnostic'].get('invalid_policy_fail_closed') is not True:
            raise RuntimeError('owned invalid policy negative evidence absent')
        list_path = '/resource/oss/config/list?pageNum=1&pageSize=10'
        page_status, page_value = api(backend_port, 'GET', list_path, control_token)
        before_page, bad_exact = safe_oss_config_page(
            page_status, page_value, id_bad, key_bad, id_p, id_a)
        report['browser_fixture'] = {'before': before_page}
        require_owned_bad_before(before_page, bad_exact)
        sql = exact_owned_bad_delete_sql(base.compared_hexsql, id_bad, key_bad)
        if count_sql(base, mysql_id, sql, 'browser_bad_policy_delete').splitlines()[-1] != '1':
            raise RuntimeError('owned invalid policy precise delete differs')
        report['browser_fixture']['deleted_owned_invalid_row_count'] = 1
        page_status, page_value = api(backend_port, 'GET', list_path, control_token)
        after_page, _ = safe_oss_config_page(
            page_status, page_value, id_bad, key_bad, id_p, id_a)
        report['browser_fixture']['after'] = after_page
        require_owned_bad_after(before_page, after_page)
        phase = 'browser'
        ...
        if len(raw_chrome) > 8_000_000:
            raise RuntimeError('Chrome raw reporter exceeds bound')
        try:
            chrome_data = json.loads(raw_chrome)
        except (ValueError, UnicodeError):
            raise RuntimeError('Chrome reporter shape differs') from None
        report['browser_failure_locations'] = safe_browser_failures(chrome_data)
        ...
        if type(exc) is RuntimeError and str(exc) in (
            ...
            'Chrome raw reporter exceeds bound', 'C3 product UI input differs',
            'C3 product UI words or selectors differ',
            'owned Admin config list HTTP contract differs',
            'owned Admin config list shape differs',
            'owned Admin config list first page incomplete',
            'owned Admin config row shape differs',
            'owned Admin config row identity differs',
            'owned Admin config row policy differs',
            'owned Admin config list has unrelated invalid policy',
            'owned invalid policy page precondition differs',
            'owned invalid policy page cleanup differs',
            'owned invalid policy delete identity differs',
            'owned invalid policy comparison collation differs',
            'owned invalid policy negative evidence absent',
            'owned invalid policy precise delete differs'):
            report['failure']['fixed_reason'] = str(exc)
        ...
        try:
            ...
            if (base_sha(UI_SPEC) != UI_HASHES['spec']
                ...
                or validate_product_ui_contract() != report.get('product_ui_contract')):
