#!/usr/bin/env python3
"""Serial owned MySQL/Redis -> Profile transfer and actual Notify queue/worker."""
import datetime
import os
import signal
import json
...
name = sys.argv[1]
assert name.startswith('T-25-') and '/' not in name
...
database = 'namewta_dept_test_' + suffix
password = 'owned-dept-test-only'
code = 1
process = None
...
start = time.time()
try:
    mysql = run(['docker', 'run', '-d', '--name', 'namewta-t25-mysql-' + suffix, '--label', 'namewta.test.owner=T-25', '-p', '127.0.0.1::3306',
    ...
    snapshot = evidence / (name + '-baseline-departments.json')
    query = "select json_object('dept_id',cast(dept_id as char),'parent_id',cast(parent_id as char),'ancestors',ancestors,'del_flag',del_flag) from sys_dept order by dept_id"
    exported = subprocess.run(['docker', 'exec', mysql, 'mysql', '-uroot', '-p' + password, '--batch', '--raw', '--skip-column-names', database, '-e', query], check=True, capture_output=True, text=True)
    snapshot.write_text(json.dumps([json.loads(line) for line in exported.stdout.splitlines() if line.strip()], ensure_ascii=False, indent=2) + '\n')
    audit_command = [sys.executable, str(evidence / 'audit-t25-tree.py'), '--input', str(snapshot), '--output', str(evidence / (name + '-tree-audit.json'))]
    audited = subprocess.run(audit_command, capture_output=True, text=True)
    record['tree_audit'] = dict(command=audit_command, exit_code=audited.returncode, result=audited.stdout.strip())
    if audited.returncode: raise RuntimeError(audited.stderr)
    command = ['./mvnw', '-B', '-ntp', '-pl', 'wta-admin', '-am', '-Dtest=DepartmentTreeMySqlIntegrationTest',
               '-Dsurefire.failIfNoSpecifiedTests=false', '-Dprofiles.active=dev,local', '-Ddepartment.mysql.integration=true',
               '-Ddepartment.mysql.integration.url=jdbc:mysql://127.0.0.1:' + mysql_port + '/' + database + '?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=Asia/Shanghai',
               '-Dnamewta.repo.root=' + str(root), 'test']
    record.update(command=command, cwd='backend', log=name + '.log')
    print('Running actual department/MySQL structure invariants', flush=True)
    with (evidence/record['log']).open('w') as log:
        ...
        for line in process.stdout:
            ...
            if any(marker in line for marker in ['[ERROR]', 'Tests run:', 'BUILD SUCCESS', 'BUILD FAILURE', 'T-25:', ' passed (', ' failed']): print(line, end='', flush=True)
        code = process.wait()
