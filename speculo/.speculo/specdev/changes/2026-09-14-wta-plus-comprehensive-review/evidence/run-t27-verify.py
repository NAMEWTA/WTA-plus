#!/usr/bin/env python3
"""Run T-27 repository checks serially and retain real outcomes."""
import json
import os
...
shim = Path(tempfile.mkdtemp(prefix='t27-pnpm-', dir=root/'temp/team/lead'))
...
if mode == 'backend':
    commands = [('backend', ['./mvnw', '-B', '-ntp', '-pl', 'wta-modules/wta-demo,wta-admin', '-am', 'test']), ('backend', ['./mvnw', '-B', '-ntp', 'test'])]
elif mode == 'frontend-focused':
    commands = [('frontend', ['corepack', 'pnpm', '--workspace-concurrency=1', '--filter', '@namewta/domain-demo', 'test'])]
elif mode == 'frontend':
    commands = [('frontend', ['corepack', 'pnpm', task]) for task in ['lint', 'typecheck', 'test', 'build:prod']]
elif mode == 'module':
    commands = [('backend', ['./mvnw', '-B', '-ntp', '-pl', 'wta-modules/wta-system', '-am', '-Dprofiles.active=dev,local', 'test'])]
elif mode == 'bundles':
    commands = [('backend', ['./mvnw', '-B', '-ntp', 'clean', 'package', '-DskipTests']), ('.', ['bash', 'scripts/ci/verify-admin-bundle.sh', 'full']),
                ('backend', ['./mvnw', '-B', '-ntp', 'clean', 'package', '-Pbundle-core', '-Dmaven.test.skip=true']), ('.', ['bash', 'scripts/ci/verify-admin-bundle.sh', 'core'])]
elif mode == 'static':
    commands = [('.', ['node', '.agents/skills/namewta-fullstack-development/scripts/validate-module-mode.mjs', 'backend/wta-modules/wta-demo', '--mode', 'classic']),
                ...
                ('.', ['node', '--test', '--test-concurrency=1', 'release-artifacts/tests/mysql-sql-contract.test.mjs', 'release-artifacts/tests/mysql-baseline-contract.test.mjs']),
                ('.', ['git', 'diff', '--check'])]
