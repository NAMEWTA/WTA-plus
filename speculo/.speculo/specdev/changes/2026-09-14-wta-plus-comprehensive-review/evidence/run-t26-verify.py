#!/usr/bin/env python3
"""Run T-26 repository checks serially and retain real outcomes."""
import json
import os
...
shim = Path(tempfile.mkdtemp(prefix='t26-pnpm-', dir=root/'temp/team/lead'))
...
if mode == 'backend':
    commands = [('backend', ['./mvnw', '-B', '-ntp', 'test'])]
elif mode == 'frontend-focused':
    commands = [('frontend', ['corepack', 'pnpm', '--workspace-concurrency=1', '--filter', '@namewta/domain-system', '--filter', '@namewta/domain-workflow', '--filter', '@namewta/domain-demo', '--filter', '@namewta/domain-admin', 'test'])]
elif mode == 'frontend':
    ...
    commands = [('backend', ['./mvnw', '-B', '-ntp', 'clean', 'package', '-DskipTests']),
                ('backend', ['./mvnw', '-B', '-ntp', 'clean', 'package', '-Pbundle-core', '-Dmaven.test.skip=true'])]
elif mode == 'static':
    commands = [('.', ['node', '.agents/skills/namewta-fullstack-development/scripts/validate-module-mode.mjs', 'backend/wta-modules/wta-system', '--mode', 'classic']),
                ...
                ('.', ['node', '.agents/skills/namewta-fullstack-development/scripts/validate-module-mode.mjs', 'backend/wta-modules/wta-demo', '--mode', 'classic']),
