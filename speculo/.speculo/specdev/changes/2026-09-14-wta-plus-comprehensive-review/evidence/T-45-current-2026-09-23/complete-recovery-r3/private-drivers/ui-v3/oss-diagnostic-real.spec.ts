...
async function diagnoseRow(page: Page, key: string, id: string, expectedObject: '观察到允许' | '观察到拒绝') {
...
test('T-45 real Admin sees scoped public and private diagnostic facts', async ({ page }) => {
  ...
  await diagnoseRow(page, owned.public_key, owned.public_id, '观察到允许');
  await diagnoseRow(page, owned.private_key, owned.private_id, '观察到拒绝');
});
