#!/usr/bin/env python3
"""Run T-24 repository checks serially and retain real outcomes."""
import json
import os
...
shim = Path(tempfile.mkdtemp(prefix='t24-pnpm-', dir=root/'temp/team/lead'))
...
if mode == 'backend':
    commands = [('backend', ['./mvnw', '-B', '-ntp', '-pl', 'wta-modules/wta-third,wta-admin', '-am', 'test'])]
elif mode == 'module':
    commands = [('backend', ['./mvnw', '-B', '-ntp', '-pl', 'wta-modules/wta-third', '-am', '-Dprofiles.active=dev,local', 'test'])]
elif mode == 'static':
    commands = [('.', ['node', '.agents/skills/namewta-fullstack-development/scripts/validate-module-mode.mjs', 'backend/wta-modules/wta-third', '--mode', 'layered']),
