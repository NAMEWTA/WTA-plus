#!/usr/bin/env python3
"""Opt-in, owned T-39 deadline and T-36/T-37/T-38 regression acceptance runner."""
...
RUN_ROOT = Path('/tmp/wta-t39/runs')
...
REDIS_CLASS = 'org.namewta.common.redis.utils.RedisUtilsDeadlineIntegrationTest'
REDIS_SOURCE = ROOT / 'backend/wta-common/wta-common-redis/src/test/java/org/namewta/common/redis/utils/RedisUtilsDeadlineIntegrationTest.java'
REDIS_REPORT = ROOT / ('backend/wta-common/wta-common-redis/target/surefire-reports/TEST-' + REDIS_CLASS + '.xml')
...
SUITES = ((TEST_CLASS, TEST_REPORT), (PROFILE_CLASS, PROFILE_REPORT),
          (WAKE_CLASS, WAKE_REPORT), (REDIS_CLASS, REDIS_REPORT),
...
def owned_ids(run_id):
    found = docker('ps', '-aq', '--no-trunc', '--filter', 'label=namewta.test.owner=T-39',
...
def assert_owned(cid, run_id):
    actual = labels(cid)
    if actual.get('namewta.test.owner') != 'T-39' or actual.get('namewta.test.run') != run_id:
...
def mysql_run_args(run_id, mysql_env):
    return ('run', '--pull=never', '-d', '--name', 't39-mysql-' + run_id,
            '--label', 'namewta.test.owner=T-39',
...
def validate_common_redis_source(redis_text):
    if 'class RedisUtilsDeadlineIntegrationTest' not in redis_text:
        raise RuntimeError('Common Redis regression source does not define the exact selected class')
    if not re.search(r'@EnabledIfSystemProperty\s*\(\s*named\s*=\s*"notify\.deadline\.redis\.integration"\s*,\s*matches\s*=\s*"true"', redis_text):
        raise RuntimeError('Common Redis regression must retain its explicit opt-in property')
    if 'notify.redis.integration.port' not in redis_text or 'redis://127.0.0.1:' not in redis_text:
        raise RuntimeError('Common Redis regression must use the owned loopback port')
    context_type = re.search(r'private\s+(GenericApplicationContext|AnnotationConfigApplicationContext)\s+context\s*;', redis_text)
    if not context_type or not re.search(r'context\s*=\s*new\s+' + context_type.group(1) + r'\s*\(', redis_text):
        raise RuntimeError('Common Redis regression must create one supported class-level context')
    if not re.search(r'@TestInstance\s*\(\s*TestInstance\.Lifecycle\.PER_CLASS\s*\)', redis_text) \
            or not re.search(r'@BeforeAll\s+void\s+\w+\s*\(', redis_text) \
            or not re.search(r'@AfterAll\s+void\s+\w+\s*\(', redis_text) \
            or not re.search(r'private\s+RedissonClient\s+redis\s*;', redis_text) \
            or 'context.close()' not in redis_text or 'redis.shutdown()' not in redis_text:
        raise RuntimeError('Common Redis regression must retain class-level client/context lifecycle')
...
def main():
    ...
    report = {'gate': 'T-39 real deadline and T-36/T-37/T-38 eight-class regression', 'run_id': run_id,
              'mode': 'acceptance',
              'acceptance': False,
    ...
    try:
        ...
        validate_common_redis_source(REDIS_SOURCE.read_text())
        ...
        write_secret(test_env, ''.join(name + '=' + app_password + '\n' for name in
                     ('T39_MYSQL_PASSWORD', 'T36_MYSQL_PASSWORD', 'T38_MYSQL_PASSWORD', 'T35_MYSQL_PASSWORD')))
        ...
        redis_id = full_id(docker('run', '--pull=never', '-d', '--name', 't39-redis-' + run_id,
                                  '--label', 'namewta.test.owner=T-39',
                                  '--label', 'namewta.test.run=' + run_id,
        ...
        selectors = ('NotifyDeadlineIntegrationTest,EnterpriseQueuedNotificationIntegrationTest,'
                     'NotifyWakeIntegrationTest,RedisUtilsDeadlineIntegrationTest,'
        ...
        argv = ('./mvnw', '-Pdev', '-pl', 'wta-admin', '-am', 'test',
                ...
                '-Dnotify.wake.integration=true', '-Dnotify.deadline.redis.integration=true',
        ...
        with os.fdopen(os.open(raw_log, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600), 'wb') as stream:
            maven_proc = subprocess.Popen(argv, cwd=ROOT / 'backend', stdout=stream,
                                          ...
                                          env=safe_env(T39_MYSQL_PASSWORD=app_password, T36_MYSQL_PASSWORD=app_password,
