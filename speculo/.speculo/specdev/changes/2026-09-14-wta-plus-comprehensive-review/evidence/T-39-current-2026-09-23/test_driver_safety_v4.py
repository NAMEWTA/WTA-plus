...
DRIVER = Path('/tmp/wta-t39/run-notify-deadline-integration-v4.py')
spec = importlib.util.spec_from_file_location('t39_driver_v4', DRIVER)
...
class DriverSafetyTest(unittest.TestCase):
    ...
    def test_source_and_eight_class_selector(self):
        ...
        self.assertLess(source.index('validate_common_redis_source(REDIS_SOURCE.read_text())'),
    ...
    def test_common_preflight_accepts_both_supported_contexts_and_rejects_missing_lifecycle(self):
        actual = driver.REDIS_SOURCE.read_text()
        driver.validate_common_redis_source(actual)
        generic = actual.replace('AnnotationConfigApplicationContext', 'GenericApplicationContext')
        driver.validate_common_redis_source(generic)
        for old, new in (('TestInstance.Lifecycle.PER_CLASS', 'TestInstance.Lifecycle.PER_METHOD'),
                         ('@BeforeAll', '@BeforeEach'), ('@AfterAll', '@AfterEach'),
                         ('redis://127.0.0.1:', 'redis://remote:'), ('context.close()', 'context.clear()'),
                         ('redis.shutdown()', 'redis.pause()')):
            with self.subTest(old=old), self.assertRaises(RuntimeError):
                driver.validate_common_redis_source(actual.replace(old, new))
