...
2. required Ticket 有 source commit、passed candidate、父分支 result SHA，且父分支包含 source commit；current Ticket 有非空 implementation commit、Lead 在精确 HEAD/tree 且全仓 clean 时点捕获的 passed direct-parent 证据、完整不可变 result SHA，当前父分支包含该 result；各票实现区间串行，对应 workspace 记录均为完成状态。
...
8. 完成治理记录提交后，最终 `change_status=completed` 的仓库 tracked 与未忽略 untracked 均 clean；读取 Git 状态失败不能视为 clean。项目发布的 clean-source 门禁独立保留，不为保存 Evidence 排除源文件或用户改动。
...
写入完成记录后尚未提交或最终校验失败时，不宣称 change 已完成。治理提交引用各票既有产品 result，不改写 result 来追逐最新 HEAD，也不要求状态记录自身承载提交的 SHA。提交后以 `--repo` 重新运行 complete 校验及适用最终门禁，确认记录与全仓 clean 后才交付完成声明。
