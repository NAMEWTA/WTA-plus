...
name = sys.argv[1]
assert name.startswith('T-22-') and '/' not in name
...
start = time.time()
try:
    mysql = run(['docker', 'run', '-d', '--name', 'namewta-t22-mysql-' + suffix, '--label', 'namewta.test.owner=T-22', '-p', '127.0.0.1::3306',
    ...
    redis = run(['docker', 'run', '-d', '--name', 'namewta-t22-redis-' + suffix, '--label', 'namewta.test.owner=T-22', '-p', '127.0.0.1::6379', images['redis'], 'redis-server', '--save', '', '--appendonly', 'no'])
    ...
    command = ['./mvnw', '-B', '-ntp', '-pl', 'wta-admin', '-am', '-Dtest=NotifyAtomicResultIntegrationTest',
    ...
    with (evidence/record['log']).open('w') as log:
        ...
        for line in process.stdout:
            ...
            if any(marker in line for marker in ['[ERROR]', 'Tests run:', 'BUILD SUCCESS', 'BUILD FAILURE', 'T-22:', ' passed (', ' failed']): print(line, end='', flush=True)
        code = process.wait()
