...
const pendingKey = 'namewta-admin-sso-pending';
...
export const adminSso = createSsoAuth({
  exchangeToken: async input => {
    const response = await adminHttp.request<{ code?: number; data?: { access_token?: string; client_id?: string } }>({
...
export function adminSsoRedirectUri(): string {
