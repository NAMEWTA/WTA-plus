...
def acl_entries(value):
    ...
    if not isinstance(value, list):
        raise RuntimeError('Owned ACL log shape differs')
    safe = []
    for entry in value:
        if not isinstance(entry, list) or len(entry) % 2:
            raise RuntimeError('Owned ACL log entry differs')
        fields = dict(zip(entry[::2], entry[1::2]))
        if (fields.get('username') == 'default' and fields.get('reason') == 'command'
                and str(fields.get('object', '')).upper() == 'PUBLISH'):
            count = fields.get('count')
            if type(count) is not int or count < 1 or count > 1_000_000:
                raise RuntimeError('Owned ACL denial count differs')
            safe.append({'reason': 'command', 'object': 'PUBLISH',
...
def redis_fallback(cid, redis_port, app_password, admin_password, backend_port,
    ...
    # Restrict only the short owned publish window so OSS/cache init is not damaged.
    if admin(('ACL', 'SETUSER', 'default', '-publish')) != 'OK':
    ...
    if denied == 'OK' or allowed != 'OK' or app(('PING',)) != 'PONG':
        raise RuntimeError('Owned Redis ACL did not isolate only PUBLISH')
    if app(('GET', key)) != '1':
    ...
    acl = acl_entries(admin(('ACL', 'LOG', '20')))
    if not acl:
        raise RuntimeError('Owned Redis PUBLISH ACL rejection was not observed')
...
def execute(args):
    ...
    try:
        ...
        report['error'] = {'phase': phase, 'type': type(error).__name__ if
            type(error).__name__ in ('RuntimeError', 'OwnedSqlError', 'TimeoutExpired',
                                     'OSError', 'ValueError', 'JSONDecodeError') else 'Exception'}
