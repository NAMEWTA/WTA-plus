...
risk: "high"
blocked_by: ["T-02"]
contract_ids: ["N-05", "N-06", "N-07", "N-09"]
owner: "audit-lead"
expected_changes: ["<Path>backend/wta-api/**</Path>", "<Path>backend/wta-modules/wta-notify/**</Path>", "<Path>backend/wta-modules/wta-demo/**</Path>", "<Path>backend/wta-modules/wta-workflow/**</Path>", "<Path>backend/wta-modules/wta-profile/**</Path>", "<Path>backend/wta-admin/**</Path>", "<Path>backend/wta-common/wta-common-notify/**</Path>", "<Path>frontend/packages/domains/notify/**</Path>", "<Path>frontend/packages/web-domains/notify/**</Path>", "<Path>frontend/apps/admin-web/**</Path>", "<Path>frontend/e2e/**</Path>", "<Path>release-artifacts/docker/infrastructure/mysql/init/10-cde-base-ddl.sql</Path>", "<Path>docs/**</Path>"]
...
# Ticket T-03：通知重试截止撤回与附件合同

总体 Map：<Path>{roots.state}/specdev/changes/2026-09-22-audit-foundation-hardening/tickets-map.md</Path>。上游 Spec：<Path>{roots.state}/specdev/changes/2026-09-22-audit-foundation-hardening/spec.md</Path>。完成 Evidence：<Path>{roots.state}/specdev/changes/2026-09-22-audit-foundation-hardening/evidence/T-03.md</Path>。先读 Map、适用项目 Skill，再读本票。

## 1. 战略与来源

来源 USER-DECISION:2026-09-22 与 <Path>docs/reviews/WTA-plus-review-64b4ea7.md</Path> 的 N-05, N-06, N-07, N-09。目标：定向重试不影响其他投递，路径主键权威；过期不调用 Provider；撤回停止未发，已发保留；邮件无业务路径可发送，附件明确授权映射。
...
定向重试不影响其他投递，路径主键权威；过期不调用 Provider；撤回停止未发，已发保留；邮件无业务路径可发送，附件明确授权映射。 错误必须可恢复且不显示伪成功，不放大无关配置故障。
...
|---|---|---|---|
| 正常/失败/边界 | Controller/应用服务及实际渠道合同，截止前后与取消竞态，零/一/多附件，公告详情和撤回浏览器场景。 | 真实断言通过；无供应商真实发送 | <Path>{roots.state}/specdev/changes/2026-09-22-audit-foundation-hardening/evidence/T-03.md</Path> |
| 回归 | 原质量门禁中的受影响模块检查 | 不关闭规则、不丢弃负向测试 | <Path>{roots.state}/specdev/changes/2026-09-22-audit-foundation-hardening/evidence/T-03.md</Path> |
...
- [ ] N-05, N-06, N-07, N-09 的可观察结果和负向断言通过。
