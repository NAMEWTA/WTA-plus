#!/usr/bin/env python3
"""Opt-in, owned T-39 deadline, Profile, Wake and Redis MySQL/Redis acceptance runner."""
...
def main():
    ...
    report = {'gate': 'T-39 real deadline, Enterprise queued notification, Wake and Redis', 'run_id': run_id,
              'mode': 'acceptance',
              'acceptance': False,
    ...
    try:
        ...
        atomic_text = (ROOT / 'backend/wta-admin/src/test/java/org/namewta/test/notify/NotifyAtomicResultIntegrationTest.java').read_text()
        ...
        redis_text = REDIS_SOURCE.read_text()
        ...
        if 'notify.redis.integration.port' not in redis_text or 'GenericApplicationContext' not in redis_text:
            raise RuntimeError('Common Redis regression must use owned Redis and one class-level context')
        ...
        write_secret(test_env, 'T39_MYSQL_PASSWORD=' + app_password + '\nT36_MYSQL_PASSWORD=' + app_password + '\n')
        ...
        for path in (TEST_REPORT, PROFILE_REPORT, WAKE_REPORT, REDIS_REPORT):
        ...
        report['methods'] = {}
        xml_errors = {}
        for test_class, path in ((TEST_CLASS, TEST_REPORT), (PROFILE_CLASS, PROFILE_REPORT),
