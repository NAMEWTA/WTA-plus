#!/usr/bin/env python3
"""Serial local T-16 checks. No release promotion or external writes."""
import json
import os
...
name = sys.argv[1]
assert name.startswith('T-16-') and '/' not in name
mode = sys.argv[2]
shim = Path(tempfile.mkdtemp(prefix='t16-pnpm-', dir=root / 'temp/team/lead'))
...
if mode == 'workflow-focused':
    commands = [('frontend', ['corepack', 'pnpm', '--filter', '@namewta/web-domain-workflow', check]) for check in ['typecheck', 'test', 'lint']]
elif mode == 'workflow-unit':
    commands = [('frontend', ['corepack', 'pnpm', '--filter', '@namewta/web-domain-workflow', 'test'])]
    ...
    commands = [('frontend', ['corepack', 'pnpm', 'exec', 'tsc', '--ignoreConfig', '--noEmit', '--strict', '--skipLibCheck', '--target', 'ES2023', '--module', 'ESNext', '--moduleResolution', 'Bundler', '--types', 'node', 'e2e/workflow-task-integrity.spec.ts', 'playwright.config.ts']),
                ('frontend', ['corepack', 'pnpm', 'exec', 'oxlint', 'e2e/workflow-task-integrity.spec.ts', 'playwright.config.ts']),
