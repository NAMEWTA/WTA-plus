...
## LOG-023 — 2026-09-12T12:25:00+08:00 — I-implement landed on local WTA-plus prep tree
- **状态:** completed-local
- **写面:** `/workspace/vp-dev/WTA-plus`（docs 聚合镜像；无 nested `.git`；无 `.gitee`；无 public remote）
- **完成:** T-00…T-13、T-16、T-17（owned `org.namewta` / `wta-*`；KEEP 四件套；Nacos 硬切无双读；DB migrate 脚本；publication checklist）
- **验证:** `./mvnw -pl wta-admin -am -DskipTests package` ×2 BUILD SUCCESS；`pnpm architecture:check` 0；`wta-rename-keep-contract.test.mjs` 3/3
- **未做:** T-14 public push（`execution_authorization.public_repo_publication=not-authorized`）；T-15 旧仓 mutation（`legacy_repo_mutation_authorized=false`）；旧三仓 force-push/rewrite
- **约束:** KEEP 字节级保留；compose 服务名 `namewta-*`；`@namewta` npm 范围不变



## LOG-024 — 2026-09-12T12:39:02+08:00 — CTO 授权公开建仓/push + 旧仓 freeze（先 freeze）
- **状态：** authorized
- **来源：** USER-DECISION [t23u] via 分管-开发
- **结论：**
  1. `execution_authorization.public_repo_publication=authorized` → 允许 create/push orphan `NAMEWTA/WTA-plus`
  2. `execution_authorization.legacy_repo_mutation=authorized` → 允许旧三仓 freeze transition（README 指向新仓 / archive / protection）
  3. **执行顺序（CTO 当面）：先 T-15 冻结旧仓，再 T-14 公开建仓 push**（覆盖 Map 上 T-15 blocked_by T-14 的默认序；freeze README 可先写继任 URL `https://github.com/NAMEWTA/WTA-plus`）
- **范围·旧仓：** `NAMEWTA/ruoyi-vue-plus-docs`、`NAMEWTA/ruoyi-vue-plus-namewta`、`NAMEWTA/plus-ui-namewta`
- **禁止：** force-push / history rewrite / filter-repo；旧仓继续功能开发
- **约束：** 唯一执行面 Grok Bot 电脑；不 push 到旧仓产品分支作 rename；公开 push 前仍过 T-12 checklist
- **后续：** Lead 派 RVP·实现：T-15 → T-14 → Evidence/LOG → 回 Lead/分管-开发

## LOG-025 — 2026-09-12T12:45:00+08:00 — T-15 freeze then T-14 orphan push (t23u)
- **状态:** done
- **顺序:** T-15 first, T-14 second; **no force-push**
- **T-15:** README successor banner + `allow_force_pushes=false` + `gh repo archive` on
  - https://github.com/NAMEWTA/ruoyi-vue-plus-docs HEAD `537d1ca835b81e58ddbd58d289ae47e54d4f0743` archived=true
  - https://github.com/NAMEWTA/ruoyi-vue-plus-namewta HEAD `b994f621ff23b4d8f25000c4db201d70aa507ae7` archived=true
  - https://github.com/NAMEWTA/plus-ui-namewta HEAD `9a6bb2d37d36aedd2533bcc032c375759ec2d403` archived=true
- **T-12 re-verify:** `.gitee=0`; KEEP present; contract 6/6; orphan root `8680fe6…`
- **T-14:** `gh repo create NAMEWTA/WTA-plus --public` then `git push -u origin main` (not `--force`)
  - https://github.com/NAMEWTA/WTA-plus HEAD `01f42cf61bd7ce12c0ece8b4f3fa3ed293e5b1cf`
  - clone smoke same SHA; legacy SHAs not ancestors
- **未做:** NAMEWTA/wta; force-push; filter-repo; CTO machine



## LOG-026 — 2026-09-12T13:16:11+08:00 — CTO 确认 monorepo 根目录英文名
- **状态：** confirmed
- **来源：** USER-DECISION widget t123s1
- **结论：**
  1. 后端目录 `wta-vue-plus-namewta` → **`backend`**
  2. 前端目录 `plus-ui-namewta` → **`frontend`**
  3. 同步更新引用并 push `NAMEWTA/WTA-plus`
- **约束：** 唯一执行面 box；不碰旧三仓（已 archive）；不 force-push 以外方式改历史
- **后续：** Lead 派 RVP·实现执行目录 mv + 引用修复 + push；码审纳入本增量
- **执行（box）：** `git mv` → `backend` / `frontend`；引用全量改为新路径；`rg` 零命中；`git commit` `1e4accd743b7f44c25d50614695ffbcd9ad7a5ea`；`git push origin main` ff `01f42cf..1e4accd`（无 `--force`）。旧三仓未碰。Evidence: `evidence/T-18-root-dirs.md`

## LOG-027 — 2026-09-12 — 删除 upstream-fork-sync + 上游叙述收敛（t126u）
- **状态:** done
- **B:** 删除 `.agents/skills/upstream-fork-sync/` 与 `speculo/skills/upstream-fork-sync/`；AGENTS 路由与 `managed.json` 去索引
- **C:** 仅根 README 一句「基于 RuoYi-Vue-Plus」；删除 `docs/upstream/`（及 backend/frontend 下同名树）；不堆 SUPERSEDED；KEEP 第三方坐标未改
- **A:** 根目录已是 `backend/` `frontend/`（LOG-026 / `1e4accd`），本波未再 mv
- **验证:** `wta-rename-keep-contract.test.mjs` 7/7；旧根路径仅负向断言
- **Push:** `cdf4d10efc76f53a020d64972488e9afde273855` on `origin/main` (`1e4accd..cdf4d10`，无 force)

## LOG-027 — 2026-09-12T13:17:09+08:00 — 删除 upstream-fork-sync；上游叙述仅留根 README
- **状态：** confirmed
- **来源：** USER [t126u]
- **结论：**
  1. 删除 Skill `upstream-fork-sync`（`.agents/skills` 与 `speculo/skills`）及 `docs/upstream/` 工作区文档（非根 README）
  2. 全仓文档/配置说明中的「上游 fork / RuoYi 溯源」**仅允许**出现在根 `README.md` 一句历史说明；其余文档以最新 WTA-plus / `org.namewta` / `backend`+`frontend` 为准，**不保留**过时双轨叙述
  3. 与 LOG-026 一并：根目录已/将改为 `backend`/`frontend`
- **后续：** 派 RVP·实现清扫 + push；码审纳入

## LOG-028 — CR-001 fix — 清除 skills 死链 docs/upstream
- **状态:** done
- **写面:** WTA-plus `.agents/skills` 仅；未恢复 `docs/upstream/`
- **改写:** 认证/权限/Client/菜单指向 AGENTS + engineering-standards + namewta-fullstack permission-routing/contract-mapping/architecture；删除 DELIVERY-002 上游同步/submodule 行
- **验收:** `rg 'docs/upstream' .agents/skills` = 0；contract 扫描 skills 文本
- **Push:** `138fedc4b9ccaca9a7e9d74e6099561572e6324d` (`cdf4d10..138fedc`，无 force)
