...
- 来源清理先持久化 `FAILED/COMPLETED/CLEANUP_OUTCOME_UNKNOWN` 执行权，再在事务外发起一次有界 DELETE。超时或提交回执不确定时，恢复、回滚、重试和新工单均不得绕过该栅栏；后续仅通过有界 HEAD 确认来源不存在且目标仍存在，才可将原工单完成，绝不重发 DELETE。
