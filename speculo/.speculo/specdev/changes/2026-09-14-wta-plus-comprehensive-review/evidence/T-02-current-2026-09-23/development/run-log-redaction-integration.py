...
def ticket_acceptance(exit_code, required_canary_method):
    return exit_code == 0 and bool(required_canary_method)
...
def main():
    ...
    if args.required_canary_method and (not re.fullmatch(r'[A-Za-z][A-Za-z0-9_]*', args.required_canary_method)
                                        or args.required_canary_method == 'credentialsNeverReachHttpEventsOperationEventsOrMySql'):
        parser.error('required canary method must name a distinct new token-path test')
    ...
    try:
        ...
        if 'log.mysql.integration.password' not in test_source and 'T02_MYSQL_PASSWORD' not in test_source:
            raise RuntimeError('HTTP/MySQL test fixture private password intake changed')
        ...
        result['error'] = (type(exc).__name__ + ': ' + str(exc)).replace(root_password, '[REDACTED]')
    finally:
        ...
        try:
            sanitize_log(raw_log, clean_log, (root_password,))
        except Exception:
