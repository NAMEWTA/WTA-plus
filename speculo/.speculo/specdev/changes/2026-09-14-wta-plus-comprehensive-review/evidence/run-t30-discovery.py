...
for index, config in enumerate(configs, 1):
    ...
    log = evidence/f'T-30-discovery-{index}.json'; start = time.time()
    print('DISCOVER', config, flush=True)
    result = subprocess.run(cmd, cwd=root/'frontend', env={**os.environ, 'npm_config_workspace_concurrency':'1'}, text=True, capture_output=True)
    log.write_text(result.stdout)
    (evidence/f'T-30-discovery-{index}.stderr.log').write_text(result.stderr)
    ...
    rows.append(row)
    (evidence/'T-30-browser-discovery.json').write_text(json.dumps(rows, ensure_ascii=False, indent=2)+'\n')
