...
BE-CRUD-003先类型化MPJ；现有BaseMapperPlus加MPJBaseMapper，fresh wrappers；内部NotifyInboxRow在domain/model/read，Service转换公开VO，DAO独占Mapper/Page类型。XML只有真实MPJ不足证据才能采用。无新DDL。本人seen/read/readAll权限/幂等不变，readAll全用户并补不含正文的安全@Log。
...
FILES-002/005要求Mapper读投影在domain/model/read，revision173预审建议的domain/dto不符合硬约束。实施前将精确写集更正为`backend/wta-modules/wta-notify/src/main/java/org/namewta/notify/domain/model/read/NotifyInboxRow.java`；不保留dto目录例外，23条范围数量不变，其他合同/阶段1不变。旧只读设计作为历史输入保留，不覆盖本裁决。

## revision176 完整候选首次验证失败与测试修复范围
