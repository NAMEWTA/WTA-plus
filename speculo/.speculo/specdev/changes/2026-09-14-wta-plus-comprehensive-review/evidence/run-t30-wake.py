...
start = time.time()
try:
    mysql = run(['docker', 'run', '-d', '--name', 'namewta-t30-28-mysql-' + suffix, '--label', 'namewta.test.owner=T-30', '-p', '127.0.0.1::3306',
    ...
    redis = run(['docker', 'run', '-d', '--name', 'namewta-t30-28-redis-' + suffix, '--label', 'namewta.test.owner=T-30', '-p', '127.0.0.1::6379', images['redis'], 'redis-server', '--save', '', '--appendonly', 'no'])
    ...
    command = ['./mvnw', '-B', '-ntp', '-pl', 'wta-admin', '-am', '-Dtest=NotifyWakeIntegrationTest,NotifyOutboxWakeCrossProcessIT',
