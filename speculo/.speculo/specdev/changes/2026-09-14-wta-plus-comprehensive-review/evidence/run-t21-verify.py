#!/usr/bin/env python3
"""Serial local T-21 checks. No release promotion or external writes."""
import json
import os
...
name = sys.argv[1]
assert name.startswith('T-21-') and '/' not in name
mode = sys.argv[2]
shim = Path(tempfile.mkdtemp(prefix='t21-pnpm-', dir=root / 'temp/team/lead'))
...
if mode == 'frontend':
    ...
    commands = [('frontend', ['corepack', 'pnpm', 'test'])]
elif mode == 'build':
    ...
    commands = [('frontend', ['corepack', 'pnpm', 'exec', 'tsc', '--ignoreConfig', '--noEmit', '--strict', '--skipLibCheck', '--target', 'ES2023', '--module', 'ESNext', '--moduleResolution', 'Bundler', '--types', 'node', 'e2e/public-accessibility.spec.ts', 'e2e/system-identity.spec.ts', 'playwright.accessibility.config.ts', 'playwright.config.ts']),
                ('frontend', ['corepack', 'pnpm', 'exec', 'oxlint', 'e2e/public-accessibility.spec.ts', 'e2e/system-identity.spec.ts', 'playwright.accessibility.config.ts', 'playwright.config.ts']),
