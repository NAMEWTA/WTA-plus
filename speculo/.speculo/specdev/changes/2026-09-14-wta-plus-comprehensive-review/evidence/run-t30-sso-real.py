"""Serial T-21 regression verification using owned localhost MySQL/Redis and real Chrome."""
import argparse
import signal
import datetime
import hashlib
...
try:
    mysql_id = run(["docker", "run", "-d", "--name", mysql, "--label", "namewta.test.owner=T-30-sso", "-p", "127.0.0.1::3306",
    ...
    redis_id = run(["docker", "run", "-d", "--name", redis, "--label", "namewta.test.owner=T-30-sso", "-p", "127.0.0.1::6379",
    ...
    if not record['counts']['tests'] or any(record['counts'][key] for key in ('failures','errors','skipped')): code = 1
...
print("T-21 regression exit:", code, flush=True)
sys.exit(code)
