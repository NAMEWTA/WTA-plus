...
def isolated_config(mysql_port, redis_port, app_user, app_password, run_dir):
    config = {
...
def validate_full_openapi(body, current):
    ...
    if missing_paths or missing_operations or missing_schemas:
        raise RuntimeError('Live full document lost paths, methods or schemas from the active snapshot')
...
def main():
    ...
    bucket = 't38-' + run_id
    secrets_to_redact = [root_password, app_password, minio_password]
    ...
    secret_files = [run_dir / name for name in ('mysql-container.env', 'minio-container.env', 'init.env', 'owned.yml')]
    try:
        ...
        helper.wait_ready(redis_id, 'redis', 45)
        ...
        config = isolated_config(mysql_port, redis_port, app_user, app_password, run_dir)
        ...
        report['openapi'].update({'http_status': 200, 'content_type': content_type,
