...
skill_scan: "scanned project Skill roots under <Path>.agents/skills/*/SKILL.md</Path> (engineering-standards, namewta-fullstack-development, ruoyi-common-modules-guide, ruoyi-module-guide, deploy-namewta-environment, java-api-compatibility, project-customization-delivery, upstream-fork-sync); excluded last three as non-axis for rename/W0b/publication; bound for T-13: engineering-standards, namewta-fullstack-development"
skill_bindings:
  - {"id":"engineering-standards","path":"<Path>.agents/skills/engineering-standards/SKILL.md</Path>","sha256":"acac7d1d4341d3361c298ebc5a61841d794cafbe9f235171b56fddd7ecb0037d","phase":"plan","operation":"record-publication-auth","inputs":["Ticket T-13","Tickets Map","Spec/ADR projections"],"outputs":["Evidence notes for T-13","next-step inputs"],"required":true,"on_failure":"block-ticket","references":[]}
  - {"id":"namewta-fullstack-development","path":"<Path>.agents/skills/namewta-fullstack-development/SKILL.md</Path>","sha256":"e17e31173638b06f93bb2c26cf3315f50a1a075b3e298434c98fc47d175d7792","phase":"plan","operation":"record-publication-auth","inputs":["Ticket T-13","Tickets Map","Spec/ADR projections"],"outputs":["Evidence notes for T-13","next-step inputs"],"required":true,"on_failure":"block-ticket","references":[]}
resource_claims:
  - "publication-auth-gate"
artifact: ticket
change: 2026-09-12-rename-ruoyi-dromara-to-wta
id: T-13
title: Explicit Publication Authorization（CTO）
status: done
planning_depth: standard
planning_depth_reason: 公开授权门记录；仅 CTO。
ready: true
risk: critical
blocked_by: [T-12]
...
# Ticket T-13: Explicit Publication Authorization（CTO）

- **Ticket 文件：** `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/ticket/13-pub-auth.md</Path>`
...
- **完成 Evidence：** `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/evidence/T-13.md</Path>`

实现本 Ticket 时，Lead 与 implementation subagent 必须按顺序完整读取总体 Map，读取项目 Skill 的 frontmatter 与入口并只展开适用于 `ALL`/`T-13` 的匹配项，再读取本 Ticket 与相关上游工件。Map 中的 Skill 是最低必读集合；新的匹配项先由 Lead 同步到 Map 并重新校验。
...
- **目标：** 在 publication checklist 绿后，由 **CTO 书面** 翻转 `public_repo_publication_authorized=true`；代理禁止自授。
- **可观察产出：** 状态字段与 LOG 记录授权来源；此前保持 false。
- **来源：** AC-PUBLICATION、AC-Gate、ADR-010。
- **当前事实：** 当前 public_repo_publication_authorized=false。
- **Planning Depth 原因：** 公开授权门记录；仅 CTO。
...
|---|---|---|
| 授权字段记录。 | T-12 evidence。 | 代 CTO 翻转；push。 |

## 4. 要构建什么

等待 CTO → 更新 status/LOG（仅那时）。
...
|---|---|---|---|---|
| 授权 | status | public_repo_publication_authorized | true 仅在 CTO 后 | evidence/T-13.md |
| Gate | AC-012 | 与实施门独立 | 是 | evidence/T-13.md |
...
- **不可逆批准点：** public auth。
- **回滚：** CTO 撤回则改回 false 并停 T-14。

## 10. 验收标准

- [ ] 仅 CTO 授权后字段为 true。
- [ ] 无自授。
- [ ] 实现开始前已完整读取 Tickets Map，已读取项目 Skill 入口并完整展开适用于 `ALL`/`T-13` 的匹配项。
- [ ] 验证矩阵全部执行并记录到 `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/evidence/T-13.md</Path>`（完成时；**本轮不新建伪 Evidence**）。
...
依据 `<Path>{roots.workflows}/specdev/common/rules/skill-invocation.md</Path>`：本票 `skill_bindings` 绑定 Map 中适用于 ALL/`T-13` 的每一 Path。
