<template>
  <div class="home-shell">
    <header class="home-header">
      ...
      <nav><router-link v-if="token && primaryMenu" :to="primaryMenu.path">{{ primaryMenu.meta?.title ?? '用户中心' }}</router-link><template v-else-if="!token"><router-link to="/login">登录</router-link><router-link v-if="registration.enabled" class="register-link" to="/register">注册</router-link></template><button v-if="token" type="button" @click="logout">退出</button></nav>
    </header>
...
@media (max-width: 420px) {
  .home-header { flex-wrap: wrap; justify-content: center; gap: 14px; padding: 18px 12px; }
  .brand { max-width: 100%; flex-wrap: wrap; justify-content: center; }
  nav { flex-wrap: wrap; justify-content: center; }
  footer { padding: 28px 12px; overflow-wrap: anywhere; }
}
</style>
