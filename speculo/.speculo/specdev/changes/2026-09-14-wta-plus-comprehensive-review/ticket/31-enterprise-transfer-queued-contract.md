...
- Integration evidence：记录parent before、implementation commit及direct-parent检查；result SHA等于通过验证的implementation commit，candidate不适用。本地验证见T-31.md及checkpoint；按用户指令不提交，implementation/result/candidate为空，不标Done。
...
- [x] 正常/失败/回归及required E2E均完成，证据写入<Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/evidence/T-31.md</Path>，未执行不得标通过。
...
Revision89：回读T-28最新414路径全部哈希一致。首轮设计：移除NotificationMode.SYNC并切换所有实际SYNC调用者，receipt以真实PENDING判断排队；transfer同事务保存服务端notificationId，confirm先锁定归属记录、拒绝未知/过期/重复，再经Notify公开query核验ACCEPTED/DELIVERED才幂等激活Redis；保留原验证码到期时间及CAS消费。状态等待由confirm明确返回QUEUED，前端保留挑战并提示等待；失败/过期可重新发码，无持久OTP或新事件总线。PersonRebind记真实QUEUED，Captcha保留缓存可用语义，测试发送保留真实返回状态。全部提交暂缓。

Revision91：原前端无转移表单；在已有档案中心组合最小转移面板，复用现有enterprise.transfer API与apply权限。登记独立Home Playwright配置及默认配置排除该owned suite，验证排队/失败/过期/重试、重复提交与卸载；不新增路由或权限。

Revision92：当前OpenAPI source实际有50条Profile路径，父级Skill与domain AGENTS仍称不存在。登记父Skill精确引用写集以修正事实；NotificationCommand.mode快照仍含SYNC，使用当前Java模型schema导出与正式fetch/generate/check更新对应enum，不声称整份历史快照为本轮全量运行时采集。

Revision93：已有快照具备Profile类型，本票transfer资源使用generated transport并映射为domain状态；登记pnpm-lock用于现有api-contracts工作区依赖，不变更外部版本。

## Revision135 实际提交与父分支验收

用户已明确授权全部commit/push。implementation commits：`ad93698fa1f5c9d41117f10f66d7ed9b65baa231`, `032bcecccec99080102a09c90a597e53fa7e0d36`；完整实现链 result SHA：`6c8764cca97bb6057fcb90ccdfe635c7efbf502a`。每个提交均非空、实际父SHA已核对且被result包含；Git归档逐文件等于T-30已验证输入，未声称拆分过程中的中间树独立通过全部测试。精确路径/共享owner/验证见 `../evidence/commit-delivery.json`。本票保持review；正式发布候选与change最终Done独立验收。
