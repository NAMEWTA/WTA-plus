#!/usr/bin/env python3
"""Serial owned MySQL/Redis and real HTTP CRUD contract validation."""
import datetime
import os
import signal
import json
...
database = 'namewta_crud_test_' + suffix
password = 'owned-crud-test-only'
code = 1
process = None
...
start = time.time()
try:
    mysql = run(['docker', 'run', '-d', '--name', 'namewta-t26-mysql-' + suffix, '--label', 'namewta.test.owner=T-26', '-p', '127.0.0.1::3306',
    ...
    redis = run(['docker', 'run', '-d', '--name', 'namewta-t26-redis-' + suffix, '--label', 'namewta.test.owner=T-26', '-p', '127.0.0.1::6379', images['redis']])
    record['containers'].append(redis)
    for attempt in range(30):
        ready = subprocess.run(['docker', 'exec', redis, 'redis-cli', 'ping'], capture_output=True, text=True)
        if ready.returncode == 0 and ready.stdout.strip() == 'PONG': break
        time.sleep(1)
    else: raise RuntimeError('Owned Redis readiness timeout')
    redis_port = run(['docker', 'port', redis, '6379/tcp']).rsplit(':', 1)[1]
    command = ['./mvnw', '-B', '-ntp', '-pl', 'wta-admin', '-am', '-Dtest=CrudMySqlHttpIntegrationTest,CrudHttpDispatchIntegrationTest,OssUploadHttpContractUnitTest',
               '-Dsurefire.failIfNoSpecifiedTests=false', '-Dprofiles.active=dev,local', '-Dcrud.mysql.integration=true',
               '-Dcrud.mysql.integration.url=jdbc:mysql://127.0.0.1:' + mysql_port + '/' + database + '?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=Asia/Shanghai',
               '-Dcrud.redis.integration.port=' + redis_port, '-Dcrud.mysql.output=' + str(evidence / (name + '-observations.json')), '-Dcrud.http.output=' + str(evidence / (name + '-http-observations.json')), 'test']
    record.update(command=command, cwd='backend', log=name + '.log')
    print('Running actual HTTP CRUD with owned MySQL/Redis', flush=True)
    with (evidence/record['log']).open('w') as log:
        ...
        for line in process.stdout:
            ...
            if any(marker in line for marker in ['[ERROR]', 'Tests run:', 'BUILD SUCCESS', 'BUILD FAILURE', 'T-26:', ' passed (', ' failed']): print(line, end='', flush=True)
        code = process.wait()
