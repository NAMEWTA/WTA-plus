...
start = time.time()
try:
    mysql = run(['docker', 'run', '-d', '--name', 'namewta-t16-mysql-' + suffix, '--label', 'namewta.test.owner=T-16', '-p', '127.0.0.1::3306',
    ...
    redis = run(['docker', 'run', '-d', '--name', 'namewta-t16-redis-' + suffix, '--label', 'namewta.test.owner=T-16', '-p', '127.0.0.1::6379', images['redis'], 'redis-server', '--save', '', '--appendonly', 'no'])
    ...
    command = ['./mvnw', '-B', '-ntp', '-pl', 'wta-admin', '-am', '-Dtest=WorkflowTaskEngineIntegrationTest',
               '-Dsurefire.failIfNoSpecifiedTests=false', '-Dworkflow.engine.integration=true', '-Dprofile.redis.integration.port=' + redis_port,
    ...
    print('Running actual Warm-Flow/MySQL authorization', flush=True)
    with (evidence/record['log']).open('w') as log:
        ...
        for line in process.stdout:
            ...
            if any(marker in line for marker in ['[ERROR]', 'Tests run:', 'BUILD SUCCESS', 'BUILD FAILURE', 'T-16:', ' passed (', ' failed']): print(line, end='', flush=True)
        code = process.wait()
