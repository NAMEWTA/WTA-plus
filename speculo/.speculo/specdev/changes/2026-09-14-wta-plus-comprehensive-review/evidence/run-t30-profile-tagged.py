...
database = 'namewta_notify_test_' + suffix
password = 'owned-notify-test-only'
code = 1
process = None
...
start = time.time()
try:
    mysql = run(['docker', 'run', '-d', '--name', 'namewta-t30-profile-e2e-mysql-' + suffix, '--label', 'namewta.test.owner=T-30', '-p', '127.0.0.1::3306',
    ...
    redis = run(['docker', 'run', '-d', '--name', 'namewta-t30-profile-e2e-redis-' + suffix, '--label', 'namewta.test.owner=T-30', '-p', '127.0.0.1::6379', images['redis'], 'redis-server', '--save', '', '--appendonly', 'no'])
    ...
    command = ['./mvnw', '-B', '-ntp', '-pl', 'wta-admin', '-am', '-Dtest=PersonRebindMySqlE2ETest,PersonIdentityLookupMySqlE2ETest,PersonAdminMySqlE2ETest,PersonApplicationMySqlE2ETest,PersonVerificationMySqlE2ETest,ProfileMaterialMySqlE2ETest,EnterpriseApplicationMySqlE2ETest,EnterpriseVerificationMySqlE2ETest,EnterpriseAdminMySqlE2ETest',
               '-Dsurefire.failIfNoSpecifiedTests=false', '-Dprofiles.active=dev,e2e', '-Dprofile.notify.integration=true', '-Dnotify.redis.integration.port=' + redis_port,
               '-Dnotify.mysql.integration.url=jdbc:mysql://127.0.0.1:' + mysql_port + '/' + database + '?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=Asia/Shanghai',
               '-Dnamewta.repo.root=' + str(root), 'test']
    record.update(command=command, cwd='backend', log=name + '.log')
    print('Running nine Profile tagged E2E classes against owned baseline', flush=True)
    with (evidence/record['log']).open('w') as log:
        process = subprocess.Popen(command, cwd=root/'backend', env={**os.environ, 'PROFILE_REDIS_E2E_ADDRESS': 'redis://127.0.0.1:' + redis_port, 'PROFILE_MYSQL_E2E_URL': 'jdbc:mysql://127.0.0.1:' + mysql_port + '/' + database + '?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=Asia/Shanghai', 'PROFILE_MYSQL_E2E_USERNAME': 'root', 'PROFILE_MYSQL_E2E_PASSWORD': password}, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
        for line in process.stdout:
            line = re.sub(r'''([?&]X-Amz-[^=]+)=([^&\s"']+)''', r'\1=[REDACTED]', line)
            log.write(line); log.flush()
            if any(marker in line for marker in ['[ERROR]', 'Tests run:', 'BUILD SUCCESS', 'BUILD FAILURE', 'T-31:', ' passed (', ' failed']): print(line, end='', flush=True)
        code = process.wait()
