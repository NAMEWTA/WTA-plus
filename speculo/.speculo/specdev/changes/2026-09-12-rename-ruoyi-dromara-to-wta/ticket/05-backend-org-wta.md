...
skill_scan: "scanned project Skill roots under <Path>.agents/skills/*/SKILL.md</Path> (engineering-standards, namewta-fullstack-development, ruoyi-common-modules-guide, ruoyi-module-guide, deploy-namewta-environment, java-api-compatibility, project-customization-delivery, upstream-fork-sync); excluded last three as non-axis for rename/W0b/publication; bound for T-05: engineering-standards, namewta-fullstack-development, ruoyi-common-modules-guide, ruoyi-module-guide"
skill_bindings:
  - {"id":"engineering-standards","path":"<Path>.agents/skills/engineering-standards/SKILL.md</Path>","sha256":"acac7d1d4341d3361c298ebc5a61841d794cafbe9f235171b56fddd7ecb0037d","phase":"implement","operation":"rename-backend-org-wta","inputs":["Ticket T-05","Tickets Map","Spec/ADR projections"],"outputs":["Evidence notes for T-05","next-step inputs"],"required":true,"on_failure":"block-ticket","references":[]}
  - {"id":"namewta-fullstack-development","path":"<Path>.agents/skills/namewta-fullstack-development/SKILL.md</Path>","sha256":"e17e31173638b06f93bb2c26cf3315f50a1a075b3e298434c98fc47d175d7792","phase":"implement","operation":"rename-backend-org-wta","inputs":["Ticket T-05","Tickets Map","Spec/ADR projections"],"outputs":["Evidence notes for T-05","next-step inputs"],"required":true,"on_failure":"block-ticket","references":[]}
  - {"id":"ruoyi-common-modules-guide","path":"<Path>.agents/skills/ruoyi-common-modules-guide/SKILL.md</Path>","sha256":"79fa186e015f38b9c91a348a08ec59c92aa5abc016c693b828577976ac33d362","phase":"implement","operation":"rename-backend-org-wta","inputs":["Ticket T-05","Tickets Map","Spec/ADR projections"],"outputs":["Evidence notes for T-05","next-step inputs"],"required":true,"on_failure":"block-ticket","references":[]}
  - {"id":"ruoyi-module-guide","path":"<Path>.agents/skills/ruoyi-module-guide/SKILL.md</Path>","sha256":"529cd870b94a789d7504dae81c95629d417fbaa4f89f9cdd0c311387128fa0db","phase":"implement","operation":"rename-backend-org-wta","inputs":["Ticket T-05","Tickets Map","Spec/ADR projections"],"outputs":["Evidence notes for T-05","next-step inputs"],"required":true,"on_failure":"block-ticket","references":[]}
resource_claims:
  - "backend-package-rename"
  - "maven-groupid-org-wta"
artifact: ticket
change: 2026-09-12-rename-ruoyi-dromara-to-wta
id: T-05
title: Backend Namespace Rename → org.namewta
status: blocked
planning_depth: standard
planning_depth_reason: 自有 Java/Maven 命名空间迁移；KEEP 防误伤；依赖 W0b。
ready: false
risk: critical
blocked_by: [T-04, T-16]
contract_ids: [AC-002, AC-003, NAC-01, NAC-06]
owner: unassigned
expected_changes:
...
# Ticket T-05: Backend Namespace Rename → org.namewta

- **Ticket 文件：** `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/ticket/05-backend-org-wta.md</Path>`
...
- **完成 Evidence：** `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/evidence/T-05.md</Path>`

实现本 Ticket 时，Lead 与 implementation subagent 必须按顺序完整读取总体 Map，读取项目 Skill 的 frontmatter 与入口并只展开适用于 `ALL`/`T-05` 的匹配项，再读取本 Ticket 与相关上游工件。Map 中的 Skill 是最低必读集合；新的匹配项先由 Lead 同步到 Map 并重新校验。
...
- **目标：** 在 **WTA-plus 准备树/授权写面** 将自有 `org.dromara`→`org.namewta`；第三方 KEEP。
- **可观察产出：** 自有包/groupId 为 org.namewta；sms4j/warm/easyes/mica 不变；编译可通过定向模块。
- **来源：** ADR-002、AC-PACKAGE、FR-002。
- **当前事实：** 根 POM groupId=org.dromara；自有包 org.dromara.{common,system,...}。
- **Planning Depth 原因：** 自有 Java/Maven 命名空间迁移；KEEP 防误伤；依赖 W0b。
...
- mapper/typeAliases/springdoc/AutoConfiguration 随包名同步。
...
|---|---|---|
| 自有后端命名空间。 | inventory、KEEP。 | 第三方包；前缀目录搬迁（T-07）；FE。 |

## 4. 要构建什么

按 inventory 迁移自有包与 groupId，同步扫描配置；KEEP 回归绿灯。
...
1. 确认 W0b 完成+授权。
2. 按 inventory 迁 OWNED。
3. 同步 MyBatis/扫描/AutoConfiguration。
4. KEEP rg + 定向 package。
5. 安全落点 commit（准备树）。
...
|---|---|---|---|---|
| 自有包 | compile | mvn -pl … -am -DskipTests | SUCCESS | evidence/T-05.md |
| KEEP | rg | 四件套坐标不变 | 是 | evidence/T-05.md |
| 无盲替换 | diff 审阅 | 无第三方改动 | 是 | evidence/T-05.md |
...
- **回滚：** 回退该波次 commit。
- **兼容：** 无对外已发布坐标承诺（未 public）。

## 10. 验收标准

- [ ] `AC-002` 自有 org.namewta。
- [ ] KEEP 回归通过。
- [ ] 非 zero-match 验收。
- [ ] 未在旧仓主线 rename。
- [ ] 实现开始前已完整读取 Tickets Map，已读取项目 Skill 入口并完整展开适用于 `ALL`/`T-05` 的匹配项。
- [ ] 验证矩阵全部执行并记录到 `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/evidence/T-05.md</Path>`（完成时；**本轮不新建伪 Evidence**）。
...
依据 `<Path>{roots.workflows}/specdev/common/rules/skill-invocation.md</Path>`：本票 `skill_bindings` 绑定 Map 中适用于 ALL/`T-05` 的每一 Path。
