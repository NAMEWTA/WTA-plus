...
DRIVER = Path('/tmp/wta-t39/run-notify-deadline-integration.py')
spec = importlib.util.spec_from_file_location('t39_driver', DRIVER)
...
class DriverSafetyTest(unittest.TestCase):
    ...
    def test_three_exact_fresh_classes_positive_zero_skip(self):
        with tempfile.TemporaryDirectory() as directory:
            started = time.time_ns()
            paths = []
            for klass in (driver.TEST_CLASS, driver.PROFILE_CLASS, driver.WAKE_CLASS):
                ...
                paths.append(path)
            paths[2].write_text(f'<testsuite name="{driver.WAKE_CLASS}" tests="2" failures="0" errors="0" skipped="1"/>')
            with self.assertRaisesRegex(RuntimeError, 'zero tests'):
                driver.xml_verdict_for(driver.WAKE_CLASS, paths[2], started)
    ...
    def test_source_and_selector_are_t39_only(self):
        source = DRIVER.read_text()
        self.assertIn("selectors = 'NotifyDeadlineIntegrationTest,EnterpriseQueuedNotificationIntegrationTest,NotifyWakeIntegrationTest'", source)
        ...
        self.assertNotIn('NotifyAtomicResultIntegrationTest,', source)
        self.assertNotIn('NotifyManualRetryIntegrationTest', source)
