...
owned = Path(tempfile.mkdtemp(prefix='t21-browser-', dir=root / 'temp/team/lead'))
servers = []
threads = []
...
try:
    ...
    for app in ('home', 'sso'):
        ...
        env[f'A11Y_{app.upper()}_ORIGIN'] = f'https://127.0.0.1:{server.server_port}'
    env['A11Y_EVIDENCE_DIR'] = str(evidence / (name + '-artifacts'))
    command = (['corepack', 'pnpm', 'exec', 'playwright', 'test', '--config', 'playwright.accessibility.config.ts', *sys.argv[3:]] if len(sys.argv) > 2 and sys.argv[2] == '--tests' else ['node', str(evidence / 'run-t21-diagnostics.mjs'), name, *sys.argv[2:]])
    ...
    row = dict(command=command, cwd='frontend', exit_code=result.returncode, seconds=round(time.time()-start, 2),
               log=str(log.relative_to(root)), owned_origins=[env[f'A11Y_{app.upper()}_ORIGIN'] for app in ('home','sso')],
