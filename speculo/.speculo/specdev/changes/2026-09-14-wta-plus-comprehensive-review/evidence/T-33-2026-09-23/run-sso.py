"""Serial T-06 verification using owned localhost MySQL/Redis and real Chrome."""
import argparse
import datetime
import json
import pathlib
...
evidence = pathlib.Path("/tmp/wta-t33")
root = pathlib.Path("/srv/WTA-plus")
...
try:
    run(["docker", "run", "-d", "--name", mysql, "--label", "namewta.test.owner=T-06", "-p", "127.0.0.1::3306",
    ...
    run(["docker", "run", "-d", "--name", redis, "--label", "namewta.test.owner=T-06", "-p", "127.0.0.1::6379",
    ...
    if args.test == "ALL":
        command = [part for part in command if not part.startswith("-Dtest=")]
    if args.journey:
...
print("T-06 exit:", code, flush=True)
sys.exit(code)
