...
name=sys.argv[1]; assert name.startswith('T-16-') and '/' not in name
owned=Path(tempfile.mkdtemp(prefix='t16-dialog-',dir=root/'temp/team/lead'))
...
pw=owned/'playwright.config.mjs';pw.write_text('export default '+json.dumps(dict(testDir=str(web/'e2e'),testMatch='workflow-task-integrity.spec.ts',outputDir=str(web/'tests/e2e/reports/t16-dialog'),workers=1,retries=0,maxFailures=1,reporter='line',use=dict(baseURL=f'http://127.0.0.1:{port}',channel='chrome'),webServer=dict(command=f'corepack pnpm exec vite preview --config {config} --host 127.0.0.1 --port {port} --strictPort',url=f'http://127.0.0.1:{port}/packages/web-domains/workflow/src/components/fixtures/task-integrity/index.html',reuseExistingServer=False,cwd=str(web))),ensure_ascii=False)+';\n')
...
try:
 ...
 record=dict(results=results,vite_config=config.read_text(),playwright_config=pw.read_text());shutil.rmtree(owned);record['owned_cleanup']=not owned.exists()
...
sys.exit(0 if len(results)==2 and all(r['exit_code']==0 for r in results) and record['preview_closed'] else 1)
