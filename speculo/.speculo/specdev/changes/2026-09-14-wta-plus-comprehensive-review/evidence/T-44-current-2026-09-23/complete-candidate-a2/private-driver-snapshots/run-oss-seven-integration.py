#!/usr/bin/env python3
"""Seven existing OSS integration classes on three wholly owned services.
...
RUN_ROOT = Path('/tmp/wta-t44/oss-seven-runs')
...
BROWSER_OUTPUT = ROOT / 'frontend/tests/e2e/reports/upload-results'
SUITES = (
    ...
    'org.namewta.test.oss.client.MinioOssClientIntegrationTest',
    'org.namewta.test.oss.migration.OssStorageMigrationIntegrationTest',
    'org.namewta.test.oss.upload.OssUploadStorageRoutingMinioIntegrationTest',
    'org.namewta.test.oss.upload.BrowserUploadLifecycleIntegrationTest',
    'org.namewta.test.oss.upload.RedisOssUploadTicketStoreIntegrationTest',
)
BROWSER_SUITE = SUITES[5]
PROPERTY_KEYS = (
    't44.owned.run', 'oss.minio.integration.endpoint',
    ...
    'oss.upload.redis.integration.port', 'browser.upload.integration', 'namewta.repo.root',
)
TOOL_BIN = Path('/tmp/wta-t02-c1/tool-bin')
TOOL_BIN_PNPM_SHA = 'e8a43b6185d59350a8d6c4f1c8e0c23b3d70d48290907d2f78098234413dbe4c'
...
def load_base():
    ...
    spec = importlib.util.spec_from_file_location('t44_oss_seven_base', BASE_PATH)
...
def dist_state(base):
    ...
    for app in ('admin-web', 'home-web', 'sso-web'):
        directory = ROOT / 'frontend/apps' / app / 'dist'
        if not (directory / 'index.html').is_file():
            raise RuntimeError('one of the three built frontend distributions is missing')
        files = sorted(path for path in directory.rglob('*') if path.is_file())
        if not files or any(path.is_symlink() for path in files):
            raise RuntimeError('frontend distribution is empty or has a symlink')
        digest = hashlib.sha256()
        for path in files:
            digest.update(str(path.relative_to(directory)).encode())
            digest.update(bytes.fromhex(base.sha_file(path)))
        result[app] = {'files': len(files), 'manifest_sha256': digest.hexdigest()}
    return result


def browser_output_before():
    """Protect the lone old Playwright .last-run.json from implicit overwrite."""
    if BROWSER_OUTPUT.is_symlink():
        raise RuntimeError('Playwright output directory is a symlink')
    if not BROWSER_OUTPUT.exists():
        return {'existed': False, 'last_run': None}
    if not BROWSER_OUTPUT.is_dir():
        raise RuntimeError('Playwright output path is not a directory')
    entries = list(BROWSER_OUTPUT.iterdir())
    if any(path.name != '.last-run.json' or path.is_symlink() or not path.is_file()
           for path in entries):
        raise RuntimeError('Existing Playwright output is not the bounded old last-run file')
    last = BROWSER_OUTPUT / '.last-run.json'
    content = last.read_bytes() if last.exists() else None
    if content is not None and len(content) > 4096:
        raise RuntimeError('Old Playwright last-run metadata exceeds bound')
    return {'existed': True, 'last_run': content}


def restore_browser_output(previous):
    if previous is None:
        return
    if BROWSER_OUTPUT.is_symlink():
        raise RuntimeError('Playwright output became a symlink')
    if BROWSER_OUTPUT.exists():
        shutil.rmtree(BROWSER_OUTPUT)
    if previous['existed']:
        BROWSER_OUTPUT.mkdir(parents=True, exist_ok=False)
        if previous['last_run'] is not None:
            private(BROWSER_OUTPUT / '.last-run.json', previous['last_run'])
...
def safe_xml_summary(name, raw, expected_run):
    ...
    marker = None if props is None else next(
        (item.attrib.get('value') for item in props.findall('property')
         if item.attrib.get('name') == 't44.owned.run'), None)
    ...
    if name == BROWSER_SUITE:
        output = '\n'.join(node.text or '' for node in suite.findall('.//system-out'))
        output = ANSI.sub('', output).replace('\r', '\n')
        indices = {int(value) for value in re.findall(r'\[(\d+)/10\]', output)}
        if not re.search(r'\b10 passed\b', output) or indices != set(range(1, 11)):
            raise RuntimeError('Embedded Playwright 10-case success evidence is absent')
        if re.search(r'\b\d+\s+(?:failed|skipped|flaky)\b', output):
            raise RuntimeError('Embedded Playwright output reports a nonpassing case')
        summary['browser'] = {'passed': 10, 'indices': sorted(indices), 'skipped': 0}
    return summary
...
def retain_fresh_reports(started_ns, run_dir, generated, run_id):
    ...
    for name in SUITES:
        ...
        try:
            ...
            temp = path.with_name(path.name + '.t44-' + run_id + '.tmp')
...
def retain_fresh_text_and_temp(started_ns, run_dir, generated):
    ...
    for name in SUITES:
        ...
        temp = path.with_name(path.name + '.t44-owned.tmp')
        private(temp, clean)
        os.replace(temp, path)
...
def retain_owned_chrome_logs(tmp_dir, run_dir, generated):
    retained = []
    ...
    for path in sorted(tmp_dir.rglob('*.log')):
        ...
        raw = path.read_bytes()
        clean, count = sanitize_bytes(raw, generated)
        destination = run_dir / ('chrome-' + str(len(retained) + 1) + '.log')
...
def live_owned_processes(tmp_dir):
    """Track Chrome/Node that may leave Maven's process group, by unique tmp marker."""
...
def main():
    ...
    phase = 'preflight'
    cleanup_errors = []
    report = {'gate': 'T44 seven existing OSS integration classes', 'run_id': run_id,
              'owner': OWNER, 'source_before': None, 'source_after': None,
              'dist_before': None, 'dist_after': None, 'maven_exit_code': None,
              'suites': [], 'cleanup': {}, 'acceptance': False, 'exit_code': 1,
              'started_utc': dt.datetime.now(dt.timezone.utc).isoformat()}
    try:
        base = load_base()
        report['source_before'] = base.source_identity(args.expected_head)
        report['dist_before'] = dist_state(base)
        browser_previous = browser_output_before()
        for name in SUITES:
        ...
        if not (ROOT / 'backend/mvnw').is_file() or not (TOOL_BIN / 'pnpm').is_file() \
                or base.sha_file(TOOL_BIN / 'pnpm') != TOOL_BIN_PNPM_SHA:
            raise RuntimeError('Required Maven wrapper or pinned pnpm tool shim differs')
        root_password, migration_password, minio_password = (random_password() for _ in range(3))
        minio_user = 't44oss' + run_id
        migration_user = 't44_' + run_id
        migration_db = 't44_oss_migration_' + run_id
        ...
        settings = {
            ...
            'browser.upload.integration': 'true',
        ...
        if started_ns is not None:
            ...
            try:
                report['owned_chrome_logs'] = retain_owned_chrome_logs(run_dir / 'tmp', run_dir, generated)
            except Exception:
                cleanup_errors.append('owned_chrome_log_retention_failed')
        raw_log = run_dir / 'maven.raw.log'
        if raw_log.is_file():
            try:
        ...
        try:
            if browser_touched:
                restore_browser_output(browser_previous)
            report['cleanup']['browser_output_restored'] = browser_touched and browser_previous is not None
        except Exception:
            cleanup_errors.append('browser_output_restore_failed')
        ...
        if base is not None:
            try:
                ...
                report['dist_after'] = dist_state(base)
                if report['source_after'] != report['source_before']:
                    cleanup_errors.append('source_changed_during_gate')
                if report['dist_after'] != report['dist_before']:
                    cleanup_errors.append('frontend_distribution_changed_during_gate')
            except Exception:
                cleanup_errors.append('source_or_distribution_after_check_failed')
        suites = report.get('suites', [])
        suite_ok = (len(suites) == len(SUITES)
                    and all(item.get('fresh') and item.get('private_properties_marker')
                            and item.get('counts', {}).get('tests') == 1
                            and all(item['counts'].get(key) == 0 for key in ('failures', 'errors', 'skipped'))
                            for item in suites)
                    and suites[5].get('browser', {}).get('passed') == 10)
