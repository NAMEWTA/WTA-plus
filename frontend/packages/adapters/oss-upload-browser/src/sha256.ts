...
function toHex(bytes: ArrayBuffer | Uint8Array) {
  return Array.from(bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes), value => value.toString(16).padStart(2, '0')).join('');
}
...
/** 纯 JS SHA-256。HTTP 局域网页面没有 crypto.subtle，上传指纹不能因此失败。 */
export function sha256Bytes(message: Uint8Array) {
  const bitLength = message.length * 8;
  const zeroPad = (56 - ((message.length + 1) % 64) + 64) % 64;
  const padded = new Uint8Array(message.length + 1 + zeroPad + 8);
  padded.set(message);
