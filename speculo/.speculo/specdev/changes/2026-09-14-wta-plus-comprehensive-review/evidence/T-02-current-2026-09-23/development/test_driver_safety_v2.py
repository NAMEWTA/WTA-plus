...
DRIVER = Path('/tmp/wta-t02/run-log-redaction-integration-v2.py')
...
class DriverSafetyTest(unittest.TestCase):
    ...
    def test_baseline_green_cannot_claim_ticket_canary(self):
        ...
        self.assertFalse(driver.ticket_acceptance(1, driver.CANARY_METHOD))
        self.assertFalse(driver.ticket_acceptance(0, 'wrongMethod'))
        self.assertTrue(driver.ticket_acceptance(0, driver.CANARY_METHOD))
        self.assertEqual(len(driver.TESTS), 7)
        self.assertIn(driver.HTTP_TEST, driver.TESTS)
        self.assertEqual(driver.CANARY_METHOD, 'onlineDeviceTokenFromRealIssuerNeverReachesHttpOrErrorAudit')
    ...
    def test_failed_xml_retains_exact_method_and_error_type_without_message(self):
        with tempfile.TemporaryDirectory() as directory:
            started = time.time_ns()
            report = Path(directory) / 'failed.xml'
            report.write_text(f'<testsuite name="{driver.HTTP_TEST}" tests="1" failures="0" errors="1" skipped="0">'
                f'<testcase name="{driver.CANARY_METHOD}"><error type="java.lang.IllegalStateException" '
                f'message="credential-canary-secret">private stack</error></testcase></testsuite>')
            issues = driver.xml_issues_for(driver.HTTP_TEST, report, started)
            self.assertEqual(issues, [{'method': driver.CANARY_METHOD, 'kind': 'error',
                                       'error_type': 'java.lang.IllegalStateException'}])
            self.assertNotIn('credential-canary-secret', str(issues))
    ...
    def test_private_log_redacts_passwords_and_known_canary(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            driver.sanitize_log(raw, clean, ('root-secret', 'app-secret'))
            self.assertFalse(raw.exists())
            content = clean.read_text()
            self.assertNotIn('root-secret', content)
            self.assertNotIn('app-secret', content)
    ...
    def test_maven_test_has_no_application_secret_property(self):
        ...
        self.assertNotIn("IDENTIFIED BY '';", source)
        self.assertIn('GRANT CREATE, DROP, INDEX, SELECT, INSERT', source)
        self.assertIn('T02_MYSQL_PASSWORD', source)
        self.assertIn('safe_env(T02_MYSQL_PASSWORD=app_password)', source)
        self.assertIn("result['owned']['captured_anonymous_volume_names'] = list(volumes)", source)
