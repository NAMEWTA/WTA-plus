...
skill_scan: "scanned project Skill roots under <Path>.agents/skills/*/SKILL.md</Path> (engineering-standards, namewta-fullstack-development, ruoyi-common-modules-guide, ruoyi-module-guide, deploy-namewta-environment, java-api-compatibility, project-customization-delivery, upstream-fork-sync); excluded last three as non-axis for rename/W0b/publication; bound for T-12: engineering-standards, namewta-fullstack-development"
skill_bindings:
  - {"id":"engineering-standards","path":"<Path>.agents/skills/engineering-standards/SKILL.md</Path>","sha256":"acac7d1d4341d3361c298ebc5a61841d794cafbe9f235171b56fddd7ecb0037d","phase":"verify","operation":"publication-readiness-checklist","inputs":["Ticket T-12","Tickets Map","Spec/ADR projections"],"outputs":["Evidence notes for T-12","next-step inputs"],"required":true,"on_failure":"block-ticket","references":[]}
  - {"id":"namewta-fullstack-development","path":"<Path>.agents/skills/namewta-fullstack-development/SKILL.md</Path>","sha256":"e17e31173638b06f93bb2c26cf3315f50a1a075b3e298434c98fc47d175d7792","phase":"verify","operation":"publication-readiness-checklist","inputs":["Ticket T-12","Tickets Map","Spec/ADR projections"],"outputs":["Evidence notes for T-12","next-step inputs"],"required":true,"on_failure":"block-ticket","references":[]}
resource_claims:
  - "publication-readiness"
artifact: ticket
change: 2026-09-12-rename-ruoyi-dromara-to-wta
id: T-12
title: Publication Readiness Checklist（含清 .gitee）
status: blocked
planning_depth: standard
planning_depth_reason: 公开前清理与清单总控。
ready: false
risk: critical
...
# Ticket T-12: Publication Readiness Checklist（含清 .gitee）

- **Ticket 文件：** `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/ticket/12-publication-readiness.md</Path>`
...
- **完成 Evidence：** `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/evidence/T-12.md</Path>`

实现本 Ticket 时，Lead 与 implementation subagent 必须按顺序完整读取总体 Map，读取项目 Skill 的 frontmatter 与入口并只展开适用于 `ALL`/`T-12` 的匹配项，再读取本 Ticket 与相关上游工件。Map 中的 Skill 是最低必读集合；新的匹配项先由 Lead 同步到 Map 并重新校验。
...
- **目标：** 完成 publication gate：secret/forbidden/KEEP/license/residual/baseline；**必须**扫描并清除 `.gitee` 及同类旧平台/CI/钩子残留；确认无上游 URL 依赖（ADR-012）。
- **可观察产出：** `evidence/publication/` checklist 全绿；无 .gitee 残留。
- **来源：** §2.5、ADR-012、AC-PUBLICATION、NAC-02。
- **当前事实：** 历史树可能含 .gitee 与上游 URL。
- **Planning Depth 原因：** 公开前清理与清单总控。

## 2. 决策状态

### 已锁定决策

- 无 public first clean later。
- 清 .gitee 必选项。
...
|---|---|---|
| publication checklist 与清理证明。 | T-09/T-10、SOURCE-BASELINE。 | 实际 public push（T-13/T-14）。 |

## 4. 要构建什么

跑扫描→清 .gitee/旧钩子→确认 URL/KEEP/secret→写 evidence/publication。

## 5. 实现契约

- **输出：** publication evidence。
- **失败：** 任一门禁红不得进 T-13。
- **兼容要求：** 见发布/迁移节；未授权不实施。
...
1. secret/forbidden 扫描。
2. `.gitee` 与旧 CI/钩子清除证明。
3. KEEP/license/residual。
4. 上游 URL 处置确认。
5. baseline 对照。
...
|---|---|---|---|---|
| .gitee | rg/清单 | 无残留 | 是 | evidence/publication/ |
| secrets | scan | 无凭据 | 是 | evidence/publication/ |
| KEEP | rg | 通过 | 是 | evidence/publication/ |
| 无上游依赖 | URL 审阅 | 自有或移除 | 是 | evidence/publication/ |
| 非 zero-match | 文案 | 遵守 NAC-05 | 是 | evidence/publication/ |
...
- [ ] `.gitee` 已清。
- [ ] 未安排未授权 public push。
- [ ] 实现开始前已完整读取 Tickets Map，已读取项目 Skill 入口并完整展开适用于 `ALL`/`T-12` 的匹配项。
- [ ] 验证矩阵全部执行并记录到 `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/evidence/T-12.md</Path>`（完成时；**本轮不新建伪 Evidence**）。
...
依据 `<Path>{roots.workflows}/specdev/common/rules/skill-invocation.md</Path>`：本票 `skill_bindings` 绑定 Map 中适用于 ALL/`T-12` 的每一 Path。
