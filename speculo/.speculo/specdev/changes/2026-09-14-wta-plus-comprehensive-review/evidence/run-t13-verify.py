#!/usr/bin/env python3
"""Serial local T-13 checks. No release promotion or external writes."""
import json
import os
...
mode = sys.argv[2]
shim = Path(tempfile.mkdtemp(prefix='t13-pnpm-', dir=root / 'temp/team/lead'))
...
if mode == 'build':
    ...
    commands = [('frontend', ['corepack', 'pnpm', 'exec', 'tsc', '--ignoreConfig', '--noEmit', '--strict', '--skipLibCheck', '--target', 'ES2023', '--module', 'ESNext', '--moduleResolution', 'Bundler', '--types', 'node', 'e2e/recoverable-registration.spec.ts', 'playwright.registration.config.ts']),
                ('frontend', ['corepack', 'pnpm', 'exec', 'oxlint', 'playwright.registration.config.ts']),
                ('.', ['node', '.agents/skills/engineering-standards/scripts/validate-skill-facts.mjs']),
                ('.', ['git', 'diff', '--check'])]
