...
upstream = json.loads((evidence/'T-30-input-checkpoint.json').read_text())
...
save('T-30-source-checkpoint.json', dict(captured_at=datetime.datetime.now(datetime.timezone.utc).isoformat(),
    parent_before=head, implementation_commit=None, result_sha=None, candidate=None,
    purpose='preparation-only; not a validated release candidate', scope=list(prefixes)+['root files'],
...
discovery = json.loads((evidence/'T-30-browser-discovery.json').read_text())
fix = json.loads((evidence/'T-30-browser-discovery-fixture.json').read_text())
for row in fix:
    config = row['command'][row['command'].index('--config')+1]
    discovery = [item for item in discovery if item['config'] != config]
    discovery.append({**row, 'config': config})
...
save('T-30-preflight-inventory.json', dict(captured_at=datetime.datetime.now(datetime.timezone.utc).isoformat(),
    status='preparation-only', acceptance_status='not-run', source_fingerprint=fingerprint,
    upstream_paths_verified=len(upstream['files']), source_paths=len(files),
    ticket_statuses={r['id']:r['status'] for r in plan},
    blockers=['T-03 legitimate JSON/machine body sizes and memory/concurrency budget unknown',
              'T-23 actual provider/account event identity and retry contract unknown',
              'All implementation commits held; direct-parent/result/release candidate incomplete'],
