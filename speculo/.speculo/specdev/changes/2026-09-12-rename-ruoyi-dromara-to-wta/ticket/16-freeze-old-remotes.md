...
skill_scan: "scanned project Skill roots under <Path>.agents/skills/*/SKILL.md</Path> (engineering-standards, namewta-fullstack-development, ruoyi-common-modules-guide, ruoyi-module-guide, deploy-namewta-environment, java-api-compatibility, project-customization-delivery, upstream-fork-sync); excluded last three as non-axis for rename/W0b/publication; bound for T-16: engineering-standards, namewta-fullstack-development"
skill_bindings:
  - {"id":"engineering-standards","path":"<Path>.agents/skills/engineering-standards/SKILL.md</Path>","sha256":"acac7d1d4341d3361c298ebc5a61841d794cafbe9f235171b56fddd7ecb0037d","phase":"implement","operation":"document-freeze-semantics","inputs":["Ticket T-16","Tickets Map","Spec/ADR projections"],"outputs":["Evidence notes for T-16","next-step inputs"],"required":true,"on_failure":"block-ticket","references":[]}
  - {"id":"namewta-fullstack-development","path":"<Path>.agents/skills/namewta-fullstack-development/SKILL.md</Path>","sha256":"e17e31173638b06f93bb2c26cf3315f50a1a075b3e298434c98fc47d175d7792","phase":"implement","operation":"document-freeze-semantics","inputs":["Ticket T-16","Tickets Map","Spec/ADR projections"],"outputs":["Evidence notes for T-16","next-step inputs"],"required":true,"on_failure":"block-ticket","references":[]}
resource_claims:
  - "legacy-freeze-semantics"
artifact: ticket
change: 2026-09-12-rename-ruoyi-dromara-to-wta
id: T-16
title: Freeze Old Remotes Semantics（alias T04b / T-freeze-old-remotes）
status: blocked
planning_depth: deep
planning_depth_reason: 旧三仓 freeze 语义与保护意图；跨 remote 策略与授权边界。
ready: false
risk: critical
blocked_by: [T-00]
contract_ids: [AC-010, AC-011, NAC-03]
owner: unassigned
expected_changes:
...
# Ticket T-16: Freeze Old Remotes Semantics（alias T04b / T-freeze-old-remotes）

- **Ticket 文件：** `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/ticket/16-freeze-old-remotes.md</Path>`
...
- **完成 Evidence：** `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/evidence/T-16.md</Path>`

实现本 Ticket 时，Lead 与 implementation subagent 必须按顺序完整读取总体 Map，读取项目 Skill 的 frontmatter 与入口并只展开适用于 `ALL`/`T-16` 的匹配项，再读取本 Ticket 与相关上游工件。Map 中的 Skill 是最低必读集合；新的匹配项先由 Lead 同步到 Map 并重新校验。
...
- 正式 id 为 T-16（alias T04b）。

### 已采用的低影响假设

- 保护规则变更需 legacy_repo_mutation_authorized。
...
|---|---|---|
| freeze 语义文档与意图 checklist。 | ADR-010 表。 | 旧仓 README/archive 落地（T-15）；产品 rename。 |

## 4. 要构建什么

记录 freeze 意图并对默认分支保护策略给出授权后执行步骤；不在本票做旧仓 mutation（除非仅文档）。

## 5. 实现契约

- **入口：** ADR-010 freeze 表。
- **输出：** freeze checklist Evidence。
- **不变量：** 无 rewrite/force-push。
- **失败：** 借 rename 改保护规则 → 停。
- **兼容要求：** 见发布/迁移节；未授权不实施。
...
1. 授权确认。
2. 列出三 remote freeze 含义。
3. 记录允许/禁止动作。
4. 与 T-04 W0b 对齐。
5. 验证无 remote mutation（无 legacy auth）。
...
|---|---|---|---|---|
| freeze 定义 | 清单 | 对照 ADR-010 | 一致 | evidence/T-16.md |
| 无 force-push | remote 状态 | 无 rewrite | 是 | evidence/T-16.md |
| 回归 | NAC-03 | 日志 | 无未授权 mutation | evidence/T-16.md |
...
- [ ] `AC-010`/`AC-011` freeze 语义可验收。
- [ ] alias T04b 仅标题，依赖用 T-16。
- [ ] 未 force-push/rewrite。
- [ ] 实现开始前已完整读取 Tickets Map，已读取项目 Skill 入口并完整展开适用于 `ALL`/`T-16` 的匹配项。
- [ ] 验证矩阵全部执行并记录到 `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/evidence/T-16.md</Path>`（完成时；**本轮不新建伪 Evidence**）。
...
依据 `<Path>{roots.workflows}/specdev/common/rules/skill-invocation.md</Path>`：本票 `skill_bindings` 绑定 Map 中适用于 ALL/`T-16` 的每一 Path。
