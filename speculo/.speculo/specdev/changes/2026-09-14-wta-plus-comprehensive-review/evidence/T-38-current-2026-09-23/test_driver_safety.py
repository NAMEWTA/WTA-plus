...
DRIVER = Path('/tmp/wta-t38/run-notify-manual-retry-integration.py')
spec = importlib.util.spec_from_file_location('t38_driver', DRIVER)
...
class DriverSafetyTest(unittest.TestCase):
    ...
    def test_owned_mysql_labels_loopback_and_trigger_option(self):
        ...
        self.assertIn('namewta.test.owner=T-38', args)
    ...
    def test_full_id_and_dual_label_discovery(self):
        ...
        with patch.object(driver, 'docker', return_value=full) as docker:
            ...
            self.assertEqual(docker.call_args.args, ('ps', '-aq', '--no-trunc',
                '--filter', 'label=namewta.test.owner=T-38', '--filter', 'label=namewta.test.run=run123'))
    ...
    def test_both_exact_fresh_classes_positive_zero_skip(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            paths = []
            for klass in (driver.TEST_CLASS, driver.WAKE_CLASS):
                ...
                paths.append(path)
            paths[1].write_text(f'<testsuite name="{driver.WAKE_CLASS}" tests="2" failures="0" errors="0" skipped="1"/>')
            with self.assertRaisesRegex(RuntimeError, 'zero tests'):
                driver.xml_verdict_for(driver.WAKE_CLASS, paths[1], started)
    ...
    def test_source_and_selector_are_t38_only(self):
        source = DRIVER.read_text()
        self.assertIn("selectors = 'NotifyManualRetryIntegrationTest,NotifyWakeIntegrationTest'", source)
        self.assertIn("'-Dnotify.manual.retry.integration=true'", source)
        self.assertIn("'-Dnotify.wake.integration=true'", source)
        self.assertIn("'-DforkCount=1', '-DreuseForks=false'", source)
        self.assertNotIn('NotifySmsDispatchIntegrationTest', source)
        self.assertNotIn('RedisNotifyIdempotencyStoreIntegrationTest', source)
    ...
    def test_password_only_child_environment(self):
        self.assertEqual(driver.safe_env(T38_MYSQL_PASSWORD='private')['T38_MYSQL_PASSWORD'], 'private')
        self.assertNotIn('T38_MYSQL_PASSWORD', driver.safe_env())
        self.assertEqual(driver.TEST_CLASS, 'org.namewta.test.notify.NotifyManualRetryIntegrationTest')
