#!/usr/bin/env python3
"""Opt-in, owned T-35 MySQL/Redis acceptance runner. Never run during preparation."""
...
RUN_ROOT = Path('/tmp/wta-t35/runs')
...
def owned_ids(run_id):
    found = docker('ps', '-aq', '--no-trunc', '--filter', 'label=namewta.test.owner=T-35',
...
def assert_owned(cid, run_id):
    actual = labels(cid)
    if actual.get('namewta.test.owner') != 'T-35' or actual.get('namewta.test.run') != run_id:
...
def xml_verdict(started_ns):
    if not TEST_REPORT.is_file() or TEST_REPORT.stat().st_mtime_ns < started_ns:
        raise RuntimeError('Target Surefire XML is absent or stale')
    suite = ET.parse(TEST_REPORT).getroot()
...
def main():
    ...
    report = {'gate': 'T-35 real DAO/Outbox/Redis with fake SMS sender', 'run_id': run_id,
    ...
    try:
        ...
        mysql_id = full_id(docker('run', '-d', '--name', 't35-mysql-' + run_id,
                                  '--label', 'namewta.test.owner=T-35',
        ...
        redis_id = full_id(docker('run', '-d', '--name', 't35-redis-' + run_id,
                                  '--label', 'namewta.test.owner=T-35',
                                  '--label', 'namewta.test.run=' + run_id,
        ...
        argv = ('./mvnw', '-Pdev', '-pl', 'wta-admin', '-am', 'test',
                '-Dtest=NotifySmsDispatchIntegrationTest', '-Dsurefire.failIfNoSpecifiedTests=false',
        ...
        report['counts'] = xml_verdict(started_ns)
        report['exit_code'] = 0
