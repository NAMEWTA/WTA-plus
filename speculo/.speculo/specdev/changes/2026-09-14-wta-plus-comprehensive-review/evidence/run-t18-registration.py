...
owned = Path(tempfile.mkdtemp(prefix='t18-registration-', dir=root / 'temp/team/lead'))
servers = []
threads = []
...
try:
    ...
    for app in ('admin', 'home'):
        ...
        env[f'REGISTRATION_{app.upper()}_ORIGIN'] = f'https://127.0.0.1:{server.server_port}'
    command = ['corepack', 'pnpm', 'exec', 'playwright', 'test', '--config', 'playwright.registration.config.ts', *sys.argv[2:]]
    ...
    row = dict(command=command, cwd='frontend', exit_code=result.returncode, seconds=round(time.time()-start, 2),
               log=str(log.relative_to(root)), owned_origins=[env[f'REGISTRATION_{app.upper()}_ORIGIN'] for app in ('admin','home')],
