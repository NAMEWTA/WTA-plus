...
Never points at deployment services. It removes only resource IDs created by this run.
"""
import argparse
...
def main():
    ...
    parser.add_argument('--playwright-config', default='frontend/e2e/playwright.inbox-real.config.ts')
    parser.add_argument('--playwright-test', default='frontend/e2e/inbox-real.e2e.ts')
    parser.add_argument('--grep', default=TEST_NAME)
    parser.add_argument('--expected-tests', type=int, default=1)
    ...
    try:
        ...
        config = (root / args.playwright_config).resolve(strict=True)
        test = (root / args.playwright_test).resolve(strict=True)
        ...
        report['artifact'] = {'jar': str(jar.relative_to(root)), 'sha256': jar_sha,
                              'playwright_config_sha256': sha(config), 'playwright_test_sha256': sha(test)}
        ...
        mysql = docker('run', '-d', '--name', 't34-mysql-' + run_id,
                       '--label', 'namewta.test.owner=T-34', '--label', label,
        ...
        cmd = ['node', str(pw), 'test', '--config', str(config),
               '--grep', args.grep, '--workers=1', '--reporter=json',
        ...
        try:
            ...
            raise RuntimeError('Playwright JSON reporter unavailable: ' + type(exc).__name__) from exc
        if playwright.returncode != 0 or report['counts'] != {'expected': args.expected_tests, 'unexpected': 0, 'skipped': 0, 'flaky': 0}:
        ...
        for proc in reversed(processes):
            try:
                ...
                report['cleanup']['process_error_' + str(proc.pid)] = type(exc).__name__
        for logfile in logs:
            logfile.close()
        for name in ('backend', 'vite'):
            ...
            if raw.exists():
                ...
                for value in (secret, monitor_secret, password):
        ...
        try:
            remaining = docker('ps', '-aq', '--filter', 'label=namewta.test.owner=T-34',
                               '--filter', 'label=namewta.test.run=' + run_id)
            report['cleanup']['owned_containers_remaining'] = remaining.splitlines() if remaining else []
        except Exception:
            report['cleanup']['owned_containers_remaining'] = 'unknown'
        ...
        try:
            ...
            report['source_after'] = {'error': str(exc), 'git_status': status,
                                      'tracked_diff_path': str(run / 'source-after.diff')}
        expected_source_after = ({k: report['source'][k] for k in ('head', 'tree', 'clean')}
                                 if 'source' in report else None)
        if (report['cleanup'].get('owned_containers_remaining') != [] or
            not all(report['cleanup']['ports_closed'].values()) or
            any(v != 'removed' for k, v in report['cleanup'].items() if k in containers) or
