...
ADR-CR-009推荐MySQL receipt唯一键；唯一身份与保留期需按实际供应商合同审核。
...
现有回调只有channel/providerKey/providerMessageId/eventId，缺少供应商账号与事件ID作用域、最大重试窗口证据。关闭条件：固定实际adapter及签名身份到供应商账号的映射、receipt复合唯一键和保留策略；无证据不凭空新增account字段或猜清理期限。
...
最小持久receipt唯一身份为明确供应商命名空间+eventId（多账号仅在实际eventId作用域需要时加入）。receipt、状态CAS与聚合同事务；同event不同payload拒绝。Worker尚未回写providerMessageId时不能先记已处理，应给供应商可重试失败；只有供应商合同证明不会重试才另评估持久inbox，不默认创建第二套消息系统。保留HMAC原文与时间窗。
...
- [ ] 正常/失败/回归及required E2E均完成，证据写入<Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/evidence/T-23.md</Path>，未执行不得标通过。
