#!/usr/bin/env python3
"""Serial local T-31 checks. No release promotion or external writes."""
import json
import os
...
mode = sys.argv[2]
shim = Path(tempfile.mkdtemp(prefix='t31-pnpm-', dir=root / 'temp/team/lead'))
...
if mode == 'frontend':
    ...
    commands = [('frontend', ['corepack', 'pnpm', 'exec', 'tsc', '--ignoreConfig', '--noEmit', '--strict', '--skipLibCheck', '--target', 'ES2023', '--module', 'ESNext', '--moduleResolution', 'Bundler', '--types', 'node', 'e2e/enterprise-transfer-queue.spec.ts', 'playwright.transfer.config.ts', 'playwright.config.ts']),
                ('frontend', ['corepack', 'pnpm', 'exec', 'oxlint', 'e2e/enterprise-transfer-queue.spec.ts', 'playwright.transfer.config.ts', 'playwright.config.ts']),
