...
DRIVER = Path('/tmp/wta-t36/run-notify-atomic-integration.py')
spec = importlib.util.spec_from_file_location('t36_driver', DRIVER)
driver = importlib.util.module_from_spec(spec)
spec.loader.exec_module(driver)


class DriverSafetyTest(unittest.TestCase):
    def test_owned_mysql_allows_seed_triggers_without_global_app_privilege(self):
        args = driver.mysql_run_args('run123', Path('/tmp/owned-mysql.env'))
        self.assertEqual(args[0:2], ('run', '-d'))
        self.assertIn('namewta.test.owner=T-36', args)
    ...
    def test_discovery_uses_both_labels_and_full_ids(self):
        ...
        with patch.object(driver, 'docker', return_value=full) as docker:
            ...
            self.assertEqual(docker.call_args.args, (
                'ps', '-aq', '--no-trunc', '--filter', 'label=namewta.test.owner=T-36',
