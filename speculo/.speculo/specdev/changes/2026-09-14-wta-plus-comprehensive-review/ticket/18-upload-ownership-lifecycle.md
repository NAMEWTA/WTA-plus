...
skill_bindings: [{"id": "engineering-standards", "path": "<Path>.agents/skills/engineering-standards/SKILL.md</Path>", "sha256": "dbc475149e3588cb840c15e0bb287920b876c0eef4c4004dde078de1c4fca6d9", "phase": "implement", "operation": "apply-scope-contract", "inputs": ["<Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/tickets-map.md</Path>", "<Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/ticket/18-upload-ownership-lifecycle.md</Path>", "<Path>frontend/packages/web-kit/file-upload/src/FileUpload.vue</Path>", "<Path>frontend/packages/web-kit/file-upload/src/ImageUpload.vue</Path>"], "outputs": ["T-18的架构/权限/数据边界检查与定向实现diff"], "required": true, "on_failure": "block-ticket"}, {"id": "namewta-fullstack-development", "path": "<Path>.agents/skills/namewta-fullstack-development/SKILL.md</Path>", "sha256": "675c053c11d8b22cd394c875f48688242d8e8328dd14e69657f5d5b12f2af68b", "phase": "implement", "operation": "apply-scope-contract", "inputs": ["<Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/tickets-map.md</Path>", "<Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/ticket/18-upload-ownership-lifecycle.md</Path>", "<Path>frontend/packages/web-kit/file-upload/src/FileUpload.vue</Path>", "<Path>frontend/packages/web-kit/file-upload/src/ImageUpload.vue</Path>"], "outputs": ["T-18的架构/权限/数据边界检查与定向实现diff"], "required": true, "on_failure": "block-ticket"}, {"id": "engineering-standards", "path": "<Path>.agents/skills/engineering-standards/SKILL.md</Path>", "sha256": "dbc475149e3588cb840c15e0bb287920b876c0eef4c4004dde078de1c4fca6d9", "phase": "verify", "operation": "verify-affected-contract-and-quality-gates", "inputs": ["<Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/ticket/18-upload-ownership-lifecycle.md</Path>", "current-workspace实际diff及本票验证矩阵"], "outputs": ["<Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/evidence/T-18.md</Path>：命令、退出码、测试数、AC与Skill Execution Records"], "required": true, "on_failure": "block-ticket"}]
resource_claims: ["workspace:current-exclusive", "finding:F-07", "finding:F-11", "contract:AC-018"]
artifact: ticket
...
- 目标与可观察产出：下载URL失败不生成无人回收的Blob URL，也不把已完成上传误报失败。
- 来源：F-07, F-11；AC-018；USER-DECISION: 全面完善计划、无兼容、单人串行。
- 当前事实与调用链：<Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/reviews/re-review.md</Path>中T-18行及对应专项报告；源码导航为frontmatter预计修改点。
- 规划深度：deep；安全/鉴权、公共合同、数据一致性或共享核心路径变更：明确上传完成、引用移除和导入失败生命周期。
...
| IN | REUSE | OUT |
|---|---|---|
| 上传完成→业务引用/预览；导入请求→成功或失败复位 | 现有upload runtime、服务端引用保护；无通用dispose接口 | 本票之外的模块重写、兼容桥、远程发布与重要数据操作 |

## 4. 要构建什么

上传完成→业务引用/预览；导入请求→成功或失败复位。调用者可观察到：下载URL失败不生成无人回收的Blob URL，也不把已完成上传误报失败。失败时：URL解析失败不变成上传失败；error/401/abort均清isUploading。

## 5. 实现契约

- 入口、输入输出与数据流：上传完成→业务引用/预览；导入请求→成功或失败复位。
- 不变量及失败语义：URL解析失败不变成上传失败；error/401/abort均清isUploading。
...
F-11确认的是下载URL失败后的Blob URL泄漏。现有上传成功不应因为预览失败变成失败或重复上传。避免另建system import框架；只在现有端口确实不能承载文件上传时增加最小方法。

## 6. 执行路线

1. 移除resolveUploadResult中无人持有的createObjectURL fallback；上传成功与预览地址获取失败分开表达。优先显示文件名/预览不可用并按需重新获取URL。
2. 如已有明确本地预览需求，由实际预览组件创建并在replace/remove/unmount释放URL；不为修泄漏新增通用dispose协议。
3. 用户导入在请求时获取headers，通过现有runtime/HTTP机制处理error、401与abort，所有路径结束isUploading。流程导入的http-request同样符合组件成功/失败回调。
4. 材料页在上传完成且归属登记后提交；保留现有OSS管理删除及服务端引用保护。T-14的新self调用负责detach，不将未证实误删推广成全局UploadClient改造。
...
|---|---|---|---|
| 正常路径 | 上传完成→业务引用/预览；导入请求→成功或失败复位；执行下列定向命令及对应场景 | 下载URL失败不生成无人回收的Blob URL，也不把已完成上传误报失败 | <Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/evidence/T-18.md</Path> |
| 失败路径 | URL解析失败不变成上传失败；error/401/abort均清isUploading；固定时序/故障注入，记录输入与最终可观察状态 | 无越权、错误状态或部分提交；可按定义恢复 | <Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/evidence/T-18.md</Path> |
| 回归 | 运行所属包原有测试及受影响调用者；逐项核对下列AC | 导入网络失败、业务错误、401和取消均复位且可重试；业务提交等待上传与材料归属登记完成；现有OSS删除权限/引用保护不退化；若保留本地URL则replace/remove/unmount释放 | <Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/evidence/T-18.md</Path> |
...
必需入口缺失或sha256漂移时阻塞本票；Lead回读差异后更新绑定及Map，不能自动接受新摘要。实际记录归<Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/evidence/T-18.md</Path>的`## Skill Execution Records`，本次规划不伪造实现调用记录。
