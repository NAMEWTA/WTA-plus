...
owned = Path(tempfile.mkdtemp(prefix='t30-browser-groups-', dir=root/'temp/team/lead'))
apps = ['admin', 'home', 'sso']
...
env.update(npm_config_workspace_concurrency='1', RAYON_NUM_THREADS='1',
           VITE_APP_BASE_API='/prod-api', VITE_APP_MESSAGE_ENABLED='true',
...
try:
    ...
    for app in apps:
        run(['corepack', 'pnpm', '--filter', f'@namewta/{app}-web', 'build:prod'],
            f'{name}-build-{app}', {'VITE_APP_CONTEXT_PATH': '/'})
    for group in ('lifecycle', 'registration', 'accessibility', 'transfer'):
        args = ['--tests'] if group == 'accessibility' else []
        run([sys.executable, str(evidence/('run-t30-'+group+'.py')), name+'-'+group, *args], name+'-'+group+'-driver')
    code = 0
