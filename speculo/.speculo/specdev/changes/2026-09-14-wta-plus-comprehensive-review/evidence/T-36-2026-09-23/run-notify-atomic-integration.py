#!/usr/bin/env python3
"""Opt-in, owned T-36 MySQL/Redis acceptance runner. Never run during preparation."""
...
RUN_ROOT = Path('/tmp/wta-t36/runs')
...
TEST_CLASS = 'org.namewta.test.notify.NotifyAtomicResultIntegrationTest'
TEST_SOURCE = ROOT / 'backend/wta-admin/src/test/java/org/namewta/test/notify/NotifyAtomicResultIntegrationTest.java'
...
RED_METHOD = 'attemptInsertFailureRollsBackDeliveryAndLeavesClaimRecoverable'
...
def source_identity(expected, *, development_red=False):
    head = git('rev-parse', 'HEAD')
    dirty = git('status', '--porcelain', '--untracked-files=all')
    if head != expected or (not development_red and dirty) or (development_red and not dirty):
        raise RuntimeError('Source does not match expected HEAD and mode-specific dirty state')
    identity = {'head': head, 'tree': git('rev-parse', 'HEAD^{tree}'), 'clean': not bool(dirty)}
    if development_red:
        diff = run(('git', 'diff', '--binary', 'HEAD'), timeout=30)
        if diff.returncode:
            raise RuntimeError('Dirty source diff inspection failed')
        identity['dirty_status_sha256'] = hashlib.sha256(dirty.encode()).hexdigest()
        identity['dirty_tracked_diff_sha256'] = hashlib.sha256(diff.stdout).hexdigest()
        identity['target_test_sha256'] = hash_file(TEST_SOURCE)
    return identity
...
def owned_ids(run_id):
    found = docker('ps', '-aq', '--no-trunc', '--filter', 'label=namewta.test.owner=T-36',
...
def assert_owned(cid, run_id):
    actual = labels(cid)
    if actual.get('namewta.test.owner') != 'T-36' or actual.get('namewta.test.run') != run_id:
...
def mysql_run_args(run_id, mysql_env):
    return ('run', '-d', '--name', 't36-mysql-' + run_id,
            '--label', 'namewta.test.owner=T-36',
...
def main():
    ...
    parser.add_argument('--development-red', action='store_true',
                        help='Dirty-source exact one-method red proof; never an acceptance verdict')
    ...
    report = {'gate': 'T-36 real DAO/Outbox/Redis with IN_APP wake', 'run_id': run_id,
              'mode': 'development-red' if args.development_red else 'acceptance',
              'acceptance': False,
    ...
    maven_proc = None
    try:
        report['source_before'] = source_identity(args.expected_head, development_red=args.development_red)
        test_text = TEST_SOURCE.read_text()
        if 'T36_MYSQL_PASSWORD' not in test_text:
            raise RuntimeError('Target test must read T36_MYSQL_PASSWORD from process environment')
        if not args.development_red and 'notify.wake.integration' not in WAKE_SOURCE.read_text():
            raise RuntimeError('Wake test must retain its explicit integration property gate')
        ...
        write_secret(test_env, 'T36_MYSQL_PASSWORD=' + app_password + '\n')
        ...
        redis_id = full_id(docker('run', '-d', '--name', 't36-redis-' + run_id,
                                  '--label', 'namewta.test.owner=T-36',
                                  '--label', 'namewta.test.run=' + run_id,
        ...
        selectors = ('NotifyAtomicResultIntegrationTest#' + RED_METHOD if args.development_red
                     else 'NotifyAtomicResultIntegrationTest,NotifyWakeIntegrationTest')
        argv = ('./mvnw', '-Pdev', '-pl', 'wta-admin', '-am', 'test',
                '-Dtest=' + selectors, '-Dsurefire.failIfNoSpecifiedTests=false',
                '-DforkCount=1', '-DreuseForks=false',
                '-Dnotify.atomic.integration=true', '-Dnotify.mysql.integration.url=' + jdbc,
                '-Dnotify.mysql.integration.username=' + app_user,
                '-Dnotify.redis.integration.port=' + str(redis_port)) \
            + (() if args.development_red else ('-Dnotify.wake.integration=true',))
        ...
        with os.fdopen(os.open(raw_log, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600), 'wb') as stream:
            maven_proc = subprocess.Popen(argv, cwd=ROOT / 'backend', stdout=stream,
                                          ...
                                          env=safe_env(T36_MYSQL_PASSWORD=app_password))
            try:
        ...
        # Save fresh exact-class counts even when Maven failed, so the red verdict is auditable.
        report['counts'] = {TEST_CLASS: xml_counts_for(TEST_CLASS, TEST_REPORT, started_ns)}
        report['methods'] = {TEST_CLASS: xml_methods_for(TEST_REPORT)}
        if args.development_red:
            counts = report['counts'][TEST_CLASS]
            if not (report['maven_exit_code'] != 0 and counts['tests'] == 1
                    and counts['failures'] == 1 and counts['errors'] == 0 and counts['skipped'] == 0
                    and report['methods'][TEST_CLASS] == [RED_METHOD]):
                raise RuntimeError('Expected exact one-method failing Surefire XML was not observed')
            report['development_red_observed'] = True
        else:
            report['counts'][WAKE_CLASS] = xml_counts_for(WAKE_CLASS, WAKE_REPORT, started_ns)
            report['methods'][WAKE_CLASS] = xml_methods_for(WAKE_REPORT)
        ...
        try:
            report['source_after'] = source_identity(args.expected_head, development_red=args.development_red)
        ...
        report['acceptance'] = bool(not args.development_red and report['exit_code'] == 0)
