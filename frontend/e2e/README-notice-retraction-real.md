...
`run-notice-retraction-real.py` 使用独占、随机 loopback MySQL/Redis/MinIO 和完整 Admin JAR。它先以独立控制账号经真实 HTTP 保存、发布公告，等待真实 Worker 为普通用户 A 送达 V1，再撤回该版本；随后仅为分页和负例插入显式合成消息。两例 Chrome 用例验证 A 的离页本人详情、旧管理链接安全转换，以及 B 对外人/不存在消息的一致拒绝。合成行不能代替真实 V1 发布证据。
