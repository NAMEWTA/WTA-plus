...
shared_path_owners: ["<Path>backend/wta-common/wta-common-web/</Path> => single-agent (Lead; exclusive current workspace; T-03 turn only)", "<Path>backend/wta-common/wta-common-openapi/</Path> => single-agent (Lead; exclusive current workspace; T-03 turn only)", "<Path>backend/wta-common/wta-common-encrypt/</Path> => single-agent (Lead; exclusive current workspace; T-03 turn only)", "<Path>backend/wta-admin/src/main/resources/application.yml</Path> => single-agent (Lead; exclusive current workspace; T-03 turn only)", "<Path>backend/wta-common/wta-common-core/src/</Path> => single-agent (Lead; exclusive current workspace; T-03 turn only)", "<Path>backend/wta-admin/src/test/java/org/namewta/test/requestbody/</Path> => single-agent (Lead; exclusive current workspace; T-03 turn only)", "<Path>backend/wta-admin/src/test/java/org/namewta/test/openapi/</Path> => single-agent (Lead; exclusive current workspace; T-03 turn only)", "<Path>backend/README.md</Path> => single-agent (Lead; exclusive current workspace; T-03 turn only)", "<Path>backend/wta-admin/src/test/java/org/namewta/test/security/BrowserHttpsTransportIntegrationTest.java</Path> => single-agent (Lead; exclusive current workspace; T-03 turn only)", "<Path>backend/wta-admin/src/test/java/org/namewta/test/logging/LogRedactionHttpMySqlIntegrationTest.java</Path> => single-agent (Lead; exclusive current workspace; T-03 turn only)"]
---
...
- 来源：R-02；AC-003；本轮用户请求与<Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/source.md</Path>。
- 当前事实：2MiB用户决定仍有效；机器验签/验证码接口在新候选继续回归。
...
既有合同保持；本轮重新评审与验收；2MiB用户决定仍有效；机器验签/验证码接口在新候选继续回归。 完整决定及来源以<Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/ADR.md</Path>为准。
...
大小边界前/等于/超限一字节结果可判定 正常、失败、越权和竞争路径按本票验收断言共同交付，不把前后端或测试分成无价值空票。

## 5. 实现契约

- 入口与输入输出：required: 通过真实HTTP发送定长/chunked边界请求、伪签名大正文及正常签名正文。
- 外部行为：大小边界前/等于/超限一字节结果可判定
...
|---|---|---|---|
| 正常 | required: 通过真实HTTP发送定长/chunked边界请求、伪签名大正文及正常签名正文 | 大小边界前/等于/超限一字节结果可判定 | <Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/evidence/T-03-replan-2026-09-23.md</Path> |
| 失败/竞争 | 未知长度无法绕过限制，413时业务尚未执行 | 明确失败/安全恢复，无伪成功、越权及部分提交 | 同上，记录故障注入与状态 |
| 回归 | 现有同域测试＋消费者＋适用静态门禁 | OpenAPI签名测试逐字节通过；同一正文不会因观察者线性增加完整副本，SSE/上传可取消 | 同上，记录测试数/skip/源码 |

命令在仓根执行，`cd backend`表示该条命令切cwd；每条独立运行。以下为实施期命令，本轮未执行：

- `cd backend && ./mvnw -pl wta-common/wta-common-web,wta-common/wta-common-openapi,wta-common/wta-common-encrypt -am test`
...
历史implementation commit：`214de538267a60fa527b1213f7dd9139622f4a2e`；历史result：`6c8764cca97bb6057fcb90ccdfe635c7efbf502a`。提交存在及祖先关系见<Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/reviews/legacy-ticket-audit.json</Path>。
