#!/usr/bin/env python3
"""Serial local T-19 checks. No release promotion or external writes."""
import json
import os
...
name = sys.argv[1]
assert name.startswith('T-19-') and '/' not in name
mode = sys.argv[2]
shim = Path(tempfile.mkdtemp(prefix='t19-pnpm-', dir=root / 'temp/team/lead'))
...
if mode == 'query-tests':
    commands = [('frontend', ['corepack', 'pnpm', '--filter', '@namewta/web-domain-system', 'exec', 'vitest', 'run', 'src/user/page-query.test.ts'])]
elif mode == 'workflow-focused':
    commands = [('frontend', ['corepack', 'pnpm', '--filter', '@namewta/web-domain-system', check]) for check in ['typecheck', 'test', 'lint']]
elif mode == 'workflow-unit':
    commands = [('frontend', ['corepack', 'pnpm', '--filter', '@namewta/web-domain-system', 'test'])]
elif mode == 'workflow-backend':
    commands = [('backend', ['./mvnw', '-B', '-ntp', '-pl', 'wta-modules/wta-workflow,wta-admin', '-am', 'test'])]
elif mode == 'workflow-static':
    commands = [('frontend', ['corepack', 'pnpm', 'exec', 'tsc', '--ignoreConfig', '--noEmit', '--strict', '--skipLibCheck', '--target', 'ES2023', '--module', 'ESNext', '--moduleResolution', 'Bundler', '--types', 'node', 'e2e/workflow-definition.spec.ts', 'playwright.config.ts']),
                ('frontend', ['corepack', 'pnpm', 'exec', 'oxlint', 'e2e/workflow-definition.spec.ts', 'playwright.config.ts']),
                ('.', ['node', '.agents/skills/namewta-fullstack-development/scripts/validate-module-mode.mjs', 'backend/wta-modules/wta-workflow', '--mode', 'classic']),
                ('.', ['node', '.agents/skills/engineering-standards/scripts/validate-skill-facts.mjs']),
                ('.', ['git', 'diff', '--check'])]
elif mode == 'identity':
    commands = [('backend', ['./mvnw', '-B', '-ntp', '-pl', 'wta-modules/wta-profile/wta-profile-person,wta-modules/wta-profile/wta-profile-enterprise', '-am', '-Dtest=ExplicitProfileIdentityContractTest', '-Dsurefire.failIfNoSpecifiedTests=false', 'test'])]
elif mode == 'consumers':
    commands = [('backend', ['./mvnw', '-B', '-ntp', '-pl', 'wta-modules/wta-profile,wta-admin', '-am', 'test'])]
    ...
    commands = [('backend', ['./mvnw', '-B', '-ntp', '-pl', 'wta-modules/wta-profile,wta-modules/wta-profile/wta-profile-person,wta-modules/wta-profile/wta-profile-enterprise', '-am', 'clean', 'package'])]
elif mode == 'backend-static':
    commands = [('.', ['node', '.agents/skills/namewta-fullstack-development/scripts/validate-module-mode.mjs', 'backend/wta-modules/wta-profile/wta-profile-person', '--mode', 'layered']),
