...
- 本轮不使用仓库披露的local连接参数，也不创建外部资源。若缺Docker/浏览器或专用测试条件，明确阻塞对应required验收，不宣称通过；唯一例外为T48 Windows实机运行，按用户明确豁免记录user-waived/not-run。
