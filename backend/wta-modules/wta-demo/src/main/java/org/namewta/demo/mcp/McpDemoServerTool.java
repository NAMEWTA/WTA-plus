 ...
 * <p>
 * 启动 `wta-admin` 后，本类中的 `@McpTool` 与 `@McpResource` 会被 Spring AI
...
public class McpDemoServerTool {
    ...
    public McpDemoData getData(@McpToolParam(description = "业务主键") String id) {
        return new McpDemoData(id, "演示数据-" + id, "wta-demo", 100, LocalDateTime.now());
    }
    ...
    public List<McpDemoData> listData(@McpToolParam(description = "返回条数") Integer limit) {
        ...
        return java.util.stream.IntStream.rangeClosed(1, size)
            .mapToObj(index -> new McpDemoData(String.valueOf(index), "演示数据-" + index, "wta-demo", index * 10, LocalDateTime.now()))
            .toList();
    }
    ...
    public String summary() {
        return "wta-demo MCP resource summary";
    }
