...
已实现公告发布版本的持久撤回栅栏、事务内精确身份/版本校验、Worker与手工重试执行权边界；撤回停止尚未获得发送权的任务，保留已送达快照和已发生外部结果。发布链接统一指向本人收件箱消息，历史管理链接按本人messageId转换。前端详情采用本人授权和query/session代次，旧响应不覆盖新详情或新身份。完整服务验证暴露的缺User-Agent和大整数JSON表示缺陷也已窄修；认证/Client/权限与公开Java签名保持原合同。

## 2. Lead Dispatch And Candidate Return

cors_audit唯一产品writer，Lead负责治理、提交和全部隔离服务；legacy_audit/ops_audit只读审查。各次红灯、分段反馈、三个失败批次及四项复盘均保留于下方历史及各manifest。Dispatch04的U1为首个完整通过候选，不追认任何先前失败。

## 3. 修改范围与路径所有权

源26f04b94db68701ade80038a763eca0ffde83918，父分支main，基线601b927313e0dfae16e75a848a97d9f06afb69bb；38条产品路径全部被Ticket27条精确/目录写集覆盖，base为result祖先。`complete-candidate-u1/gates/u1-write-scope-and-ancestry.json`保存实际diff与祖先核查。持久化仍在Notify DAO；System存量classic与common依赖方向未改变，未新增SQL/权限/HTTP schema或wta-api签名。源码与命中Skill输入hash均核对。

## 4. 验收与合同映射

- AC-040撤回先获锁/发送权：R2真实18例证明双JDBC连接锁等待、撤回后零新外呼/站内写入、重复撤回与版本隔离、重发布正常、晚期SQL回滚、旧lease拒绝。
- AC-040外部已发生结果及已送达快照保留：类型化Provider替身ACCEPTED/UNKNOWN/UNSENT_TERMINAL结果在真实DB落库；完整Spring HTTP真实发布→Worker IN_APP送达→撤回→A从分页外读取原正文通过。
- AC-040本人权限：Chrome A同页迟到成功保持新详情、历史链接不访问管理GET、关闭pending模态后真实注销，精确旧Request ERR_ABORTED且无Response，B本人正文保持且无A正文；独立B正控/外人/不存在一致拒绝、非法ID零详情请求通过。控制HTTP验证A/B管理Notice GET均拒绝。

## 5. Workspace Verification

Workspace: current-workspace；单一工作区、Lead独占集成与服务，无source-worktree分支。

U1 clean HEAD/tree 26f04b94db68701ade80038a763eca0ffde83918 / ad6f9ed73ff4c437741372149670d824327ec98a；新Python26、strict E2E tsc、architecture/OpenAPI/lint/typecheck、652Vitest+108Node和3App production build均exit0，329产物哈希保留；额外architecture:test为101项重复独立门禁，不叠加为新业务用例。fresh full package/bundle exit0，JAR SHA efafa17218f8b4ccdf964a21751179cbde0505e2162d1d52b0e6080cc58087e1。

后端Java/SQL/POM/配置/依赖/Skill自R2 8a387192字节不变，`u1-source-equivalence.json`明确复用R2 default261类1105项（889pass、216环境skip）、真实18与共享135（各0fail/error/skip）、core package/bundle和Skill/模块/手册静态门禁。默认环境skip不计真实验证，18/135原始fresh XML及坐标仍在complete-candidate-r2。不是声称这些门禁在U1重新运行。

## 6. 双轴审查

实现/安全轴：最终独立合同报告逐项核AC、撤回边界/真实数据库和供应商替身差别；U1模态/Request对象/取消/安全annotation审查无阻断。工程轴：写集38/38、祖先/clean source、Skill输入hash、层次与生成合同、全前端及full构建通过；不改lint规则或总超时换绿。报告见complete-candidate-u1/reviews。

## 7. Integration Verification

method=direct-parent；parent_before=601b927313e0dfae16e75a848a97d9f06afb69bb；source/result=26f04b94db68701ade80038a763eca0ffde83918；base祖先检查通过，无worktree/merge/push。owned browser run dd86442542876baf acceptance=true/exit0，Chrome2各一次、零skip/flaky，source前后clean同值、JAR同值。seed前后计数/离页位置与真实V1撤回栅栏稳定；3进程组/3容器/2匿名卷/5端口清理且errors=[]。原始安全报告、命令/时间/退出码、产物摘要和哈希清单见complete-candidate-u1/manifest.json。

## 8. 偏差与决策

三批各3次失败及对应复盘按原时点保留。完整服务所需Unknown UA元数据和canonical正整数JSON字符串修复均在事前登记的路径内；没有改认证、授权或允许不匹配身份。测试合成消息时间取精确真实V1关系+1秒，不修改真实V1；跨会话验收按产品真实取消合同，保留同页迟到成功及SFC epoch单测。阶段摘要只表示last_started_phase，不冒称成功；原始敏感report/log已清理。

## 9. 残余风险与交付定位

T40 done不代表整个change完成。外部供应商为测试替身，未实际发送SMTP/SMS；完整生产启动/HTTP/Chrome链路验证IN_APP，不声称验证供应商可追回。没有推送、部署、生产数据修复或归档。当前14done/2cancelled/34ready，下一T44；历史取消票AC001/003与所有票最终统一候选仍由T30负责。

## Skill Execution Records

输入为本Ticket、38路径diff、真实消费者和固定源码；执行了Notify层次/DAO/事务/本人授权检查、App拥有Router与会话代次、common UA消费者/JSON配置核查、独立双轴审查及实际质量门禁。输出为本Evidence §§3–9与complete-candidate-u1的命令/产物/来源/合同报告；各binding入口hash与当前文件一致。
...
[
  {
    ...
    "status": "passed",
    "evidence": [
      "<Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/evidence/T-40.md</Path> §§3–9 and complete-candidate-u1 manifest; exact scope/ancestry and independent contract/browser reviews"
    ]
  },
  {
