...
def count_sql(base, cid, sql, stage):
    ...
    if completed.returncode:
        raise RuntimeError('owned SQL failed at ' + stage)
...
def one(base, cid, sql, stage):
    ...
    if len(rows) != 1 or not rows[0].isdigit():
        raise RuntimeError('owned SQL cardinality failed at ' + stage)
    return int(rows[0])
...
def main():
    ...
    try:
        ...
        sql = ("UPDATE sys_oss_config SET access_key=" + h(access) + ",secret_key=" + h(secret)
            ...
            + ",domain_url=" + h('') + ",is_https='N',access_policy='0',status='Y'"
        ...
        for _ in range(20):
            audit = one(base, mysql_id,
                ...
                + h('/resource/oss/config/diagnose/%')
        ...
        for canary in (access, secret, minio_user, minio_pass):
            leaked = one(base, mysql_id,
                'SELECT COUNT(*) FROM sys_oper_log WHERE oper_url LIKE '
                + h('/resource/oss/config/diagnose/%')
                + ' AND (COALESCE(oper_param,\'\') LIKE CONCAT(\'%\',' + h(canary) + ',\'%\')'
                + ' OR COALESCE(json_result,\'\') LIKE CONCAT(\'%\',' + h(canary) + ',\'%\')'
                + ' OR COALESCE(error_msg,\'\') LIKE CONCAT(\'%\',' + h(canary) + ',\'%\'));',
        ...
        if any(secret_value.encode() in raw_backend for secret_value in
               (root_pass, app_pass, redis_pass, minio_user, minio_pass,
                access, secret, control_pass, ordinary_pass)):
            raise RuntimeError('owned backend raw log contains credential material')
        report['counts']['raw_backend_secret_canaries'] = 0
