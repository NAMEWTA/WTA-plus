...
skill_scan: "scanned project Skill roots under <Path>.agents/skills/*/SKILL.md</Path> (engineering-standards, namewta-fullstack-development, ruoyi-common-modules-guide, ruoyi-module-guide, deploy-namewta-environment, java-api-compatibility, project-customization-delivery, upstream-fork-sync); excluded last three as non-axis for rename/W0b/publication; bound for T-10: engineering-standards, namewta-fullstack-development"
skill_bindings:
  - {"id":"engineering-standards","path":"<Path>.agents/skills/engineering-standards/SKILL.md</Path>","sha256":"acac7d1d4341d3361c298ebc5a61841d794cafbe9f235171b56fddd7ecb0037d","phase":"verify","operation":"build-and-keep-regression","inputs":["Ticket T-10","Tickets Map","Spec/ADR projections"],"outputs":["Evidence notes for T-10","next-step inputs"],"required":true,"on_failure":"block-ticket","references":[]}
  - {"id":"namewta-fullstack-development","path":"<Path>.agents/skills/namewta-fullstack-development/SKILL.md</Path>","sha256":"e17e31173638b06f93bb2c26cf3315f50a1a075b3e298434c98fc47d175d7792","phase":"verify","operation":"build-and-keep-regression","inputs":["Ticket T-10","Tickets Map","Spec/ADR projections"],"outputs":["Evidence notes for T-10","next-step inputs"],"required":true,"on_failure":"block-ticket","references":[]}
resource_claims:
  - "build-keep-verification"
artifact: ticket
change: 2026-09-12-rename-ruoyi-dromara-to-wta
id: T-10
title: Static/Build + KEEP 回归验证
status: blocked
planning_depth: standard
planning_depth_reason: 构建与 KEEP 回归总闸。
ready: false
risk: high
blocked_by: [T-09]
contract_ids: [AC-002, AC-003, AC-001, AC-004]
owner: unassigned
expected_changes:
...
# Ticket T-10: Static/Build + KEEP 回归验证

- **Ticket 文件：** `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/ticket/10-build-keep-verify.md</Path>`
...
- **完成 Evidence：** `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/evidence/T-10.md</Path>`

实现本 Ticket 时，Lead 与 implementation subagent 必须按顺序完整读取总体 Map，读取项目 Skill 的 frontmatter 与入口并只展开适用于 `ALL`/`T-10` 的匹配项，再读取本 Ticket 与相关上游工件。Map 中的 Skill 是最低必读集合；新的匹配项先由 Lead 同步到 Map 并重新校验。
...
- **目标：** 跑后端/前端适用构建与 KEEP rg，证明 rename 波次可合并。
- **可观察产出：** build logs + KEEP 回归绿灯（非伪绿）。
- **来源：** AC-KEEP/NAME/PACKAGE/PREFIX。
- **当前事实：** 验证命令以票内矩阵为准。
- **Planning Depth 原因：** 构建与 KEEP 回归总闸。
...
|---|---|---|
| 静态/构建/KEEP 验证证据。 | T-05…T-09 产出。 | E2E（T-11）；public push。 |

## 4. 要构建什么

执行 mvn/pnpm/rg 并记录退出码。
...
|---|---|---|---|---|
| 后端构建 | mvn | exit 0 | 是 | evidence/T-10.md |
| 前端 | pnpm architecture:check | exit 0 | 是 | evidence/T-10.md |
| KEEP | rg | 四件套保留 | 是 | evidence/T-10.md |
| 非 zero-match | 文案 | 无清零 AC | 是 | evidence/T-10.md |
...
- [ ] 构建+KEEP 通过。
- [ ] 无伪绿。
- [ ] 实现开始前已完整读取 Tickets Map，已读取项目 Skill 入口并完整展开适用于 `ALL`/`T-10` 的匹配项。
- [ ] 验证矩阵全部执行并记录到 `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/evidence/T-10.md</Path>`（完成时；**本轮不新建伪 Evidence**）。
...
依据 `<Path>{roots.workflows}/specdev/common/rules/skill-invocation.md</Path>`：本票 `skill_bindings` 绑定 Map 中适用于 ALL/`T-10` 的每一 Path。
