#!/usr/bin/env python3
"""One exact OSS migration integration class on wholly owned MySQL and MinIO.

Inert without --execute and an exact clean 40-character source SHA. This
runner never uses an existing WTA database, MinIO identity or bucket.
"""
...
RUN_ROOT = Path('/tmp/wta-t46/oss-migration-runs')
...
EXPECTED_METHODS = (
    'migratesWithProductionStoreAndDualBucketsThenCleansUpOrRollsBack',
    'restoreAndCleanupSerializeOnRealObjectRowAndPreserveCurrentSource',
)
PROPERTY_KEYS = (
    't46.owned.run', 'oss.minio.integration.endpoint',
...
PHASES = ('preflight', 'mysql', 'minio', 'maven', 'reports', 'cleanup')
...
def load_base():
    ...
    spec = importlib.util.spec_from_file_location('t46_oss_migration_base', BASE_PATH)
...
def safe_xml_summary(name, raw, expected_run):
    ...
    marker = None if props is None else next(
        (item.attrib.get('value') for item in props.findall('property')
         if item.attrib.get('name') == 't46.owned.run'), None)
...
def parse_expected_methods(value):
    ...
    if methods != EXPECTED_METHODS:
        raise ValueError('Expected methods must match the two frozen T46 test methods exactly')
    return methods
...
def retain_fresh_reports(started_ns, run_dir, generated, run_id):
    ...
    for name in SUITES:
        ...
        try:
            ...
            temp = path.with_name(path.name + '.t46-' + run_id + '.tmp')
...
def retain_fresh_text_and_temp(started_ns, run_dir, generated):
    ...
    for name in SUITES:
        ...
        temp = path.with_name(path.name + '.t46-owned.tmp')
        private(temp, clean)
        os.replace(temp, path)
...
def main():
    ...
    parser.add_argument('--expected-methods', help='Exact two frozen JUnit method names, comma-separated')
    args = parser.parse_args()
    if not args.execute or not args.expected_head or not re.fullmatch(r'[0-9a-f]{40}', args.expected_head) or not args.expected_methods:
        parser.error('requires --execute --expected-head <exact clean 40-hex SHA> --expected-methods <old,new,...>')
    try:
    ...
    phase = 'preflight'
    cleanup_errors = []
    report = {'gate': 'T46 exact OSS migration integration class', 'run_id': run_id,
    ...
    try:
        ...
        for name in SUITES:
            ...
            if any(not re.search(r'\bvoid\s+' + re.escape(method) + r'\s*\(', text)
                   for method in expected_methods):
                raise RuntimeError('An exact expected integration method is missing')
        ...
        minio_user = 't46oss' + run_id
        migration_user = 't46_' + run_id
        migration_db = 't46_oss_migration_' + run_id
        ...
        suite_ok = exact_suite_gate(suites, expected_methods)
