#!/usr/bin/env python3
"""Opt-in T-41 full-JAR live /v3/api-docs capture on owned services only."""
...
RUN_ROOT = Path('/tmp/wta-t41/openapi-live')
...
def validate_inbox_migration(live, current, missing_schemas):
    """Allow only the observed T-41 one-use array envelope replacement, never other losses."""
    schemas = live['components']['schemas']
    paths = live['paths']
    def response_ref(path):
        return paths.get(path, {}).get('get', {}).get('responses', {}).get('200', {}).get('content', {}).get('*/*', {}).get('schema', {}).get('$ref')
    if response_ref('/notify/inbox') != '#/components/schemas/RNotifyInboxPageVo':
        raise RuntimeError('Live inbox GET is not the paged response envelope')
    if response_ref('/notify/inbox/{messageId}') != '#/components/schemas/RNotifyInboxMessageVo':
        raise RuntimeError('Live owned inbox detail GET is absent or wrongly typed')
    if schemas.get('RNotifyInboxPageVo', {}).get('properties', {}).get('data', {}).get('$ref') != '#/components/schemas/NotifyInboxPageVo':
        raise RuntimeError('Live inbox envelope does not reference the actual page')
    if schemas.get('RNotifyInboxMessageVo', {}).get('properties', {}).get('data', {}).get('$ref') != '#/components/schemas/NotifyInboxMessageVo':
        raise RuntimeError('Live detail envelope does not reference the actual message')
    page = schemas.get('NotifyInboxPageVo', {}).get('properties', {})
    if (page.get('rows', {}).get('type') != 'array'
        or page.get('rows', {}).get('items', {}).get('$ref') != '#/components/schemas/NotifyInboxMessageVo'
        or any(page.get(key, {}).get('type') != 'integer' for key in ('total', 'unreadTotal'))):
        raise RuntimeError('Live inbox page lacks typed rows/total/unreadTotal')
    parameters = {item.get('name'): item for item in paths['/notify/inbox']['get'].get('parameters', [])}
    if any(parameters.get(key, {}).get('in') != 'query'
           or parameters.get(key, {}).get('schema', {}).get('type') != 'integer'
           for key in ('pageNum', 'pageSize')):
        raise RuntimeError('Live inbox GET lacks integer page query parameters')
    retired = 'RListNotifyInboxMessageVo'
    replacements = []
    if retired in missing_schemas:
        needle = '#/components/schemas/' + retired
        references = []
        def walk(value, path=()):
            if isinstance(value, dict):
                for key, child in value.items():
                    if key == '$ref' and child == needle:
        ...
        walk(current)
        expected = ('paths', '/notify/inbox', 'get', 'responses', '200', 'content', '*/*', 'schema', '$ref')
        old_data = current.get('components', {}).get('schemas', {}).get(retired, {}).get('properties', {}).get('data', {})
        if references != [expected] or old_data.get('type') != 'array' or old_data.get('items', {}).get('$ref') != '#/components/schemas/NotifyInboxMessageVo':
            raise RuntimeError('Retired array schema has another consumer or unexpected shape')
        replacements.append({'retired_schema': retired, 'replacement_schema': 'RNotifyInboxPageVo',
                             'sole_old_consumer': '/notify/inbox GET 200',
                             'new_detail_schema': 'RNotifyInboxMessageVo'})
    unapproved = [name for name in missing_schemas if name != retired]
    return replacements, unapproved
...
def validate_full_openapi(body, current):
    ...
    replacements, unapproved_schemas = validate_inbox_migration(live, current, missing_schemas)
    if missing_paths or missing_operations or unapproved_schemas:
    ...
    return {'openapi': live['openapi'], 'paths': len(paths), 'schemas': len(schemas),
            ...
            'missing_paths': 0, 'missing_operations': 0, 'missing_schemas': len(missing_schemas),
            'unapproved_missing_schemas': 0, 'intentional_schema_replacements': replacements,
            'inbox_contract': 'typed page query + rows/total/unreadTotal + owned detail',
...
def main():
    ...
    try:
        ...
        mysql_id = helper.full_id(helper.docker(
            'run', '--pull=never', '-d', '--name', 't41-openapi-mysql-' + run_id,
        ...
        redis_id = helper.full_id(helper.docker(
            'run', '--pull=never', '-d', '--name', 't41-openapi-redis-' + run_id,
            '--label', 'namewta.test.owner=' + OWNER, '--label', 'namewta.test.run=' + run_id,
        ...
        minio_id = helper.full_id(helper.docker(
            'run', '--pull=never', '-d', '--name', 't41-openapi-minio-' + run_id,
            '--label', 'namewta.test.owner=' + OWNER, '--label', 'namewta.test.run=' + run_id,
