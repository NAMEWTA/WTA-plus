...
def owned_login(port, username, password, report, role):
    request = {'username': username, 'password': password,
               'clientId': ADMIN_CLIENT_ID, 'grantType': 'password'}
    status, response = owned_http(port, 'POST', '/auth/login', None, request)
    report.setdefault('http_login', {})[role] = safe_response(status, response)
...
def real_http_attachment_checks(port, cid, proxy, proxy_port, run_id, report, redactions, aux):
    report['http_progress'] = 'owned_login_prerequisites'
    ...
    report['http_progress'] = 'captcha_configuration'
    status, captcha = owned_http(port, 'GET', '/auth/code')
    if status != 200 or captcha.get('code') != 200 or not isinstance(captcha.get('data'), dict) or captcha['data'].get('captchaEnabled') is not False:
        raise RuntimeError('Owned login captcha configuration differs')
    report['http_captcha'] = {'http_status': status, 'code': 200, 'captcha_enabled': False}
    report['http_progress'] = 'admin_login'
    admin_token = owned_login(port, 'WTA', admin_pass, report, 'admin')
    redactions.append(admin_token)
    report['http_progress'] = 'ordinary_login'
    ordinary_token = owned_login(port, 'test', ordinary_pass, report, 'ordinary')
    redactions.append(ordinary_token)
    report['http_progress'] = 'real_upload'
    ...
    report['http_progress'] = 'upload_metadata'
    ...
    def submit(case, attachments, token):
        report['http_progress'] = 'submit_' + case
