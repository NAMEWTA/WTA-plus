...
其他模块通过 `wta-api` 接入，不要 Maven 依赖 `wta-workflow`，不要调用 `IFlw*` 或 Warm-Flow `InsService` / `TaskService` / `DefService`。能力描述不清时，按文中路径读源码，不得凭空补调用。
...
| 办理人 | 设计器配置用户/角色/部门/岗位/SpEL；不要复制 `PermissionHandler` | `backend/wta-modules/wta-workflow/src/main/java/org/namewta/workflow/handler/WorkflowPermissionHandler.java` |

`wta-demo` / `wta-system` / `wta-ai` / `wta-job` 当前没有接入实现。新模块复制请假模式，换成自己的 `flowCode`。

唯一装配方：`backend/wta-admin/pom.xml` 依赖 `wta-workflow`。

## 禁止

- 在业务模块 `pom.xml` 增加 `wta-workflow` 依赖。
- import `org.namewta.workflow.service.IFlw*` 或 `org.dromara.warm.flow.*` 引擎服务。
- 实现 Warm-Flow `GlobalListener` / `PermissionHandler`（已由本模块占用：`WorkflowGlobalListener`、`WorkflowPermissionHandler`）。
- 为接入审批新增 LiteFlow 组件或改 `backend/wta-modules/wta-workflow/src/main/resources/liteflow/task-chain.el.xml` / `instance-chain.el.xml`。
...
1. **业务表加 `status`**，取值对齐 `BusinessStatusEnum`（`draft` / `waiting` / `finish` / `cancel` / `invalid` / `back` / `termination`）。路径：`backend/wta-common/wta-common-core/src/main/java/org/namewta/common/core/enums/BusinessStatusEnum.java`
...
7. **删业务数据时**调 `workflowService.deleteInstance(ids)`（字符串业务 id 列表）。删实例链路会先发 `ProcessDeleteEvent`：`backend/wta-modules/wta-workflow/src/main/java/org/namewta/workflow/liteflow/instance/InstanceDeleteEventComponent.java`
8. **人的待办/已办/抄送**走 `/workflow/task/pageByTaskWait` 等；业务页用 `formPath` + query `id` / `taskId` 打开，办理走 `POST /workflow/task/completeTask`。

同一 `flowCode+businessId` 启动带 `@Lock4j`：`backend/wta-modules/wta-workflow/src/main/java/org/namewta/workflow/service/impl/FlwTaskServiceImpl.java`。不要并行对同一单据重复启动。
...
事件类型路径：`backend/wta-api/src/main/java/org/namewta/workflow/api/event/`
发布：`backend/wta-modules/wta-workflow/src/main/java/org/namewta/workflow/handler/FlowProcessEventHandler.java`
...
```ts
// frontend/packages/web-domains/workflow/src/task/TaskListPage.vue
router.push({
...
业务页用 `id` 调自己的 CRUD，用 `taskId` 调 `POST /workflow/task/completeTask`。不要在本模块找表单设计 CRUD。当前请假领域页位于 `frontend/packages/web-domains/workflow/src/leave/`；Admin 通过 workflow manifest 装配页面。
