...
owned = Path(tempfile.mkdtemp(prefix='t30-journey-', dir=root/'temp/team/lead'))
apps = ['admin', 'home', 'sso']
...
try:
    ...
    for app in apps:
        run(['corepack', 'pnpm', '--filter', f'@namewta/{app}-web', 'build:prod'],
            f'{name}-build-{app}', {'VITE_APP_CONTEXT_PATH': '/' if app == 'sso' else f'/{app}/'})
    run([sys.executable, str(evidence/'run-t30-sso-real.py'), '--evidence-name', name+'-journey',
         '--test', 'SsoHttpsSessionIntegrationTest', '--journey'], name+'-journey-driver')
    run([sys.executable, str(evidence/'run-t30-sso-real.py'), '--evidence-name', name+'-security',
         '--test', 'SsoHttpsSessionIntegrationTest'], name+'-security-driver')
    code = 0
