...
database = 'namewta_workflow_test_' + suffix
...
start = time.time()
try:
    mysql = run(['docker', 'run', '-d', '--name', 'namewta-t30-workflow-mysql-' + suffix, '--label', 'namewta.test.owner=T-30', '-p', '127.0.0.1::3306',
    ...
    redis = run(['docker', 'run', '-d', '--name', 'namewta-t30-workflow-redis-' + suffix, '--label', 'namewta.test.owner=T-30', '-p', '127.0.0.1::6379', images['redis']])
    ...
    command = ['./mvnw', '-B', '-ntp', '-pl', 'wta-admin', '-am', '-Dtest=WorkflowTaskEngineIntegrationTest',
               '-Dsurefire.failIfNoSpecifiedTests=false', '-Dprofiles.active=dev,local', '-Dworkflow.engine.integration=true',
               '-Dworkflow.mysql.integration.url=jdbc:mysql://127.0.0.1:' + mysql_port + '/' + database + '?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=Asia/Shanghai',
               '-Dprofile.redis.integration.port=' + redis_port, 'test']
    record.update(command=command, cwd='backend', log=name + '.log')
    print('Running actual Warm-Flow engine authorization and task transitions', flush=True)
