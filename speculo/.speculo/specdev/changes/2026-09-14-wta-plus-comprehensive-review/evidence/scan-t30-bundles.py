...
for scope in ['wta-common-encrypt']:
...
result['source_fingerprint'] = json.loads((evidence/'T-30-source-current.json').read_text())['source_fingerprint']
result['scope_note'] = 'Current built App bytes plus retired backend class absence and unchanged database encryption sources; OpenAPI request body was intentionally changed by T-03 and is verified in T-30 HTTP/signature gates, not asserted identical to HEAD.'
(evidence / 'T-30-bundle-scan.json').write_text(json.dumps(result, ensure_ascii=False, indent=2) + '\n')
