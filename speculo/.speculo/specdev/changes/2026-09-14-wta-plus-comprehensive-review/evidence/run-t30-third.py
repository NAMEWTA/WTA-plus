...
database = 'namewta_third_test_' + suffix
password = 'owned-third-test-only'
code = 1
process = None
...
start = time.time()
try:
    mysql = run(['docker', 'run', '-d', '--name', 'namewta-t30-24-mysql-' + suffix, '--label', 'namewta.test.owner=T-24', '--label', 'namewta.test.run=T-30', '-p', '127.0.0.1::3306',
    ...
    redis = run(['docker', 'run', '-d', '--name', 'namewta-t30-24-redis-' + suffix, '--label', 'namewta.test.owner=T-24', '--label', 'namewta.test.run=T-30', '-p', '127.0.0.1::6379', images['redis'], 'redis-server', '--save', '', '--appendonly', 'no'])
    ...
    command = ['./mvnw', '-B', '-ntp', '-pl', 'wta-admin', '-am', '-Dtest=ThirdResilienceRecoveryIntegrationTest,ThirdRedisIntegrationTest,ThirdResiliencePolicyAdapterTest,ThirdLimitConfigurationIntegrationTest',
               '-Dsurefire.failIfNoSpecifiedTests=false', '-Dprofiles.active=dev,local', '-Dthird.configuration.integration=true', '-Dthird.redis.integration.port=' + redis_port, '-Dthird.redis.integration.container=' + redis,
               '-Dthird.mysql.integration.url=jdbc:mysql://127.0.0.1:' + mysql_port + '/' + database + '?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=Asia/Shanghai',
               '-Dnamewta.repo.root=' + str(root), 'test']
    record.update(command=command, cwd='backend', log=name + '.log')
    print('Running actual Third/MySQL/Redis/HTTP quota recovery', flush=True)
    with (evidence/record['log']).open('w') as log:
        ...
        for line in process.stdout:
            ...
            if any(marker in line for marker in ['[ERROR]', 'Tests run:', 'BUILD SUCCESS', 'BUILD FAILURE', 'T-24:', ' passed (', ' failed']): print(line, end='', flush=True)
        code = process.wait()
