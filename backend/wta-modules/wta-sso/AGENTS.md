...
`WEB_CORS_ALLOWED_ORIGINS` 是逗号分隔的精确 HTTP(S) Origin 白名单。未设置时默认为空，不许可跨来源访问；跨来源必须显式列出不含路径的精确 Origin。任何 `*` 或通配子域配置均拒绝启动，包括不携带凭据的配置。同源访问不需要 CORS 许可。跨 Origin 不自动使用 SameSite=None 或共享父域 Cookie。
