#!/usr/bin/env python3
"""Serial owned MySQL/Redis -> real Notify callback transactions; no external provider calls."""
import datetime
import hashlib
import signal
import json
...
name = sys.argv[1]
assert name.startswith('T-23-') and '/' not in name
...
start = time.time()
try:
    mysql = run(['docker', 'run', '-d', '--name', 'namewta-t23-mysql-' + suffix, '--label', 'namewta.test.owner=T-23', '-p', '127.0.0.1::3306',
    ...
    redis = run(['docker', 'run', '-d', '--name', 'namewta-t23-redis-' + suffix, '--label', 'namewta.test.owner=T-23', '-p', '127.0.0.1::6379', images['redis'], 'redis-server', '--save', '', '--appendonly', 'no'])
    ...
    command = ['./mvnw', '-B', '-ntp', '-pl', 'wta-admin', '-am', '-Dtest=' + (sys.argv[2] if len(sys.argv) > 2 else 'NotifyAtomicResultIntegrationTest'),
               '-Dsurefire.failIfNoSpecifiedTests=false', '-Dnotify.atomic.integration=true', '-Dnotify.redis.integration.port=' + redis_port,
    ...
    with (evidence/record['log']).open('w') as log:
        ...
        for line in process.stdout:
            ...
            if any(marker in line for marker in ['[ERROR]', 'Tests run:', 'BUILD SUCCESS', 'BUILD FAILURE', 'T-23:', ' passed (', ' failed']): print(line, end='', flush=True)
        code = process.wait()
