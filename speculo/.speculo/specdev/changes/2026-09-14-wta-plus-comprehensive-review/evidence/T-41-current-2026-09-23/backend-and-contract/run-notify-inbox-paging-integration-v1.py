#!/usr/bin/env python3
"""Opt-in, owned T-41 inbox paging one-class integration runner."""
...
RUN_ROOT = Path('/tmp/wta-t41/runs')
...
TEST_CLASS = 'org.namewta.test.notify.NotifyInboxPagingIntegrationTest'
TEST_SOURCE = ROOT / 'backend/wta-admin/src/test/java/org/namewta/test/notify/NotifyInboxPagingIntegrationTest.java'
...
def owned_ids(run_id):
    found = docker('ps', '-aq', '--no-trunc', '--filter', 'label=namewta.test.owner=T-41',
...
def assert_owned(cid, run_id):
    actual = labels(cid)
    if actual.get('namewta.test.owner') != 'T-41' or actual.get('namewta.test.run') != run_id:
...
def mysql_run_args(run_id, mysql_env):
    return ('run', '--pull=never', '-d', '--name', 't41-mysql-' + run_id,
            '--label', 'namewta.test.owner=T-41',
...
def retain_fresh_xml(run_dir, test_class, report_path, started_ns):
    """Keep the exact sanitized selected XML even when its assertions fail."""
    if started_ns is None or not report_path.is_file() or report_path.stat().st_mtime_ns < started_ns:
        return None
    if report_path.name != 'TEST-' + test_class + '.xml':
        raise RuntimeError('Refusing to retain a non-selected Surefire XML')
    target_dir = run_dir / 'xml'
    ...
    with os.fdopen(os.open(target, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600), 'wb') as stream:
        stream.write(data)
    return {'file': str(target.relative_to(run_dir)), 'sha256': hashlib.sha256(data).hexdigest(),
            'bytes': len(data), 'source_mtime_ns': report_path.stat().st_mtime_ns}
...
def validate_inbox_source(inbox_text, atomic_text):
    """Require the exact opt-in class and owned DB/Redis seam before any launch."""
    if 'class NotifyInboxPagingIntegrationTest' not in inbox_text:
        raise RuntimeError('T41 exact integration class is absent')
    if not re.search(r'@EnabledIfSystemProperty\s*\(\s*named\s*=\s*"notify\.inbox\.paging\.integration"\s*,\s*matches\s*=\s*"true"', inbox_text):
        raise RuntimeError('T41 integration class lacks its opt-in property')
    direct_seam = (all(value in inbox_text for value in
                       ('T41_MYSQL_PASSWORD', 'notify.mysql.integration.url',
                        'notify.redis.integration.port')))
    atomic_fixture_seam = ('NotifyAtomicResultIntegrationTest' in inbox_text
                           and '.open()' in inbox_text
                           and 'T36_MYSQL_PASSWORD' in atomic_text
                           and 'notify.mysql.integration.url' in atomic_text
                           and 'notify.redis.integration.port' in atomic_text)
    if not (direct_seam or atomic_fixture_seam):
        raise RuntimeError('T41 integration class lacks an owned DB/Redis fixture seam')
...
def main():
    ...
    report = {'gate': 'T-41 inbox paging exact one-class real MySQL/Redis integration', 'run_id': run_id,
              'mode': 'acceptance',
              'acceptance': False,
    ...
    try:
        ...
        validate_inbox_source(test_text, atomic_text)
        ...
        redis_id = full_id(docker('run', '--pull=never', '-d', '--name', 't41-redis-' + run_id,
                                  '--label', 'namewta.test.owner=T-41',
                                  '--label', 'namewta.test.run=' + run_id,
        ...
        with os.fdopen(os.open(raw_log, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600), 'wb') as stream:
            maven_proc = subprocess.Popen(argv, cwd=ROOT / 'backend', stdout=stream,
                                          ...
                                          env=safe_env(T41_MYSQL_PASSWORD=app_password,
        ...
        try:
            report['retained_xml'] = [item for test_class, path in SUITES
                                      if (item := retain_fresh_xml(run_dir, test_class, path, started_ns))]
        except Exception:
            errors.append('fresh_surefire_xml_retention_failed')
