...
skill_scan: "scanned project Skill roots under <Path>.agents/skills/*/SKILL.md</Path> (engineering-standards, namewta-fullstack-development, ruoyi-common-modules-guide, ruoyi-module-guide, deploy-namewta-environment, java-api-compatibility, project-customization-delivery, upstream-fork-sync); excluded last three as non-axis for rename/W0b/publication; bound for T-15: engineering-standards, namewta-fullstack-development"
skill_bindings:
  - {"id":"engineering-standards","path":"<Path>.agents/skills/engineering-standards/SKILL.md</Path>","sha256":"acac7d1d4341d3361c298ebc5a61841d794cafbe9f235171b56fddd7ecb0037d","phase":"implement","operation":"legacy-freeze-transition","inputs":["Ticket T-15","Tickets Map","Spec/ADR projections"],"outputs":["Evidence notes for T-15","next-step inputs"],"required":true,"on_failure":"block-ticket","references":[]}
  - {"id":"namewta-fullstack-development","path":"<Path>.agents/skills/namewta-fullstack-development/SKILL.md</Path>","sha256":"e17e31173638b06f93bb2c26cf3315f50a1a075b3e298434c98fc47d175d7792","phase":"implement","operation":"legacy-freeze-transition","inputs":["Ticket T-15","Tickets Map","Spec/ADR projections"],"outputs":["Evidence notes for T-15","next-step inputs"],"required":true,"on_failure":"block-ticket","references":[]}
resource_claims:
  - "legacy-freeze-transition"
artifact: ticket
change: 2026-09-12-rename-ruoyi-dromara-to-wta
id: T-15
title: Legacy Repository Freeze Transition
status: done
planning_depth: deep
planning_depth_reason: 旧仓 README/archive/protection 落地；需独立 legacy auth。
ready: true
risk: high
blocked_by: [T-14]
contract_ids: [AC-010]
...
# Ticket T-15: Legacy Repository Freeze Transition

- **Ticket 文件：** `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/ticket/15-legacy-freeze.md</Path>`
...
- **完成 Evidence：** `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/evidence/T-15.md</Path>`

实现本 Ticket 时，Lead 与 implementation subagent 必须按顺序完整读取总体 Map，读取项目 Skill 的 frontmatter 与入口并只展开适用于 `ALL`/`T-15` 的匹配项，再读取本 Ticket 与相关上游工件。Map 中的 Skill 是最低必读集合；新的匹配项先由 Lead 同步到 Map 并重新校验。
...
- **目标：** 在 `legacy_repo_mutation_authorized=true` 后，对旧三仓做 README 指向新仓 / archive / protection 等 **批准的** freeze transition；禁止 force-push rewrite。
- **可观察产出：** remote state proof；旧仓指向 NAMEWTA/WTA-plus。
- **来源：** ADR-010、AC-LEGACY、HC-05/10。
- **当前事实：** legacy_repo_mutation_authorized 当前 false。
- **Planning Depth 原因：** 旧仓 README/archive/protection 落地；需独立 legacy auth。
...
- 需独立授权。
- 禁止 rewrite/force-push。

### 已采用的低影响假设

- 具体 archive 与否由 CTO 在授权范围写明。
...
|---|---|---|
| 旧仓 freeze transition。 | T-16 语义、T-14 新仓 URL。 | 旧仓继续开发；filter-repo。 |

## 4. 要构建什么

按授权清单改 README/archive/protection 并取证。
...
|---|---|---|---|---|
| 授权 | status | legacy true | 是 | evidence/T-15.md |
| 无 rewrite | remote | 无 force-push | 是 | evidence/T-15.md |
| 指向新仓 | README | WTA-plus | 是 | evidence/T-15.md |
...
- [ ] `AC-010`。
- [ ] 无 force-push/rewrite。
- [ ] 实现开始前已完整读取 Tickets Map，已读取项目 Skill 入口并完整展开适用于 `ALL`/`T-15` 的匹配项。
- [ ] 验证矩阵全部执行并记录到 `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/evidence/T-15.md</Path>`（完成时；**本轮不新建伪 Evidence**）。
...
依据 `<Path>{roots.workflows}/specdev/common/rules/skill-invocation.md</Path>`：本票 `skill_bindings` 绑定 Map 中适用于 ALL/`T-15` 的每一 Path。
