#!/usr/bin/env python3
"""Run T-31 repository checks serially and retain real outcomes."""
import json
import os
...
if mode == 'backend':
    commands = [('backend', ['./mvnw', '-B', '-ntp', '-pl', 'wta-modules/wta-profile,wta-modules/wta-notify,wta-admin', '-am', 'test'])]
elif mode == 'callers':
    commands = [('backend', ['./mvnw', '-B', '-ntp', '-pl', 'wta-admin', '-am', '-Dtest=EnterpriseTransferServiceTest,EnterpriseTransferPersistenceTest,PersonRebindNotificationTest,CaptchaNotifyCallerUnitTest,NotifyTestSendServiceTest', '-Dsurefire.failIfNoSpecifiedTests=false', 'test'])]
elif mode == 'static':
