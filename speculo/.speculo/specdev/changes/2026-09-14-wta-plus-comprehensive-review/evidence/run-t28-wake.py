...
start = time.time()
try:
    mysql = run(['docker', 'run', '-d', '--name', 'namewta-t28-mysql-' + suffix, '--label', 'namewta.test.owner=T-28', '-p', '127.0.0.1::3306',
    ...
    redis = run(['docker', 'run', '-d', '--name', 'namewta-t28-redis-' + suffix, '--label', 'namewta.test.owner=T-28', '-p', '127.0.0.1::6379', images['redis'], 'redis-server', '--save', '', '--appendonly', 'no'])
    ...
    command = ['./mvnw', '-B', '-ntp', '-pl', 'wta-admin', '-am', '-Dtest=NotifyWakeIntegrationTest,NotifyAtomicResultIntegrationTest,NotifyOutboxWakeCrossProcessIT',
               '-Dsurefire.failIfNoSpecifiedTests=false', '-Dnotify.atomic.integration=true', '-Dnotify.wake.integration=true', '-Dnotify.outbox.wake.redis.port=' + redis_port, '-Dnotify.redis.integration.port=' + redis_port,
    ...
    with (evidence/record['log']).open('w') as log:
        ...
        for line in process.stdout:
            ...
            if any(marker in line for marker in ['[ERROR]', 'Tests run:', 'BUILD SUCCESS', 'BUILD FAILURE', 'T-28:', ' passed (', ' failed']): print(line, end='', flush=True)
        code = process.wait()
