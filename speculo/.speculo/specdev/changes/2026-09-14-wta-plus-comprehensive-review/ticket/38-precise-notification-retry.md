...
shared_path_owners: ["<Path>backend/wta-modules/wta-notify/src/main/java/org/namewta/notify/controller/admin/NotificationController.java</Path> => cors_audit (single product writer; Lead integration and governance)", "<Path>backend/wta-modules/wta-notify/src/main/java/org/namewta/notify/service/runtime/NotificationApplicationRuntimeService.java</Path> => cors_audit (single product writer; Lead integration and governance)", "<Path>backend/wta-modules/wta-notify/src/main/java/org/namewta/notify/dao/NotifyNotificationDao.java</Path> => cors_audit (single product writer; Lead integration and governance)", "<Path>backend/wta-modules/wta-notify/src/main/java/org/namewta/notify/mapper/NotifyOutboxMapper.java</Path> => cors_audit (single product writer; Lead integration and governance)", "<Path>backend/wta-modules/wta-notify/src/main/resources/mapper/notify/NotifyOutboxMapper.xml</Path> => cors_audit (single product writer; Lead integration and governance)", "<Path>backend/wta-api/src/main/java/org/namewta/notify/api/NotificationRetryCommand.java</Path> => cors_audit (single product writer; Lead integration and governance)", "<Path>backend/wta-api/src/main/java/org/namewta/notify/api/RetryReceipt.java</Path> => cors_audit (single product writer; Lead integration and governance)", "<Path>frontend/packages/domains/notify/src/</Path> => cors_audit (single product writer; Lead integration and governance)", "<Path>frontend/packages/web-domains/notify/src/NotificationPage.vue</Path> => cors_audit (single product writer; Lead integration and governance)", "<Path>frontend/packages/api-contracts/</Path> => cors_audit (single product writer; Lead integration and governance)", "<Path>frontend/tooling/openapi/</Path> => cors_audit (single product writer; Lead integration and governance)", "<Path>backend/wta-admin/src/test/java/org/namewta/test/notify/</Path> => cors_audit (single product writer; Lead integration and governance)", "<Path>backend/wta-api/src/main/java/org/namewta/notify/api/NotificationApplicationService.java</Path> => cors_audit (single product writer; Lead integration and governance)", "<Path>frontend/packages/web-domains/notify/src/index.ts</Path> => cors_audit (single product writer; Lead integration and governance)", "<Path>frontend/packages/web-domains/notify/src/index.test.ts</Path> => cors_audit (single product writer; Lead integration and governance)", "<Path>frontend/packages/web-domains/notify/src/NotificationPage.test.ts</Path> => cors_audit (single product writer; Lead integration and governance)", "<Path>backend/wta-modules/wta-notify/src/test/java/org/namewta/notify/adapter/event/NotifyOutboxWakePublisherTest.java</Path> => cors_audit (single product writer; Lead integration and governance)", "<Path>.agents/skills/engineering-standards/references/notification.md</Path> => cors_audit (single product writer; Lead integration and governance)", "<Path>release-artifacts/docker/infrastructure/mysql/init/50-cde-base-dml.sql</Path> => cors_audit (T38 single PENDING dictionary value only; no other SQL edits)"]
---

# T-38：重试与取消接口准确定位资源和任务

Map：<Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/tickets-map.md</Path>；Spec：<Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/spec.md</Path>；Goal：<Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/goal-plan.md</Path>。
用户已激活Goal并允许native gpt-6-sol/xhigh子代理。当前base b47ff8b91cfef02a9f28de0e201edcd1575a950f，cors_audit单产品writer；Lead独占治理、commit、服务与验收，其他子代理只读/私有驱动准备。不创建新worktree。
...
保留工程分层、Client/权限、资源owner、安全日志、真实供应商协议和唯一六SQL基座。当前Goal授权本地实施、commit与direct-parent，额外远程/真实数据操作不推定授权。
...
base b47ff8b91cfef02a9f28de0e201edcd1575a950f（产品与已验收T02的617a369等价）。T36/T37已done；T02/T03当前验收闭合。唯一本票产品writer cors_audit，Lead治理/提交/Maven与owned MySQL/Redis验收；只读ops/legacy审查。正式Packet为 evidence/dispatch-T-38-20260923-01.md，写集在动手前补齐公共Service说明、前端权限注册/SFC测试、已有wake消费者及通知规范。无新DDL/身份owner模型。

Controller path必须定位实际intent，有body且ID不同先明确拒绝；body无ID时可按path补全，未实现的非空retry key拒绝。定向delivery不存在/跨intent拒绝且零写。控制面沿当前route权限以及公共登录/Client-token/access_path验证；appId是业务namespace（admin-web/workflow/profile/notify），不是会话ClientID，不能新增错误的相等检查。对不同intent的delivery归属检查才是本票数据范围不变量。

Intent→Outbox→Delivery统一锁序；先当前读完整候选/校验，再实际requeue的精确行数决定queuedCount、聚合和提交后wake。零任务保留真实当前状态。不能凭FAILED一律授权再次物理发送，须核本地未发送或typed可重试事实与剩余预算；外部UNKNOWN明确拒绝；外部WAITING_RECEIPT/活PROCESSING/已有READY不新建并行任务，不清Redis或重置活租约与自动重试预算。IN_APP已有消息事实/原子路径与唯一关系保持，未知结果本地可安全重做条件须由真实DB证明。未发送本地配置失败在条件满足时仍可实际重试，不能用禁用所有FAILED虚满足A/B定向需求。过期全面合同由T39负责，不在本票延长任何有效期。

先用既有API可编译HTTP冲突用例取行为红灯，随后完整实现、新queuedCount及全部仓内消费者/生成物同步；不以新增API编译错误代替红灯。真实DB新类NotifyManualRetryIntegrationTest、T38_MYSQL_PASSWORD仅子进程环境；验证A/B定向、无任务、非法归属/ID/key、外部UNKNOWN拒绝、两事务同任务并发与claim交错、rollback/wake/owner，不连接供应商。驱动先在/tmp冻结并合成自检，Lead才启动隔离MySQL/Redis。原始失败/skip必须保留。

### revision164 IN_APP UNKNOWN边界澄清

原已接受第6节允许IN_APP依T36安全重试；不得把外部UNKNOWN/WAITING禁重发泛化为所有本地UNKNOWN一律禁用。仅IN_APP、同Intent/Delivery、唯一无活租约任务、剩余预算及既有消息/关系事实经T36幂等原子入口闭合时，可安全重做；真实测试证明已有消息不重复、事件不重复，不能证明的历史形状精确拒绝。外部SMS/MAIL UNKNOWN继续明确拒绝，批量请求如含外部未知须在任何写入前失败关闭；定向已证明安全A不因未选中B未知被误伤。此处落实原Ticket合同，不增加新状态机/新身份模型。
