...
resource_claims: ["finding:R-01", "contract:AC-002"]
artifact: ticket
...
planning_depth_reason: "本票涉及 R-01；需先完成证据/高影响取舍，不能直接实现。"
ready: false
risk: high
blocked_by: []
contract_ids: [AC-002]
...
| IN | REUSE | OUT |
|---|---|---|
| 先固定换票响应access_token进入OperLogEvent的红灯，以及原queryString泄漏canary的红灯；立即令凭据签发接口不记录响应正文；保留审计元数据；由现有common拥有一个日志副本策略，HTTP/操作日志adapter复用；删除原始query输出和未经策略处理的response序列化 | 现有模块边界、权限、日志/错误合同中未被本票改变者 | 其他 finding 的实现、无证据的全仓重写、提交/部署、历史数据清理 |

## 4. 行为与实现契约

- **入口或 seam：** <Path>backend/wta-common/wta-common-log/</Path>；由该 module 的公开 interface 接受输入。
- **输入/输出：** 输入、错误和状态必须保持对应现有合同；任何外部行为变化先更新 Spec/ADR并阻塞本票。
- **不变量：** 失败关闭、权限/Client上下文、资源所有权、事务/幂等和可观测性按适用工程规则成立。
- **错误：** 普通字段/操作者/耗时/失败状态仍可观测
...
|---|---|---|---|
| AC 场景 1 | <Path>backend/wta-common/wta-common-log/</Path> 定向测试/静态或隔离运行 | canary不出现在HTTP sink、OperLogEvent、数据库或错误日志 | <Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/evidence/T-02.md</Path> |
| AC 场景 2 | <Path>backend/wta-common/wta-common-log/</Path> 定向测试/静态或隔离运行 | 普通字段/操作者/耗时/失败状态仍可观测 | <Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/evidence/T-02.md</Path> |
| AC 场景 3 | <Path>backend/wta-common/wta-common-log/</Path> 定向测试/静态或隔离运行 | 签名、加解密、SSE与正常token响应保持正确 | <Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/evidence/T-02.md</Path> |
| AC 场景 4 | <Path>backend/wta-common/wta-common-log/</Path> 定向测试/静态或隔离运行 | 旧日志处置与凭据轮换另有批准记录，本票不自动删历史数据 | <Path>{roots.state}/specdev/changes/2026-09-14-wta-plus-comprehensive-review/evidence/T-02.md</Path> |
...
- **迁移/兼容：** ADR-0025已禁止泄漏；新的默认日志保留策略见ADR-CR-002待审。
...
- 在 <Path>backend/</Path> 的 PowerShell 执行 `.\mvnw.cmd -pl wta-common/wta-common-json,wta-common/wta-common-web,wta-common/wta-common-log,wta-modules/wta-sso,wta-modules/wta-system,wta-admin -am test`；Bash 使用 `./mvnw` 同参数入口。
