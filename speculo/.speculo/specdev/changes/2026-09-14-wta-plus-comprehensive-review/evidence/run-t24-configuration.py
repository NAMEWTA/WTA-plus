...
name = sys.argv[1]
assert name.startswith('T-24-') and '/' not in name
...
start = time.time()
try:
    mysql = run(['docker', 'run', '-d', '--name', 'namewta-t24-mysql-' + suffix, '--label', 'namewta.test.owner=T-24', '-p', '127.0.0.1::3306',
    ...
    redis = run(['docker', 'run', '-d', '--name', 'namewta-t24-redis-' + suffix, '--label', 'namewta.test.owner=T-24', '-p', '127.0.0.1::6379', images['redis'], 'redis-server', '--save', '', '--appendonly', 'no'])
