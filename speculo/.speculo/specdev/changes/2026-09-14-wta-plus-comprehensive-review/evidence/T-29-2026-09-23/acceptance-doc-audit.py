...
originals.update(json.loads((evidence/'T-29-supplement-originals.json').read_text()))
originals.update(json.loads((evidence/'T-29-followup-originals.json').read_text()))
originals.update({p:d for p,d in json.loads((evidence/'T-29-checker-originals.json').read_text()).items() if p.endswith('.md')})
...
# Keep the historical migration/rule owner separate from the handbook that actually inherits a deleted leaf today.
report['removed_owner_rows'] = []
report['removed_original_sha_verified'] = 0
...
for row in manifest:
    ...
    if not isinstance(original, dict) or not isinstance(original.get('content'), str):
        fail('Missing immutable original for removed handbook: '+row['path'])
    elif hashlib.sha256(original['content'].encode('utf8')).hexdigest() != row['before_sha256'] or original.get('sha256') != row['before_sha256']:
        fail('Immutable original SHA mismatch for removed handbook: '+row['path'])
    else:
        report['removed_original_sha_verified'] += 1
    directory = (root/row['path']).parent
    while directory != root and not (directory/'AGENTS.md').is_file():
        directory = directory.parent
    effective = str((directory/'AGENTS.md').relative_to(root)) if directory != root else None
    result = dict(path=row['path'], historical_owner=row['owner'],
                  effective_owner=effective, rule_destination=row['rule_destination'],
                  navigation_destination=row['navigation_destination'])
    report['removed_owner_rows'].append(result)
    if effective != row['owner'] and expected_owner_delta.get(row['path']) != effective:
        fail('Unreviewed effective owner drift: '+row['path'])
    if effective is None or not (root/effective).is_file():
        fail('Missing effective owner: '+row['path'])
report['reviewed_effective_owner_deltas'] = [row for row in report['removed_owner_rows']
                                             if row['effective_owner'] != row['historical_owner']]
...
report['protected_changed_paths'] = [path for path, expected in protected.items() if sha(root/path) != expected]
report['protected_files_unchanged'] = not report['protected_changed_paths']
report['reviewed_blank_line_only_root_agents'] = False
if report['protected_changed_paths']:
    if report['protected_changed_paths'] != ['AGENTS.md']:
        fail('Protected files changed beyond reviewed root AGENTS.md blank lines')
    else:
        historical_commit = '70eed51acb703ed19a48a47d182f2476f3eabcf7'
        result = subprocess.run(['git', 'show', historical_commit+':AGENTS.md'], cwd=root,
                                capture_output=True)
        if result.returncode or hashlib.sha256(result.stdout).hexdigest() != protected['AGENTS.md']:
            fail('Root AGENTS historical input does not match protected checkpoint')
        else:
            old_lines = [line for line in result.stdout.splitlines(keepends=True) if line.strip()]
            new_lines = [line for line in (root/'AGENTS.md').read_bytes().splitlines(keepends=True) if line.strip()]
            report['reviewed_blank_line_only_root_agents'] = old_lines == new_lines
            if not report['reviewed_blank_line_only_root_agents']:
                fail('Root AGENTS changed beyond blank lines')
...
(Path('/tmp/wta-t29/acceptance-doc-audit-result.json')).write_text(json.dumps(report, ensure_ascii=False, indent=2)+'\n')
...
print('links', len(report['links']), 'module rows', len(report['module_rows']), 'removed references', len(report['remaining_removed_references']), 'effective-owner deltas', len(report['reviewed_effective_owner_deltas']), 'blank-only-root-reviewed', report['reviewed_blank_line_only_root_agents'])
