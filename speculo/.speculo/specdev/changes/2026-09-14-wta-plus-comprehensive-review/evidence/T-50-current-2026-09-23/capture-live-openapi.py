#!/usr/bin/env python3
"""Opt-in T-50 full-JAR live /v3/api-docs capture on owned services only."""
...
RUN_ROOT = Path('/tmp/wta-t50/openapi-live')
...
def main():
    ...
    app_user = 't50o_' + run_id
    minio_user = 't50minio' + run_id
    bucket = 't50-' + run_id
    secrets_to_redact = [root_password, app_password, redis_password, minio_password]
    report = {'gate': 'T-50 full-JAR live OpenAPI capture', 'run_id': run_id,
    ...
    try:
        ...
        mysql_id = helper.full_id(helper.docker(
            'run', '--pull=never', '-d', '--name', 't50-openapi-mysql-' + run_id,
        ...
        redis_id = helper.full_id(helper.docker(
            'run', '--pull=never', '-d', '--name', 't50-openapi-redis-' + run_id,
            '--label', 'namewta.test.owner=' + OWNER, '--label', 'namewta.test.run=' + run_id,
        ...
        minio_id = helper.full_id(helper.docker(
            'run', '--pull=never', '-d', '--name', 't50-openapi-minio-' + run_id,
            '--label', 'namewta.test.owner=' + OWNER, '--label', 'namewta.test.run=' + run_id,
