...
skill_bindings:
  - {"id":"engineering-standards","path":"<Path>.agents/skills/engineering-standards/SKILL.md</Path>","sha256":"dbc475149e3588cb840c15e0bb287920b876c0eef4c4004dde078de1c4fca6d9","phase":"implement","operation":"build-wta-sso-oauth-pkce","inputs":["本Ticket T-03 规划合同与路径契约","Tickets Map Skill 矩阵","上游 Spec/ADR/CONTEXT"],"outputs":["符合工程硬约束的实现落点说明","未越界路径与模块边界记录"],"required":true,"on_failure":"block-ticket","references":[]}
  - {"id":"namewta-fullstack-development","path":"<Path>.agents/skills/namewta-fullstack-development/SKILL.md</Path>","sha256":"65d6f22d11b990de23135c5a2c0bceb3907e964c800d527d58667d12db784988","phase":"implement","operation":"build-wta-sso-oauth-pkce","inputs":["本Ticket T-03 垂直切片范围","Spec AC 与 DEC 冻结","相关前后端接缝"],"outputs":["跨层合同对齐的可观察产出清单","下一票依赖的稳定接缝说明"],"required":true,"on_failure":"block-ticket","references":[]}
...
shared_paths:
  ...
  - "<Path>release-artifacts/docker/infrastructure/mysql/init/60-namewta-dml.sql</Path>"
shared_path_owners:
  - "<Path>backend/wta-modules/pom.xml</Path> => T-03"
  - "<Path>backend/wta-admin/pom.xml</Path> => T-03"
  - "<Path>backend/pom.xml</Path> => T-03"
  - "<Path>backend/wta-admin/src/main/resources/application.yml</Path> => T-03"
...
# Ticket T-03: 新建 wta-sso：authorize/token/revoke + PKCE 负向 + 模块边界

- **Ticket 文件：** `<Path>{roots.state}/specdev/changes/2026-09-12-wta-sso/ticket/03-wta-sso-oauth-pkce-module.md</Path>`
...
实现本 Ticket 时，Lead 与 implementation subagent 必须按顺序完整读取总体 Map，读取适用于 `ALL`/`T-03` 的项目 Skill，再读本票。可复用 `<Path>{roots.state}/specdev/changes/2026-09-12-wta-sso/superseded-r3/ticket/02-wta-sso-oauth-pkce-module.md</Path>` 的协议细节，但 ID/file/blocked_by/管理面句必须按 Round4。

## 1. 战略与来源

- **目标：** 落地 `wta-sso` 模块，提供 Authorization Code + PKCE S256 的 authorize/token/revoke，并强制负向合同与模块边界。
- **可观察产出：** 合法 PKCE 流可换得**目标业务 Client** 的 Sa-Token；AC-004…012 负向全拒绝；revoke 仅废本令牌；模块不直连 system Mapper/`ISys*`；仅 API/Port。
- **来源：** `AC-004`…`AC-013`、`AC-021`、`AC-022`、`DEC-001`/`DEC-010`/`DEC-012`/`DEC-015`/`DEC-016`/`DEC-020`、`ADR-001`/`ADR-002`。
- **当前事实：** 工作树可能已有 `backend/wta-modules/wta-sso/`（Round3 实现残留）；本票以 Round4 合同重验收，管理面依赖改为 T-01 SSO 管理登记结果。
...
- 短寿命 code 表可在 DDL 追加；shared owner 仍 T-01；Cookie 登录会话表面与 T-04 衔接。
...
| IN | REUSE | OUT |
|---|---|---|
| wta-sso 模块；authorize/token/revoke；PKCE 负向；code 表追加；组装开关 | Sa-Token/LoginHelper；SysClient 字段(T-01)；SecurityConfig 隔离 | SSO 管理 UI；客户端管理接入；sso-web UI；OIDC/SLO；外置 IdP |
...
2. 按 wta-module-guide 建/对齐模块与 API Port。
...
- frontmatter 为准；DDL 短寿命表追加时不破坏 T-01 列语义；pom/application.yml owner=T-03。
- **保留或不动：** system Mapper 直连；notify；SSO/客户端管理 UI。

## 8. 验证矩阵

| 行为或风险 | 验证接缝 | 命令或步骤 | 预期结果 | Evidence |
|---|---|---|---|---|
| PKCE 负向 | API | AC-004…012 | 全拒绝 | `<Path>{roots.state}/specdev/changes/2026-09-12-wta-sso/evidence/T-03.md</Path>` |
...
- **E2E disposition：** not-required：API/集成为主；浏览器三门归 T-07。
...
- [x] `AC-004`…`AC-013`、`AC-021`、`AC-022`。
- [x] Map→Skill(含 module/common)→本票；blocked-by-auth。
- [x] 不造假 Evidence；不引入外置 IdP SDK。
...
- 交付数量空；未授权即停；冲突上报；完成出口仅 Lead 另派 I 后。
