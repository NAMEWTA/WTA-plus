...
DRIVER = Path('/tmp/wta-t50/run-notify-supported-mode-integration-v2.py')
spec = importlib.util.spec_from_file_location('t50_driver', DRIVER)
...
class DriverSafetyTest(unittest.TestCase):
    ...
    def test_owned_mysql_labels_loopback_and_trigger_option(self):
        ...
        self.assertIn('namewta.test.owner=T-50', args)
    ...
    def test_full_id_and_dual_label_discovery(self):
        ...
        with patch.object(driver, 'docker', return_value=full) as docker:
            ...
            self.assertEqual(docker.call_args.args, ('ps', '-aq', '--no-trunc',
                '--filter', 'label=namewta.test.owner=T-50', '--filter', 'label=namewta.test.run=run123'))
    ...
    def test_eight_exact_fresh_classes_positive_zero_skip(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for klass, _ in driver.SUITES:
                ...
                paths.append(path)
            paths[3].write_text(f'<testsuite name="{driver.MODE_CLASS}" tests="2" failures="0" errors="0" skipped="1"/>')
            with self.assertRaisesRegex(RuntimeError, 'zero tests'):
                driver.xml_verdict_for(driver.MODE_CLASS, paths[3], started)
    ...
    def test_source_and_eight_class_selector(self):
        ...
        self.assertIn("'NotifyWakeIntegrationTest,NotifySupportedModeIntegrationTest,'", source)
        ...
        self.assertIn("'-Dnotify.supported.mode.integration=true'", source)
        self.assertIn("'-DforkCount=1', '-DreuseForks=false'", source)
        self.assertIn('backend/wta-admin/target/surefire-reports/TEST-', source)
        self.assertLess(source.index('validate_mode_source(MODE_SOURCE.read_text())'),
                        source.index('docker_touched = True'))
        self.assertIn('cleanup_containers(run_id, captured) if docker_touched else ([], [])', source)
        self.assertIn('notify\\.supported\\.mode\\.integration', source)
    ...
    def test_mode_report_is_required_from_admin_module(self):
        self.assertEqual(driver.MODE_REPORT, driver.ROOT / (
            'backend/wta-admin/target/surefire-reports/TEST-' + driver.MODE_CLASS + '.xml'))
        with tempfile.TemporaryDirectory() as directory:
            stale = Path(directory) / 'stale-mode.xml'
            with self.assertRaisesRegex(RuntimeError, 'absent or stale'):
                driver.xml_counts_for(driver.MODE_CLASS, stale, time.time_ns())

    def test_mode_preflight_rejects_missing_class_flag_credential_or_fixture(self):
        actual = ('@EnabledIfSystemProperty(named = "notify.supported.mode.integration", matches = "true") '
                  'class NotifySupportedModeIntegrationTest { '
                  'String password = System.getenv("T50_MYSQL_PASSWORD"); '
                  'var fixture = new NotifyAtomicResultIntegrationTest(); fixture.open(); }')
        driver.validate_mode_source(actual)
        direct = actual.replace('new NotifyAtomicResultIntegrationTest(); fixture.open();',
                                'null; // notify.mysql.integration.url notify.redis.integration.port')
        driver.validate_mode_source(direct)
        for old, new in (('class NotifySupportedModeIntegrationTest', 'class OtherTest'),
                         ('notify.supported.mode.integration', 'notify.other.integration'),
                         ('T50_MYSQL_PASSWORD', 'NO_PASSWORD'),
                         ('fixture.open()', 'fixture.close()')):
            with self.subTest(old=old), self.assertRaises(RuntimeError):
                driver.validate_mode_source(actual.replace(old, new))
    ...
    def test_password_only_child_environment(self):
        self.assertEqual(driver.safe_env(T50_MYSQL_PASSWORD='private')['T50_MYSQL_PASSWORD'], 'private')
        self.assertNotIn('T50_MYSQL_PASSWORD', driver.safe_env())
        ...
        self.assertEqual(driver.MODE_CLASS, 'org.namewta.test.notify.NotifySupportedModeIntegrationTest')
        ...
        source = DRIVER.read_text()
        for name in ('T50_MYSQL_PASSWORD', 'T39_MYSQL_PASSWORD', 'T36_MYSQL_PASSWORD', 'T38_MYSQL_PASSWORD', 'T35_MYSQL_PASSWORD'):
