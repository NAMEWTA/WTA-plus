...
owned = Path(tempfile.mkdtemp(prefix='t21-e2e-', dir=root/'temp/team/lead'))
...
config.write_text('import base from '+json.dumps(str(root/'frontend/playwright.config.ts'))+';\n'
    ...
    'outputDir: '+json.dumps(str(root/'frontend/tests/e2e/reports/t21-regression'))+',\n'
...
env = os.environ.copy()
env['NAMEWTA_T21_RUN'] = owned.name
...
try:
    ...
    marker = ('NAMEWTA_T21_RUN=' + owned.name).encode()
    owned_children = []
