...
skill_scan: "scanned project Skill roots under <Path>.agents/skills/*/SKILL.md</Path> (engineering-standards, namewta-fullstack-development, ruoyi-common-modules-guide, ruoyi-module-guide, deploy-namewta-environment, java-api-compatibility, project-customization-delivery, upstream-fork-sync); excluded last three as non-axis for rename/W0b/publication; bound for T-03: engineering-standards, namewta-fullstack-development"
skill_bindings:
  - {"id":"engineering-standards","path":"<Path>.agents/skills/engineering-standards/SKILL.md</Path>","sha256":"acac7d1d4341d3361c298ebc5a61841d794cafbe9f235171b56fddd7ecb0037d","phase":"implement","operation":"extend-keep-inventory","inputs":["Ticket T-03","Tickets Map","Spec/ADR projections"],"outputs":["Evidence notes for T-03","next-step inputs"],"required":true,"on_failure":"block-ticket","references":[]}
  - {"id":"namewta-fullstack-development","path":"<Path>.agents/skills/namewta-fullstack-development/SKILL.md</Path>","sha256":"e17e31173638b06f93bb2c26cf3315f50a1a075b3e298434c98fc47d175d7792","phase":"implement","operation":"extend-keep-inventory","inputs":["Ticket T-03","Tickets Map","Spec/ADR projections"],"outputs":["Evidence notes for T-03","next-step inputs"],"required":true,"on_failure":"block-ticket","references":[]}
resource_claims:
  - "third-party-keep-inventory"
artifact: ticket
change: 2026-09-12-rename-ruoyi-dromara-to-wta
id: T-03
title: Build Third-party KEEP Inventory
status: done
planning_depth: standard
planning_depth_reason: 扩展 THIRD-PARTY-KEEP 与检测规则；执行依赖授权。
ready: true
risk: high
blocked_by: [T-01]
contract_ids: [AC-003, NAC-01, NAC-05]
owner: unassigned
expected_changes:
...
# Ticket T-03: Build Third-party KEEP Inventory

- **Ticket 文件：** `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/ticket/03-keep-inventory.md</Path>`
...
- **完成 Evidence：** `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/evidence/T-03.md</Path>`
...
- **目标：** 固化 sms4j/warm-flow/easy-es/mica-mqtt KEEP 检测与易混淆对照。
- **可观察产出：** KEEP 证据可被后续波次回归引用；第三方坐标未被改写。
- **来源：** ADR-003、AC-KEEP、HC-03/11。
- **当前事实：** THIRD-PARTY-KEEP.md 已有初稿。
- **Planning Depth 原因：** 扩展 THIRD-PARTY-KEEP 与检测规则；执行依赖授权。
...
|---|---|---|
| KEEP 清单与 inventory KEEP 行。 | 现有 THIRD-PARTY-KEEP。 | 改 SDK 坐标；改产品包名。 |

## 4. 要构建什么

在授权后补全 KEEP 行与检测命令；rename 波次必须跑 KEEP 回归。
...
|---|---|---|---|---|
| KEEP 清单 | 文档 | 读 THIRD-PARTY-KEEP | 四件套齐全 | evidence/T-03.md |
| 未改 SDK | rg groupId | 坐标原样 | 是 | evidence/T-03.md |
| 回归 | NAC-05 | 验收文案 | 无 zero-match 目标 | evidence/T-03.md |
...
- [ ] 未改第三方坐标。
- [ ] 禁止 zero-match 验收已写明。
- [ ] 实现开始前已完整读取 Tickets Map，已读取项目 Skill 入口并完整展开适用于 `ALL`/`T-03` 的匹配项。
- [ ] 验证矩阵全部执行并记录到 `<Path>{roots.state}/specdev/changes/2026-09-12-rename-ruoyi-dromara-to-wta/evidence/T-03.md</Path>`（完成时；**本轮不新建伪 Evidence**）。
...
依据 `<Path>{roots.workflows}/specdev/common/rules/skill-invocation.md</Path>`：本票 `skill_bindings` 绑定 Map 中适用于 ALL/`T-03` 的每一 Path。
