...
条目描述不够明确时，按路径读取对应接口与实现，不得凭空推断。路径相对工作区，前缀 `backend/`。未列出的方法以接口源码为准。
...
- 跨模块：`wta-api/src/main/java/org/namewta/system/api/UserService.java` → 按 ID 查账号/昵称/手机/邮箱/`UserDTO`，按角色/部门/岗位查用户。实现 `wta-modules/wta-system/src/main/java/org/namewta/system/service/impl/SysUserServiceImpl.java`。
...
- 翻译样例：`wta-common/wta-common-translation/src/main/java/org/namewta/common/translation/core/impl/UserNameTranslationImpl.java`；`wta-common/wta-common-translation/src/main/java/org/namewta/common/translation/core/impl/NicknameTranslationImpl.java`。

### 角色

- 跨模块：`wta-api/src/main/java/org/namewta/system/api/RoleService.java` 仅 `selectRoleNamesByIds`。实现 `service/impl/SysRoleServiceImpl.java`。
...
- 外部名称回显目前集中在 `wta-modules/wta-workflow/src/main/java/org/namewta/workflow/service/impl/FlwTaskAssigneeServiceImpl.java`。
...
- 跨模块：`wta-api/src/main/java/org/namewta/system/api/DeptService.java` — 部门名串、负责人 ID、部门列表、ID→名称 Map。实现 `service/impl/SysDeptServiceImpl.java`。
- 管理面：`service/ISysDeptService.java` — 树 CRUD、`checkDeptDataScope`。HTTP `controller/system/SysDeptController.java` `/system/dept`。
- 样例：`wta-modules/wta-workflow/src/main/java/org/namewta/workflow/rule/SpelRuleComponent.java` 调 `selectDeptLeaderById`；翻译 `wta-common/wta-common-translation/src/main/java/org/namewta/common/translation/core/impl/DeptNameTranslationImpl.java`。

### 岗位

- 跨模块：`wta-api/src/main/java/org/namewta/system/api/PostService.java` 仅 `selectPostNamesByIds`。实现 `service/impl/SysPostServiceImpl.java`。
...
- SpEL 模板 `wta-common/wta-common-mybatis/src/main/java/org/namewta/common/mybatis/enums/DataScopeType.java`。新增范围必须保持 Bean 名 `sdss`，或同步改枚举模板。

### 流程办理人

- 跨模块：`wta-api/src/main/java/org/namewta/system/api/TaskAssigneeService.java`，实现 `service/impl/SysTaskAssigneeServiceImpl.java`（内部再调 ISysUser/Role/Dept/Post）。调用方 `wta-modules/wta-workflow/src/main/java/org/namewta/workflow/service/impl/FlwTaskAssigneeServiceImpl.java`。
...
| 会话模型 | `wta-api/src/main/java/org/namewta/system/api/model/LoginUser.java` 持有 `userType`（登录域编码）、`userTypeId`、`clientPk` |
...
- 跨模块/翻译：注入 `org.namewta.common.core.service.DictService`（`wta-common/wta-common-core/src/main/java/org/namewta/common/core/service/DictService.java`），不是 ISysDict*。实现 `service/impl/SysDictTypeServiceImpl.java`。
...
- 使用方：`wta-common/wta-common-translation/src/main/java/org/namewta/common/translation/core/impl/DictTypeTranslationImpl.java`；`wta-common/wta-common-excel/src/main/java/org/namewta/common/excel/convert/ExcelDictConvert.java`；`wta-common/wta-common-excel/src/main/java/org/namewta/common/excel/core/ExcelDownHandler.java`；`wta-common/wta-common-core/src/main/java/org/namewta/common/core/validate/dicts/DictPatternValidator.java`。

### 参数配置

- 跨模块：`wta-api/src/main/java/org/namewta/system/api/ConfigService.java` — `getConfigValue` 及类型化 default 方法、`getConfigMap/ArrayMap/Object/Array`。实现 `service/impl/SysConfigServiceImpl.java`。
...
- 跨模块：`wta-api/src/main/java/org/namewta/system/api/OssService.java`。新代码在业务授权后使用 `resolveAccessUrl` 获取无签名到期时间的 PUBLIC 地址，或带 `expiresAt` 的 PRIVATE 短时签名；明确需要私有下载时使用 `presignDownload`，命名策略只能由服务端选择。`selectUrlByIds`、`selectByIds` 已标记为兼容接口，仅供翻译器和存量调用方，不作为新接口范式。
...
- 启动与诊断：`runner/SystemApplicationRunner.java` 仅调 `ossConfigService.init()` 从 DB 重建专用缓存，清除旧默认指针，仅唯一合法 PRIVATE 默认可供新上传。管理员通过 `POST /resource/oss/config/diagnose/{ossConfigId}` 独立诊断单个配置，结果携带固定来源/范围/依据/时间的有限事实；策略、ACL 仅是可理解的声明，匿名 HEAD/Range GET 只观察该对象，未知不证明全桶或匿名写安全。诊断不创建 Bucket、不修改 Policy，也不作为上传、下载、迁移或核心健康的前置门禁。业务仍验证对象 ACTIVE、service、owner/Client、当前配置访问类型，远端操作按真实结果反馈。`/actuator/health/readiness` 与 `/liveness` 不含 OSS 诊断，`/actuator/health/ossdiagnostics` 仅汇总已显式检查的快照（并非 DB 全部配置、也不保证未来操作成功）；每网络步骤诊断超时限 100ms–3s，最多五个顺序步骤，网络等待最多 15s。
- 配置变更：`event/OssConfigChangeEvent.java` + `listener/OssConfigChangeListener.java`，发布于 `service/impl/SysOssConfigServiceImpl.java`。配置管理写事务按配置主键序先锁所有现有配置；迁移新工单按配置→对象→工单锁序，在预检前后核对无凭据的物理身份。当前对象或未终结工单引用时冻结 configKey、Bucket、访问策略、endpoint、isHttps、region，可在同一身份下轮换凭据。来源 cleanup 在短事务持久化 `FAILED/COMPLETED/CLEANUP_OUTCOME_UNKNOWN` 执行权后事务外仅发一次有界 DELETE；未知结果的对象 HEAD 404 还需同一预算内 Bucket HEAD 成功才能视为来源缺失，且目标仍在才完成。Bucket 不可确认时保留 UNKNOWN，恢复、回滚、重试和新工单不能绕过栅栏，取消请求不代表远端 DELETE 被撤回。
...
- 业务通知跨模块入口：`wta-api/src/main/java/org/namewta/notify/api/NotificationApplicationService.java`。调用方提交 `NotificationCommand`，由 `wta-notify` 的 UseCase 校验目标、保存意图并通过 Outbox 异步投递。
